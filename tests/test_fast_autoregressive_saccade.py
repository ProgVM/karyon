...
# ==============================================================================
# Saccadic Latent Recirculation Engine:
# 1. Bidirectional Deliberation on Prompt
# 2. Recurrent Cell for Motor Generation with Saccadic Pointer and Local Memory
# ==============================================================================

class SaccadicRecurrentCell(nn.Module):
    ...
    def forward(self, x_t, h_prev, p_field):
        # x_t: [B, D], h_prev: [B, D], p_field: [B, P, D]
        h_next = self.gru(x_t, h_prev) # [B, D]
        
        # Saccade into prompt field
        q = self.q_proj(h_next).unsqueeze(1) # [B, 1, D]
        k = self.k_proj(p_field) # [B, P, D]
        v = self.v_proj(p_field) # [B, P, D]
        
        scores = torch.bmm(q, k.transpose(1, 2)) / math.sqrt(self.dim) # [B, 1, P]
        attn = F.softmax(scores, dim=-1)
        gaze = torch.bmm(attn, v).squeeze(1) # [B, D]
        
        fused = self.out_gate(torch.cat([h_next, gaze], dim=-1))
        out = self.norm(h_next + fused)
        return out, h_next

class FastSaccadicMind(nn.Module):
    def __init__(self, vocab_size=258, dim=128, wave_steps=4):
        ...
        # Motor Decoder Cell
        self.cell = SaccadicRecurrentCell(dim)
    ...
    def forward_train(self, p_tokens, a_tokens):
        ...
        h_prev = p_field.mean(dim=1) # Initial state seeded by global prompt representation
        
        logits_list = []
        for t in range(A):
            x_t = self.emb(a_tokens[:, t])
            out_t, h_prev = self.cell(x_t, h_prev, p_field)
            logits_t = self.head(out_t)
            logits_list.append(logits_t)
            
        return torch.stack(logits_list, dim=1) # [B, A, V]
...
agent = FastSaccadicMind(vocab_size=258, dim=128, wave_steps=4).to(device)
opt = torch.optim.AdamW(agent.parameters(), lr=0.003, weight_decay=1e-5)
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=120, eta_min=1e-4)

print("\n--- Training FastSaccadicMind (120 Epochs) ---")
...
agent.eval()
print("\n" + "=" * 80)
print("=== FAST SACCADIC MIND MULTI-DOMAIN EVALUATION ===")
print("=" * 80)
