"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-40 (FAIR PACKED PARITY BENCHMARK)
Equal 100-Step Optimization Budget & Held-Out Real Validation Evaluation:
Baseline 512-Byte Truncated Stream vs EXP-40 2048-Byte Continuous Packed Stream.
...
print(f"\n[EXP-40] Initializing Fair Packed Parity Benchmark on: {device_str.upper()}")
...
# =============================================================================
# 5. DATASETS: BASELINE (512 TRUNCATED), PROPOSED (2048 PACKED), HELD-OUT TEST
# =============================================================================
...
# =============================================================================
# 6. EXP-40 MASTER EXECUTION & AUTOMATED DECISION ENGINE
# =============================================================================
def run_exp_40_benchmark():
    print("\n" + "="*85)
    print(" === EXP-40: FAIR PACKED LONG-CONTEXT PARITY BENCHMARK (100 STEPS) ===")
    print("="*85)
    ...
    # 1. Baseline Training Loader: 100 batches of B=32, S=512
    dataset_train_base = BaselineTruncatedDataset(train_split, tokenizer, max_samples=NUM_STEPS, max_len=512)
    loader_train_base = DataLoader(dataset_train_base, batch_size=32, shuffle=True, collate_fn=collate_pad_fn, drop_last=True)

    # 2. Proposed Training Loader: 100 batches of B=16, S=2048 (0% padding)
    dataset_train_prop = ContinuousPackedDataset(train_split, tokenizer, max_samples=NUM_STEPS, seq_len=2048)
    loader_train_prop = DataLoader(dataset_train_prop, batch_size=16, shuffle=True, collate_fn=collate_packed_fn, drop_last=True)

    # 3. Held-Out Common Validation Set (Real Unpadded Full-Length Dialogues)
    dataset_val = ContinuousPackedDataset(val_split, tokenizer, max_samples=15, seq_len=2048)
    loader_val = DataLoader(dataset_val, batch_size=8, shuffle=False, collate_fn=collate_packed_fn, drop_last=True)

    logger.info(f"Parity Setup Ready. Train Steps: {NUM_STEPS} for BOTH models | Val Batches: {len(loader_val)}")

    criterion = nn.CrossEntropyLoss(ignore_index=256)

    # Warmup + Cosine Scheduler Lambda (100 steps total, 5 steps warmup)
    def get_lr_multiplier(step):
        if step < 5:
            return float(step + 1) / 5.0
        progress = float(step - 5) / float(max(1, NUM_STEPS - 5))
        return 0.5 * (1.0 + math.cos(math.pi * progress * 0.7)) # Decay to ~1e-3

    # -------------------------------------------------------------------------
    # PART A: RUN BASELINE 512 TRUNCATED STREAM (100 STEPS)
    # -------------------------------------------------------------------------
    print("\n" + "-"*85)
    print(" [1/2] RUNNING BASELINE: 512-BYTE TRUNCATED STREAM (100 GRADIENT STEPS)")
    print("-" * 85)
    ...
    for idx, batch_tokens in enumerate(loader_train_base):
        ...
        if (idx + 1) % 25 == 0 or idx == NUM_STEPS - 1:
            print(f"  Baseline Step [{idx+1:03d}/{NUM_STEPS}] | Train Loss: {loss:.4f} (PPL: {math.exp(min(loss, 20.0)):.2f})")
    ...
    # -------------------------------------------------------------------------
    # PART B: RUN PROPOSED EXP-40 CONTINUOUS PACKED STREAM (100 STEPS)
    # -------------------------------------------------------------------------
    print("\n" + "-"*85)
    print(" [2/2] RUNNING PROPOSED: EXP-40 CONTINUOUS PACKED STREAM (100 GRADIENT STEPS)")
    print("-" * 85)
    ...
    for idx, batch_tokens in enumerate(loader_train_prop):
        if idx >= NUM_STEPS: break
        batch_tokens = batch_tokens.to(device)
        in_seq = batch_tokens[:, :-1]
        tgt_seq = batch_tokens[:, 1:]
        ...
        if (idx + 1) % 25 == 0 or idx == NUM_STEPS - 1:
            print(f"  Proposed Step [{idx+1:03d}/{NUM_STEPS}] | Train Loss: {loss:.4f} (PPL: {math.exp(min(loss, 20.0)):.2f})")
    ...
    # -------------------------------------------------------------------------
    # PART C: HELD-OUT COMMON VALIDATION SET EVALUATION (FAIR ARBITER)
    # -------------------------------------------------------------------------
    print("\n" + "="*85)
    print(" === HELD-OUT VALIDATION SET EVALUATION (UNBIASED BENCHMARK) ===")
    print("="*85)
    model_base.eval()
    model_prop.eval()
    ...
    with torch.no_grad():
        for batch_tokens in loader_val:
            ...
            # Evaluate Baseline
            loss_b = model_base.forward_sequence(in_seq, tgt_seq, hu_val, criterion, chunk_size=64, optimizer=None)
            val_loss_base_accum += loss_b

            # Evaluate Proposed
            loss_p = model_prop.forward_sequence(in_seq, tgt_seq, hu_val, criterion, chunk_size=64, optimizer=None)
    ...
    print(f"  Held-Out Validation Loss (Baseline 512): {final_val_loss_base:.4f} (PPL: {math.exp(min(final_val_loss_base, 20.0)):.2f})")
    print(f"  Held-Out Validation Loss (Proposed EXP-40): {final_val_loss_prop:.4f} (PPL: {math.exp(min(final_val_loss_prop, 20.0)):.2f})")
    print("="*85)

    # -------------------------------------------------------------------------
    # PART D: DIAGNOSTIC SPEECH SAMPLE (KEP RULE #4)
    # -------------------------------------------------------------------------
    print("\n" + "="*85)
    ...
    sample_base = model_base.generate_thought_and_speech(prompt_1, diag_hu, max_generated_tokens=75)
    sample_prop = model_prop.generate_thought_and_speech(prompt_1, diag_hu, max_generated_tokens=75)

    print(f"  [Baseline Output]: \"{sample_base}\"")
    print(f"  [EXP-40 Proposed Output]: \"{sample_prop}\"")
    print("="*85)
    ...
    print("\n" + "="*85)
    print(" === EXP-40 FINAL TELEMETRY AUDIT TABLE (FAIR 100-STEP PARITY) ===")
    print("="*85)
    print(f"{'Metric':<34} | {'Baseline (512 Truncated + Pad)':<30} | {'Proposed (EXP-40 2048 Packed Stream)':<36}")
    print("-" * 105)
    print(f"{'Optimization Steps':<34} | {'100 Gradient Updates':<30} | {'100 Gradient Updates':<36}")
    print(f"{'Context Window (S)':<34} | {'512 Bytes (~80 words)':<30} | {'2,048 Bytes (~400 words)':<36}")
    print(f"{'Padding Overhead':<34} | {'~50% (Padded with 256)':<30} | {'0% (100% Pure Signal)':<36}")
    print(f"{'Final Train Loss':<34} | {final_train_loss_base:<30.4f} | {final_train_loss_prop:<36.4f}")
    print(f"{'Held-Out Validation Loss':<34} | {final_val_loss_base:<30.4f} | {final_val_loss_prop:<36.4f}")
    print(f"{'Validation Perplexity (PPL)':<34} | {math.exp(min(final_val_loss_base, 20.0)):<30.2f} | {math.exp(min(final_val_loss_prop, 20.0)):<36.2f}")
    print(f"{'Throughput Speed':<34} | {f'{tok_per_sec_base:,.1f} tok/s':<30} | {f'{tok_per_sec_prop:,.1f} tok/s':<36}")
    print(f"{'Peak VRAM Memory':<34} | {f'{vram_base:.1f} MB':<30} | {f'{vram_prop:.1f} MB':<36}")
    print(f"{'Total Execution Time':<34} | {f'{t_total_base:.2f} s':<30} | {f'{t_total_prop:.2f} s':<36}")
    print("="*105)

    print(f"\n[EXP-40 Decision Engine] Delta Validation Loss: {delta_val_loss:+.4f} | Speed Ratio: {speed_ratio:.1f}%")
    
    if delta_val_loss <= -0.08 and speed_ratio >= 75.0:
        verdict = "🟢 POSITIVE — Significant long-context packed gain on held-out validation with high throughput. Ready for production merge."
