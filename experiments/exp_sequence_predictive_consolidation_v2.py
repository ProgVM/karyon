# experiments/exp_sequence_predictive_consolidation_v2.py
"""
===============================================================================
KARYON ENGINEERING PROTOCOL (KEP) EXPERIMENT #10.1 (ITERATION 2)
Topic: Adaptive High-Frequency Sequence Predictive Consolidation Engine
Author: Karyon-CoRE Open-Source Research Team (2026)
===============================================================================

Hypothesis:
    Increasing sleep consolidation frequency (triggering sleep at Energy < 0.50, 
    yielding ~6-8 sleep cycles per 800 steps) and modulating sleep learning rate 
    by Arousal (NA) will prevent trajectory staleness, dropping Free Energy error 
    degradation under 10% while preserving >1.4x perception speedup.

Control Group: 
    Traditional Step-by-Step Backprop Cramming.

Experimental Group: 
    Adaptive High-Frequency Trajectory Consolidation Engine (Energy < 0.50).

Metrics Tracked:
    1. Sleep Consolidation Cycles Triggered
    2. Mean Variational Free Energy (F_t)
    3. Hidden State Divergence (L2 Norm)
    4. Wake Perception Speedup Ratio (x)
===============================================================================
"""
...
print(f"[KEP Experiment #10.1] Execution Context: {device.type.upper()}")
...
class SequenceTrajectoryMemory:
    ...
    def store_trajectory(self, h0_fast, h0_slow, w_sequence, target_sequence, fe_val):
        """Stores a short temporal trajectory snippet with its surprise level."""
        if len(self.trajectory_chunks) >= self.max_chunks:
            self.trajectory_chunks.pop(0)
            
        self.trajectory_chunks.append({
            "h0_fast": h0_fast.detach().clone(),
            "h0_slow": h0_slow.detach().clone(),
            "w_sequence": w_sequence.detach().clone(),
            "target_sequence": target_sequence.detach().clone(),
            "fe_val": fe_val
        })
...
# =============================================================================
# 2. ADAPTIVE WAKE-SLEEP PREDICTIVE AGENT
# =============================================================================

class AdaptiveTrajectoryAgent(nn.Module):
    ...
    def perception_step(self, input_id, h_fast, h_slow):
        """Wake Phase: Single step perception without Backprop."""
        ...
        # Adaptive Energy Drain
        self.energy = max(0.0, self.energy - 0.008)
        # Higher sleep frequency threshold (Energy < 0.50)
        should_sleep = self.energy < 0.50

        return h_f_next, h_s_next, x_emb, logits, free_energy, should_sleep

    def consolidate_trajectory_sleep_v2(self, trajectory_memory, optimizer, criterion, sleep_epochs=2):
        """
        Sleep Phase v2: Replays recorded trajectory chunks with adaptive learning rates.
        """
        num_chunks = len(trajectory_memory.trajectory_chunks)
        if num_chunks == 0:
            self.energy = 1.0
            return 0.0
        ...
        for ep in range(sleep_epochs):
            for chunk in trajectory_memory.trajectory_chunks:
                ...
                w_seq = chunk["w_sequence"]
                target_seq = chunk["target_sequence"]
...
def run_adaptive_trajectory_benchmark(mode="control_cramming", stream=None, seq_len=800):
    agent = AdaptiveTrajectoryAgent().to(device)
    ...
    for step in range(seq_len - 1):
        ...
        if mode == "control_cramming":
            ...
            # Trigger Sleep Phase under adaptive high-frequency schedule
            if should_sleep and len(trajectory_memory.trajectory_chunks) > 0:
                sleep_cycles_triggered += 1
                agent.consolidate_trajectory_sleep_v2(trajectory_memory, optimizer, criterion)
...
if __name__ == "__main__":
    ...
    control_res = run_adaptive_trajectory_benchmark(mode="control_cramming", stream=stream, seq_len=STREAM_LEN)

    print("[KEP Step 2/2] Running EXPERIMENTAL Group (Adaptive High-Frequency Trajectory Consolidation)...")
    experimental_res = run_adaptive_trajectory_benchmark(mode="trajectory_consolidation_v2", stream=stream, seq_len=STREAM_LEN)
    ...
    print("="*80)
    print(f"{'Metric':<32} | {'Control Cramming':<18} | {'Trajectory v2':<18} | {'Delta / Gain':<15}")
    print("-" * 88)
    ...
    if speedup_ratio >= 1.3 and fe_change_pct <= 10.0:
        print("🟢 VERDICT: POSITIVE EXPERIENCE ADOPTED!")
        print(f"   Reason: Adaptive trajectory consolidation achieved {speedup_ratio:.2f}x speedup with low Free Energy error ({fe_change_pct:+.2f}%).")
        print("   Action: Adopt Trajectory-Preserving Predictive Consolidation v2 into master Karyon!")
