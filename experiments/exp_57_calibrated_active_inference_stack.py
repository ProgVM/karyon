# experiments/exp_57_calibrated_active_inference_stack.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-57 (CALIBRATED ACTIVE INFERENCE CORTICAL STACK)
Balanced Synaptic Gradient Highway (sigma=1/D^(1/4)=0.25) + 2-Stage Cascaded SSD +
Active Inference Latent World Model (F_t) + Dopaminergic Motor Resonance.
...
# =============================================================================
# 4. EXP-57 CALIBRATED ACTIVE INFERENCE 2-STAGE CORTICAL AGENT
# =============================================================================
class CalibratedActiveCorticalAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # Calibrated Synaptic Scale sigma = 1 / D^(1/4) = 0.25
        synaptic_scale = 1.0 / math.sqrt(math.sqrt(self.text_dim))
        nn.init.normal_(self.pos_embeddings.byte_embed.weight, mean=0.0, std=synaptic_scale)
        ...
        # Active Inference Latent World Model (Predictive Coding)
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        motor_gain = (1.0 + 1.5 * da_level).unsqueeze(1)
        ...
        for chunk_idx in range(num_chunks):
            ...
            # Dopaminergic Afferent-Efferent Motor Resonance
            ...
            # Active Inference World Model Predictor
            w_slice = chunk_emb[:, -1, :]
            h_last = h_s2[:, -1, :]
...
# =============================================================================
# 5. REAL DATASET & RUNNER (WSD SCHEDULE: 3.0e-3 -> 1.0e-3)
# =============================================================================
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-57 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_57_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-57 (CALIBRATED ACTIVE INFERENCE STACK)] ===")
    print("="*85)
    ...
    # 1. EVALUATE CALIBRATED AGENT
    print("\n[1/1] Training Calibrated 2-Stage Cortical Stack with Active Inference (300 Steps)...")
    torch.manual_seed(42)
    agent = CalibratedActiveCorticalAgent(config, device_str=device_str).to(device)
    ...
    # Warmup-Stable-Decay: 30 warmup -> 200 stable (3.0e-3) -> 70 smooth decay (to 1.0e-3)
    warmup_steps = 30
    stable_steps = 200
    ...
    for step in range(num_eval_steps):
        ...
        with torch.amp.autocast(device_type=device_str, dtype=torch.float16, enabled=use_amp):
            tot_loss, speech_loss, fe_val = agent.forward_sequence(input_s, target_s, hu, criterion, chunk_size=chunk_size)
        ...
        # Real Homeostatic Somatic Coupling with Active Inference
        with torch.no_grad():
        ...
        if (step + 1) % 50 == 0 or step == num_eval_steps - 1:
            ...
            cur_fe = sum(fe_list[-20:]) / min(len(fe_list), 20)
            cur_lr = optimizer.param_groups[0]['lr']
            print(f"  Step [{step+1:03d}/{num_eval_steps}] | Loss: {speech_loss:.4f} (Avg: {cur_loss:.4f}, PPL: {math.exp(cur_loss):.2f}) | F_t: {cur_fe:.4f} | LR: {cur_lr:.6f}")
    ...
    print("\n" + "="*85)
    print(" === [KEP EXP-57 CALIBRATED CORTICAL STACK TELEMETRY DASHBOARD] ===")
    print("="*85)
    print(f"{'Performance Metric':<36} | {'EXP-57 Calibrated Value':<40}")
    print("-" * 85)
