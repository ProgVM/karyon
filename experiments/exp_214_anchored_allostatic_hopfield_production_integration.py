# experiments/exp_214_anchored_allostatic_hopfield_production_integration.py
"""
===============================================================================
EXP-214: Anchored Allostatically-Gated Hopfield Attractor Integration into CoRE Agent
Grounding: KEP Principle 2 (Living AGI & Biological Realism),
           KEP Principle 5 (Autonomous Tool Action Over Chatter),
           KEP Principle 7 (Axiom of Unshackled Flow),
           KEP Principle 14 (Axiom of Allostatic Dynamic Forces — No Static Constants),
           KEP Principle 15 (Net2Net Smooth Grafting: Strict Identity at Birth).
===============================================================================
Hypothesis:
In EXP-205, Anchored Allostatically-Gated Hopfield Attractor Energy Relaxation demonstrated
a substantial 0.4129 nats speech loss reduction (Loss Delta = 0.4129 >= 0.08) by anchoring
attractor basins directly to pre-trained token embedding centroids in motor space, scaling precision
via allostatic state u_t and Free Energy F_t, and grafting into the motor stream with a zero-initialized
residual projection.
However, in `karyon_agent.py` lines 1216, 1843, and 2090, `attractor_head` remains the dense unanchored
random Hopfield head (`DesaturatedHopfieldAttractorHead`), leaving the motor representation slightly
unaligned with byte vocabulary centroids.

Integrating `AnchoredAllostaticHopfieldMotorRelaxation` into `karyon_agent.py` as an active motor stage:
  h_relaxed_anchored = h_relaxed + step_gain(u_t, F_t) * W_out(Snapped(Norm(h_relaxed), Basins))
with W_out initialized to zero (exact mathematical zero-delta identity at birth t_0, KEP Principle 15)
will align cortical attractor trajectories with the phonetic-semantic byte manifold, yielding
a significant empirical breakthrough in speech loss (Loss Delta >= 0.08) and accelerating convergence.
"""

import sys
import os
import time
import math
import json
import logging
from typing import Tuple, List, Optional, Dict
import torch
import torch.nn as nn
import torch.nn.functional as F

sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

from karyon_entity import KaryonEntity
from karyon_hardware import get_hardware_engine

logging.basicConfig(level=logging.INFO, format="%(module)-15s | %(levelname)-8s | %(asctime)s | %(message)s")
logger = logging.getLogger("EXP-214")


class AnchoredAllostaticHopfieldMotorRelaxation(nn.Module):
    """
    Anchored Allostatically-Gated Hopfield Attractor Energy Relaxation Module (EXP-205 & EXP-214).
    Anchors basins to pre-trained token embedding centroids and uses zero-initialized
    residual projections to ensure exact zero-delta identity at birth.
    """
    def __init__(self, hidden_dim: int, embed_weights: torch.Tensor, device_str: str = 'cpu'):
        super().__init__()
        self.device = torch.device('cuda' if 'cuda' in device_str else 'cpu')
        self.hidden_dim = hidden_dim
        self.vocab_size = embed_weights.size(0)
        text_dim = embed_weights.size(1)

        if text_dim != hidden_dim:
            proj = nn.Linear(text_dim, hidden_dim, bias=False).to(self.device)
            nn.init.orthogonal_(proj.weight)
            anchored_basins = proj(embed_weights.detach().to(self.device))
        else:
            anchored_basins = embed_weights.detach().to(self.device)

        self.basins = nn.Parameter(F.normalize(anchored_basins, p=2, dim=-1))

        # Zero-initialized residual output projection (Net2Net Smooth Grafting / KEP Principle 15)
        self.out_proj = nn.Linear(hidden_dim, hidden_dim, bias=False).to(self.device)
        nn.init.zeros_(self.out_proj.weight)

        # Allostatic Precision & Step Controller Net
        self.allostatic_controller = nn.Sequential(
            nn.Linear(6 + 1, 32),
            nn.SiLU(),
            nn.Linear(32, 2),
            nn.Sigmoid()
        ).to(self.device)

    def forward(self, h_relaxed: torch.Tensor, u_t: torch.Tensor, fe_loss: Optional[torch.Tensor] = None) -> torch.Tensor:
        is_2d = (h_relaxed.dim() == 2)
        if is_2d:
            h_state = h_relaxed.unsqueeze(1)
        else:
            h_state = h_relaxed

        B, S, D = h_state.shape

        fe_scalar = fe_loss.detach() if fe_loss is not None else torch.tensor(0.50, device=self.device)
        fe_norm = torch.clamp(fe_scalar / 10.0, 0.0, 1.0).view(-1, 1)

        if u_t.dim() == 2:
            u_exp = u_t.unsqueeze(1).expand(B, S, -1) if u_t.size(0) == B else u_t[0:1].unsqueeze(1).expand(B, S, -1)
        else:
            u_exp = u_t.view(1, 1, -1).expand(B, S, -1)

        fe_exp = fe_norm.view(1, 1, 1).expand(B, S, -1)
        ctrl_in = torch.cat([u_exp, fe_exp], dim=-1) # [B, S, 7]
        ctrl_out = self.allostatic_controller(ctrl_in) # [B, S, 2]

        beta_precision = 4.0 + ctrl_out[..., 0:1] * 8.0
        step_gain = ctrl_out[..., 1:2] * 0.05

        norm_h = F.normalize(h_state, p=2, dim=-1)
        sims = torch.einsum('bsd,vd->bsv', norm_h, F.normalize(self.basins, p=2, dim=-1))
        attn_weights = F.softmax(sims * beta_precision, dim=-1)
        snapped_state = torch.einsum('bsv,vd->bsd', attn_weights, self.basins)

        out_state = h_state + step_gain * self.out_proj(snapped_state)
        if is_2d:
            return out_state.squeeze(1)
        return out_state


def evaluate_model(brain, entity, text_samples, num_steps=25, lr=1e-3):
    hw = get_hardware_engine()
    criterion = nn.CrossEntropyLoss(ignore_index=256)
    optimizer = torch.optim.AdamW(brain.parameters(), lr=lr, weight_decay=1e-4)

    step_losses = []
    step_fe_losses = []

    start_time = time.perf_counter()

    for step in range(num_steps):
        text = text_samples[step % len(text_samples)]
        prompt_ids = brain.tokenizer.encode(text)
        seq_t = torch.tensor([prompt_ids[:-1]], dtype=torch.long, device=hw.device)
        target_t = torch.tensor([prompt_ids[1:]], dtype=torch.long, device=hw.device)

        optimizer.zero_grad()
        tot_loss, speech_loss, fe_loss, _, _, _, _ = brain.forward_sequence(
            seq_t, target_t, entity.hu, criterion, chunk_size=seq_t.size(1), use_checkpointing=False
        )
        tot_loss.backward()
        torch.nn.utils.clip_grad_norm_(brain.parameters(), 1.0)
        optimizer.step()

        step_losses.append(float(speech_loss))
        step_fe_losses.append(float(fe_loss))

    elapsed = time.perf_counter() - start_time
    total_tokens = sum(len(brain.tokenizer.encode(t)) - 1 for t in text_samples[:num_steps])
    tok_per_sec = total_tokens / elapsed if elapsed > 0 else 0.0

    return {
        "final_loss": step_losses[-1],
        "initial_loss": step_losses[0],
        "mean_loss": sum(step_losses) / len(step_losses),
        "final_fe": step_fe_losses[-1],
        "initial_fe": step_fe_losses[0],
        "tok_per_sec": tok_per_sec,
        "elapsed": elapsed
    }


def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-214: ANCHORED ALLOSTATIC HOPFIELD INTEGRATION BENCHMARK]")
    logger.info("=" * 80)

    hw = get_hardware_engine()
    logger.info(f"Target Accelerator: {hw.device_str.upper()}")

    text_samples = [
        "The quick brown fox jumps over the lazy dog near the riverbank with high agility.",
        "Active Inference formulates brain dynamics as continuous minimization of variational free energy.",
        "Homeostasis and allostasis regulate physiological variables through predictive bodily setpoints.",
        "Neural state space duality enables zero-loop associative parallel scans across deep cortical layers.",
        "Continuous Hopfield attractors snap neural trajectories into discrete conceptual semantic basins.",
        "Cortical laminar hierarchy routes top-down predictions and bottom-up precision-weighted error residuals."
    ] * 5

    # 1. Baseline Evaluation
    entity_base = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_base = entity_base.brain
    logger.info("Running Baseline Evaluation...")
    b_results = evaluate_model(brain_base, entity_base, text_samples, num_steps=25)

    # 2. Proposed Evaluation (EXP-214 Anchored Hopfield Integration)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain

    embed_w = brain_prop.pos_embeddings.byte_embed.weight.data
    anchored_hopfield = AnchoredAllostaticHopfieldMotorRelaxation(
        hidden_dim=brain_prop.hidden_dim,
        embed_weights=embed_w,
        device_str=hw.device_str
    ).to(hw.device)
    brain_prop.anchored_hopfield_motor_relaxation = anchored_hopfield

    # Non-invasive wrapper on compute_volitional_logits_with_grafts to inject anchored hopfield relaxation
    orig_compute_volitional_logits_with_grafts = brain_prop.compute_volitional_logits_with_grafts

    def wrapped_compute_volitional_logits(h_relaxed: torch.Tensor, effective_u_t: torch.Tensor) -> torch.Tensor:
        h_anchored = brain_prop.anchored_hopfield_motor_relaxation(h_relaxed, effective_u_t)
        return orig_compute_volitional_logits_with_grafts(h_anchored, effective_u_t)

    brain_prop.compute_volitional_logits_with_grafts = wrapped_compute_volitional_logits

    logger.info("Running Proposed Anchored Allostatic Hopfield Integration Evaluation...")
    p_results = evaluate_model(brain_prop, entity_prop, text_samples, num_steps=25)

    loss_delta = b_results['final_loss'] - p_results['final_loss']
    fe_delta = b_results['final_fe'] - p_results['final_fe']

    logger.info("=" * 80)
    logger.info("📊 === EXP-214 TELEMETRY REPORT ===")
    logger.info(f"  - Baseline Final Loss : {b_results['final_loss']:.4f} nats | Throughput: {b_results['tok_per_sec']:.1f} tok/s")
    logger.info(f"  - Proposed Final Loss : {p_results['final_loss']:.4f} nats | Throughput: {p_results['tok_per_sec']:.1f} tok/s")
    logger.info(f"  - Loss Delta (B - P)  : {loss_delta:.4f} nats")
    logger.info(f"  - Free Energy Delta   : {fe_delta:.6f}")
    logger.info(f"  - Baseline Duration   : {b_results['elapsed']:.3f} s")
    logger.info(f"  - Proposed Duration   : {p_results['elapsed']:.3f} s")

    verdict = "POSITIVE" if (loss_delta >= 0.08 or (loss_delta >= 0.02 and p_results['tok_per_sec'] >= 0.90 * b_results['tok_per_sec'])) else "NEUTRAL"

    results = {
        "exp_id": "EXP-214",
        "verdict": verdict,
        "base_initial_loss": b_results['initial_loss'],
        "base_final_loss": b_results['final_loss'],
        "proposed_initial_loss": p_results['initial_loss'],
        "proposed_final_loss": p_results['final_loss'],
        "loss_delta": loss_delta,
        "fe_delta": fe_delta,
        "throughput_tok_per_sec": p_results['tok_per_sec'],
        "execution_time_s": p_results['elapsed']
    }

    with open("experiments/exp_214_results.json", "w") as f:
        json.dump(results, f, indent=2)

    logger.info(f"🏆 Final Verdict: 🟢 {verdict}" if verdict == "POSITIVE" else f"🏆 Final Verdict: ⚪ {verdict}")
    return results


if __name__ == "__main__":
    run_benchmark()
