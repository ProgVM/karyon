# experiments/exp_90_1_pure_checkpointing_cpp_critic.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-90.1
Hypothesis:
1. Native C++20 TDFreeEnergyCritic fully satisfies Principle 1 (C++ as Engine, Python as Client).
2. Pure Activation Checkpointing on Cortical Stages preserves 100% of mathematical convergence
   (Delta Loss ~ 0.00) while reducing peak VRAM by >3.3 GB (from 9.7GB to ~6.4GB), enabling higher
   batch sizes and context lengths without any regression.
...
def prepare_packed_stream(num_batches: int = 150, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-90.1 (S={seq_len}, Steps={num_batches})...")
...
class PureCheckpointedCoREAgent(CoREAgent):
    """Applies pure activation checkpointing to Stage 1 and Stage 2 without architectural noise."""
...
def run_exp_90_1_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-90.1 (PURE CORTICAL CHECKPOINTING)] ===")
    print("="*85)
    ...
    # 2. EVALUATE PROPOSED (Pure Activation Checkpointing)
    print("\n" + "-"*85)
    print(" >>> RUNNING PHASE 2: PROPOSED (Pure Checkpointed CoREAgent) <<<")
    print("-"*85)
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats()

    agent_prop = PureCheckpointedCoREAgent(config, device=device_str).to(device)
