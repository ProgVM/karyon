# experiments/exp_65_bidirectional_topdown_cortical_feedback.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-65 (BI-DIRECTIONAL TOP-DOWN CORTICAL FEEDBACK - BTCF)
Tests Bi-Directional Hierarchical Active Inference:
  1. Top-Down Semantic Feedback: h_in = in_proj(emb) + topdown_semantic_proj(h2_prev)
  2. Stage 1 (Fast Morpho-Syntactic Sheet) decodes phonemes guided by macro-semantic context
  3. LPER Error Residuals: e1 = h1 - topdown_pred(h1_prev)
  4. Stage 2 (Slow Semantic-Discourse Sheet) integrates unpredicted novelty e1
This completes the canonical Bastos-Friston bi-directional cortical loop to eliminate
pseudo-morphemic drift and achieve deep semantic convergence.
...
# =============================================================================
# 4. EXP-65 MASTER AGENT WITH BI-DIRECTIONAL TOP-DOWN CORTICAL FEEDBACK (BTCF)
# =============================================================================
class Exp65BiDirectionalCorticalAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # Top-Down Semantic Feedback Projection (Stage 2 -> Stage 1)
        self.topdown_semantic_proj = nn.Sequential(
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.SiLU(),
            nn.Linear(self.hidden_dim, self.hidden_dim)
        ).to(self.device)
        nn.init.zeros_(self.topdown_semantic_proj[2].weight)
        nn.init.zeros_(self.topdown_semantic_proj[2].bias)
        ...
        # LPER Top-Down Predictive Filter
        ...
        # Stage 2: Slow Semantic Sheet with Exact Parallel Log-Decay Scan
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        for chunk_idx in range(num_chunks):
            ...
            # --- 1. BI-DIRECTIONAL TOP-DOWN SEMANTIC FEEDBACK ---
            # Stage 1 input receives bottom-up byte embeddings + top-down semantic context from Stage 2
            topdown_prior = self.topdown_semantic_proj(h2_prev_last) # [B, 1, D]
            h_in = self.in_proj(chunk_emb) + topdown_prior
    ...
    def generate_natural_speech(self, prompt: str, hu_state, max_tokens: int = 75) -> str:
        ...
        for c_idx in range(0, prompt_len, 64):
            ...
            topdown_prior = self.topdown_semantic_proj(h2_prev_last)
            h_in = self.in_proj(c_emb) + topdown_prior
        ...
        with torch.no_grad():
            for step in range(max_tokens):
                ...
                topdown_prior = self.topdown_semantic_proj(h2_prev_last)
                h_in = self.in_proj(t_emb) + topdown_prior
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-65 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_65_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-65 (BI-DIRECTIONAL TOP-DOWN CORTICAL FEEDBACK)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-65 Bi-Directional Cortical Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp65BiDirectionalCorticalAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            g_top = 0.0
            for p in agent.topdown_semantic_proj.parameters():
                if p.grad is not None:
                    g_top = max(g_top, p.grad.norm().item())
            g_pred = 0.0
            for p in agent.topdown_pred_net.parameters():
                if p.grad is not None:
                    g_pred = max(g_pred, p.grad.norm().item())

            print("\n" + "="*95)
            print(f" === [KEP EXP-65 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | TopDown Prior: {g_top:.6f} | LPER Pred: {g_pred:.6f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-65 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-65 Bi-Directional Cortical Value':<40}")
    print("-" * 95)
