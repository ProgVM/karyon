# experiments/exp_predictive_coding_online.py
"""
===============================================================================
KARYON ENGINEERING PROTOCOL (KEP) EXPERIMENT #11
Topic: Continuous Bi-Directional Predictive Coding (Local Layer Error Propagation)
Author: Karyon-CoRE Open-Source Research Team (2026)
===============================================================================

Hypothesis:
    Replacing global BPTT (.backward()) with Continuous Local Predictive Coding 
    (where each layer calculates its local prediction error vector e_t = x - x_hat 
    and updates weights in O(1) during the forward pass) will eliminate stale 
    gradient lags, enable true real-time continuous learning without artificial 
    sleep crutches, and maintain low Free Energy.

Control Group: 
    Traditional Step-by-Step Global Backpropagation (.backward() + Adam).

Experimental Group: 
    Continuous Local Predictive Coding Plasticity (Zero .backward(), Real-Time Updates).

Metrics Tracked:
    1. Mean Variational Free Energy (F_t)
    2. Per-Step Execution Latency (ms)
    3. Peak VRAM Memory (MB)
    4. Speech Loss / Perplexity Progression
===============================================================================
"""

import sys
import types
import time
import math
import torch

# =============================================================================
# DYNAMIC HOTFIX FOR PYTORCH 2.4/2.5+ PYTHON 3.12 DYNAMO BUG ON KAGGLE
# =============================================================================
class DummyDynamoModule(types.ModuleType):
    def __getattr__(self, name):
        if name == "decorators":
            return decorators_mod
        if name == "disable":
            return _disable
        if name == "is_compiling":
            return lambda *args, **kwargs: False
        return lambda *args, **kwargs: None

def _disable(fn=None, *args, **kwargs):
    if fn is None or not callable(fn):
        return lambda *a, **kw: None
    return fn

decorators_mod = types.ModuleType("torch._dynamo.decorators")
class _DimRange:
    pass
decorators_mod._DimRange = _DimRange

dynamo_mod = DummyDynamoModule("torch._dynamo")
dynamo_mod.decorators = decorators_mod
dynamo_mod.disable = _disable

sys.modules["torch._dynamo"] = dynamo_mod
sys.modules["torch._dynamo.decorators"] = decorators_mod
torch._dynamo = dynamo_mod

import torch.nn as nn
import torch.nn.functional as F

# Ensure autograd tracking is enabled
torch.set_grad_enabled(True)

# Set global seed for exact reproducibility
torch.manual_seed(42)

# Select execution hardware
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"[KEP Experiment #11] Execution Context: {device.type.upper()}")


# =============================================================================
# 1. LOCAL PREDICTIVE CODING LAYER (NO GLOBAL BACKPROP)
# =============================================================================

class PredictiveCodingLayer(nn.Module):
    """
    Bi-directional Predictive Coding Layer computing local forward predictions 
    and performing O(1) immediate local weight updates based on local error e_t.
    """
    def __init__(self, in_features, out_features, learning_rate=0.01):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.learning_rate = learning_rate

        self.weight = nn.Parameter(torch.randn(out_features, in_features) * 0.05)
        self.bias = nn.Parameter(torch.zeros(out_features))

    def update_local_weights(self, x_in, local_error):
        """
        Local Predictive Coding Update Rule:
        Delta W = eta * (local_error * x_in^T)
        Executes in O(1) during forward pass without global autograd graph!
        """
        with torch.no_grad():
            # Local error gradient flow
            delta_w = torch.bmm(local_error.unsqueeze(2), x_in.unsqueeze(1)).mean(dim=0)
            self.weight.add_(delta_w, alpha=self.learning_rate)
            self.bias.add_(local_error.mean(dim=0), alpha=self.learning_rate)

    def forward(self, x_in):
        """Forward prediction pass."""
        return F.linear(x_in, self.weight, self.bias)


# =============================================================================
# 2. CONTINUOUS PREDICTIVE CODING KARYON AGENT
# =============================================================================

class PredictiveCodingAgent(nn.Module):
    """
    Karyon Agent driven entirely by Continuous Real-Time Predictive Coding.
    """
    def __init__(self, vocab_size=258, embed_dim=128, hidden_dim=256, latent_dim=64):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.embed_dim = embed_dim

        self.token_embeddings = nn.Embedding(vocab_size, embed_dim)
        self.sensory_layer = PredictiveCodingLayer(embed_dim, hidden_dim)
        
        self.rnn_fast = nn.GRUCell(hidden_dim, hidden_dim)
        self.rnn_slow = nn.GRUCell(hidden_dim, hidden_dim)

        self.prior_net = nn.Linear(hidden_dim, latent_dim * 2)
        self.posterior_net = nn.Linear(hidden_dim + embed_dim, latent_dim * 2)
        self.decoder_layer = PredictiveCodingLayer(latent_dim + hidden_dim, embed_dim)
        self.head = nn.Linear(hidden_dim, vocab_size)

    def step_predictive_coding(self, input_id, h_fast, h_slow, mode="predictive_coding"):
        """
        Performs real-time perception with instant local predictive coding updates.
        """
        x_emb = self.token_embeddings(input_id)
        x_proj = F.silu(self.sensory_layer(x_emb))

        h_f_next = self.rnn_fast(x_proj, h_fast)
        h_s_next = self.rnn_slow(h_f_next, h_slow)

        # Active Inference Predictor
        prior_out = self.prior_net(h_fast)
        mu_p, logvar_p = prior_out.chunk(2, dim=-1)
        
        post_out = self.posterior_net(torch.cat([h_fast, x_emb], dim=-1))
        mu_q, logvar_q = post_out.chunk(2, dim=-1)

        std_q = torch.exp(0.5 * torch.clamp(logvar_q, -10.0, 10.0))
        z_t = mu_q + torch.randn_like(std_q) * std_q

        # Local Prediction of Sensory Input
        decoder_input = torch.cat([z_t, h_s_next], dim=-1)
        x_pred = self.decoder_layer(decoder_input)

        # Local Prediction Error Vector
        local_pred_error = x_emb - x_pred # e_t = x_real - x_pred
        
        kl_div = 0.5 * torch.mean(logvar_p - logvar_q + (torch.exp(logvar_q) + (mu_q - mu_p)**2) / torch.exp(logvar_p) - 1.0, dim=-1)
        rec_loss = torch.mean(local_pred_error**2, dim=-1)
        free_energy = kl_div + rec_loss

        logits = self.head(h_f_next + h_s_next)

        # REAL-TIME LOCAL WEIGHT UPDATES (ZERO GLOBAL BACKPROP!)
        if mode == "predictive_coding":
            # 1. Update decoder layer weights from local prediction error
            self.decoder_layer.update_local_weights(decoder_input.detach(), local_pred_error.detach())
            
            # 2. Back-project local error to sensory layer and update locally
            err_sensory = F.linear(local_pred_error.detach(), self.decoder_layer.weight[:, :64].T) if self.decoder_layer.weight.size(1) >= 64 else local_pred_error.detach()
            self.sensory_layer.update_local_weights(x_emb.detach(), x_proj.detach() * 0.1)

        return h_f_next, h_s_next, logits, free_energy


# =============================================================================
# 3. EXPERIMENTAL BENCHMARK ENGINE
# =============================================================================

def run_predictive_coding_benchmark(mode="autograd_control", stream=None, seq_len=800):
    agent = PredictiveCodingAgent().to(device)
    optimizer = torch.optim.Adam(agent.parameters(), lr=3e-3)
    criterion = nn.CrossEntropyLoss()

    h_fast = torch.zeros(1, agent.hidden_dim, device=device)
    h_slow = torch.zeros(1, agent.hidden_dim, device=device)

    free_energy_history = []
    latency_history = []

    if device.type == 'cuda':
        torch.cuda.reset_peak_memory_stats()

    start_time = time.time()

    for step in range(seq_len - 1):
        step_start = time.perf_counter()
        token_id = stream[step].unsqueeze(0)
        target_id = stream[step + 1].unsqueeze(0)

        if mode == "autograd_control":
            # CONTROL: Global Autograd Backprop
            h_fast, h_slow, logits, free_energy = agent.step_predictive_coding(token_id, h_fast, h_slow, mode="control")
            
            optimizer.zero_grad()
            loss = criterion(logits, target_id) + 0.1 * free_energy.mean()
            loss.backward()
            optimizer.step()
            
            h_fast = h_fast.detach()
            h_slow = h_slow.detach()
        else:
            # EXPERIMENTAL: Real-Time Local Predictive Coding (ZERO Autograd .backward()!)
            with torch.no_grad():
                h_fast, h_slow, logits, free_energy = agent.step_predictive_coding(token_id, h_fast, h_slow, mode="predictive_coding")
                h_fast = h_fast.detach()
                h_slow = h_slow.detach()

        free_energy_history.append(free_energy.item())
        latency_history.append((time.perf_counter() - step_start) * 1000.0)

    total_duration = time.time() - start_time
    avg_fe = sum(free_energy_history) / len(free_energy_history)
    avg_latency = sum(latency_history) / len(latency_history)
    peak_vram_mb = (torch.cuda.max_memory_allocated() / (1024 * 1024)) if device.type == 'cuda' else 0.0

    return {
        "mode": mode,
        "total_steps": seq_len - 1,
        "mean_free_energy": avg_fe,
        "peak_vram_mb": peak_vram_mb,
        "avg_latency_ms": avg_latency,
        "total_duration_sec": total_duration
    }


# =============================================================================
# 4. MAIN EVALUATION & TELEMETRY DASHBOARD
# =============================================================================

if __name__ == "__main__":
    STREAM_LEN = 800
    pattern = [ord(c) for c in "Continuous Active Inference Real-Time Predictive Coding Engine. "]
    stream = torch.tensor([pattern[i % len(pattern)] for i in range(STREAM_LEN)], dtype=torch.long, device=device)

    print("\n[KEP Step 1/2] Running CONTROL Group (Global Autograd BPTT .backward())...")
    control_res = run_predictive_coding_benchmark(mode="autograd_control", stream=stream, seq_len=STREAM_LEN)

    print("[KEP Step 2/2] Running EXPERIMENTAL Group (Real-Time Local Predictive Coding - ZERO .backward())...")
    experimental_res = run_predictive_coding_benchmark(mode="predictive_coding", stream=stream, seq_len=STREAM_LEN)

    # Calculate Telemetry Gains
    speedup_ratio = control_res["total_duration_sec"] / experimental_res["total_duration_sec"]
    fe_change_pct = ((experimental_res["mean_free_energy"] - control_res["mean_free_energy"]) / control_res["mean_free_energy"]) * 100.0
    vram_saved_mb = control_res["peak_vram_mb"] - experimental_res["peak_vram_mb"]

    # =========================================================================
    # TELEMETRY RESULTS DASHBOARD
    # =========================================================================
    print("\n" + "="*80)
    print(" === KARYON ENGINEERING PROTOCOL (KEP) TELEMETRY DASHBOARD ===")
    print("="*80)
    print(f"{'Metric':<32} | {'Autograd BPTT':<18} | {'Predictive Coding':<18} | {'Delta / Gain':<15}")
    print("-" * 88)
    print(f"{'Total Stream Steps':<32} | {control_res['total_steps']:<18} | {experimental_res['total_steps']:<18} | {'0 (Identical)':<15}")
    print(f"{'Autograd .backward() Calls':<32} | {control_res['total_steps']:<18} | {0:<18} | {'-100% Graph Ops':<15}")
    print(f"{'Peak VRAM Memory (MB)':<32} | {control_res['peak_vram_mb']:<18.2f} | {experimental_res['peak_vram_mb']:<18.2f} | {vram_saved_mb:+.2f} MB")
    print(f"{'Mean Free Energy (F_t)':<32} | {control_res['mean_free_energy']:<18.4f} | {experimental_res['mean_free_energy']:<18.4f} | {fe_change_pct:+.2f}% Error")
    print(f"{'Average Step Latency (ms)':<32} | {control_res['avg_latency_ms']:<18.2f} | {experimental_res['avg_latency_ms']:<18.2f} | {speedup_ratio:.2f}x Faster")
    print("="*80)

    # KEP Evaluation Logic
    print("\n--- [KEP EVALUATION & VERDICT] ---")
    if speedup_ratio >= 1.5 and fe_change_pct <= 15.0:
        print("🟢 VERDICT: POSITIVE EXPERIENCE ADOPTED!")
        print(f"   Reason: Real-time Predictive Coding achieved {speedup_ratio:.2f}x speedup with ZERO autograd calls and low Free Energy error ({fe_change_pct:+.2f}%).")
        print("   Action: Adopt Continuous Real-Time Predictive Coding Engine into master Karyon!")
    elif speedup_ratio < 1.1:
        print("⚪ VERDICT: NEUTRAL EXPERIENCE DISCARDED.")
    else:
        print("🔴 VERDICT: NEGATIVE EXPERIENCE REJECTED.")
        print("   Reason: Free Energy degradation exceeded acceptable boundary.")
    print("="*80 + "\n")
