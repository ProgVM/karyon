# experiments/exp_66_3stage_laminar_error_stack.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-66 (3-STAGE LAMINAR ERROR RESIDUAL STACK - LPER-3)
Tests a 3-Tier Hierarchical Cortical Architecture with Sequential Predictive Error Routing:
  1. Stage 1 (Fast Morpho-Syntactic): h1, e1 = h1 - Pred1(h1_prev)
  2. Stage 2 (Lexical-Syntactic): h2 = Stage2(e1), e2 = h2 - Pred2(h2_prev)
  3. Stage 3 (Discourse-Semantic): h3 = Stage3(e2)
  4. Combined Output: h_combined = h1 + h2 + h3
Each higher stage receives strictly unpredicted residual surprise from the tier below,
protecting Stage 3 slow memory channels from character noise and driving sub-2.00 nats loss.
...
# =============================================================================
# 1. EXACT PARALLEL LOG-SPACE CUMULATIVE DECAY SSD LAYER
# =============================================================================
...
# =============================================================================
# 4. EXP-66 MASTER AGENT WITH 3-STAGE LAMINAR ERROR RESIDUAL STACK (LPER-3)
# =============================================================================
class Exp66ThreeStageLaminarAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # Stage 1: Fast Morpho-Syntactic Sheet (Decay 0.005 - 0.20)
        self.stage1 = CorticalStage(
            hidden_dim=self.hidden_dim, expand_dim=self.expand_dim, num_heads=self.num_heads,
            head_k=self.head_k, head_v=self.head_v, min_beta=0.005, max_beta=0.20, device_str=device_str
        ).to(self.device)

        # Stage 1 Top-Down Predictor
        self.pred_net_1 = nn.Sequential(
        ...
        # Stage 2: Medium Lexical-Syntactic Sheet (Decay 0.0005 - 0.05)
        self.stage2 = CorticalStage(
            hidden_dim=self.hidden_dim, expand_dim=self.expand_dim, num_heads=self.num_heads,
            head_k=self.head_k, head_v=self.head_v, min_beta=0.0005, max_beta=0.05, device_str=device_str
        ).to(self.device)

        # Stage 2 Top-Down Predictor
        self.pred_net_2 = nn.Sequential(
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.SiLU(),
            nn.Linear(self.hidden_dim, self.hidden_dim)
        ).to(self.device)

        # Stage 3: Slow Discourse-Semantic Sheet (Decay 0.00005 - 0.01)
        self.stage3 = CorticalStage(
            hidden_dim=self.hidden_dim, expand_dim=self.expand_dim, num_heads=self.num_heads,
            head_k=self.head_k, head_v=self.head_v, min_beta=0.00005, max_beta=0.01, device_str=device_str
        ).to(self.device)
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        m_s3 = torch.zeros(batch_size, self.num_heads, self.head_k, self.head_v, dtype=torch.float32, device=self.device)
        ...
        for chunk_idx in range(num_chunks):
            ...
            # --- LPER Tier 1 Error Routing ---
            h1_shifted = torch.cat([h1_prev_last, h_s1[:, :-1, :]], dim=1)
            e1 = h_s1 - self.pred_net_1(h1_shifted)
            h1_prev_last = h_s1[:, -1:, :].detach()

            # Stage 2: Medium Lexical-Syntactic Pass on Error e1
            h_s2, m_s2, dt2 = self.stage2(e1, m_s2, curr_u_t, saliency_gate=saliency_gate, dt=1.0)

            # --- LPER Tier 2 Error Routing ---
            h2_shifted = torch.cat([h2_prev_last, h_s2[:, :-1, :]], dim=1)
            e2 = h_s2 - self.pred_net_2(h2_shifted)
            h2_prev_last = h_s2[:, -1:, :].detach()

            # Stage 3: Slow Discourse-Semantic Pass on Error e2
            h_s3, m_s3, dt3 = self.stage3(e2, m_s3, curr_u_t, saliency_gate=saliency_gate, dt=1.0)
            eff_dts.append((dt1 + dt2 + dt3) / 3.0)

            # Combined Laminar Representation (Stage 1 + Stage 2 + Stage 3)
            h_combined = h_s1 + h_s2 + h_s3
        ...
        return total_loss, avg_loss.item(), m_s3, h_s3[:, -1, :], curr_u_t, avg_eff_dt
    ...
    def generate_natural_speech(self, prompt: str, hu_state, max_tokens: int = 75) -> str:
        ...
        m_s3 = torch.zeros(1, self.num_heads, self.head_k, self.head_v, dtype=torch.float32, device=self.device)
        ...
        for c_idx in range(0, prompt_len, 64):
            ...
            h_s3, m_s3, _ = self.stage3(e2, m_s3, hu_state, saliency_gate=sal_gate, dt=1.0)
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-66 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_66_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-66 (3-STAGE LAMINAR ERROR STACK - LPER-3)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-66 3-Stage Laminar Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp66ThreeStageLaminarAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            g_s3 = 0.0
            for p in agent.stage3.parameters():
                if p.grad is not None:
                    g_s3 = max(g_s3, p.grad.norm().item())

            print("\n" + "="*95)
            print(f" === [KEP EXP-66 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | S1: {g_s1:.4f} | S2: {g_s2:.4f} | S3: {g_s3:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-66 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-66 3-Stage Laminar Value':<40}")
    print("-" * 95)
