# experiments/exp_rope_swiglu_ssd.py
"""
feat(exp): implement rotary position embeddings and expanded swiglu in ssd core

===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-27 (ROPE + EXPANDED SWIGLU SSD SCAN)
Hypothesis: Rotary Position Embeddings (RoPE) applied to intra-chunk Q/K,
combined with expanded SwiGLU (1536) and recalibrated DFET mastery setpoint (0.30),
eliminates premature backprop resting and breaks the 1.0 Loss ceiling.
...
# =============================================================================
# MODULE 1: ROTARY POSITION EMBEDDINGS (ROPE OPERATOR)
# =============================================================================
...
def apply_rotary_pos_emb(x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
    """Applies RoPE rotation to query/key tensors: [Batch, Heads, SeqLen, HeadDim]."""
    return (x * cos) + (rotate_half(x) * sin)


# =============================================================================
# MODULE 2: ROPE-ENHANCED PARALLEL SSD CORE
# =============================================================================

class RotaryParallelSSDCore(nn.Module):
    """
    State-Space Duality Core with Complex Rotary Position Embeddings (RoPE).
    Embeds exact relative phase shifts into intra-chunk attention Q and K.
    """
    def __init__(self, text_dim=128, unified_dim=256, hidden_dim=512, num_heads=8, head_k=32, head_v=64):
        ...
        # Precompute RoPE frequency inverse scales
        inv_freq = 1.0 / (10000.0 ** (torch.arange(0, head_k, 2, device=device).float() / head_k))
        self.register_buffer('inv_freq', inv_freq)

    def _get_rope_cos_sin(self, seq_len: int, device_ctx):
        t = torch.arange(seq_len, device=device_ctx).float()
        freqs = torch.einsum('i,j->ij', t, self.inv_freq)
        emb = torch.cat((freqs, freqs), dim=-1)
        cos = emb.cos().view(1, 1, seq_len, self.head_k)
        sin = emb.sin().view(1, 1, seq_len, self.head_k)
        return cos, sin

    def forward_chunk_rope_ssd(self, chunk_emb, m_prev, u_t, dt=1.0):
        ...
        # 2. Apply RoPE to Intra-Chunk Q and K
        cos, sin = self._get_rope_cos_sin(chunk_len, chunk_emb.device)
        ...
        # 3. Calibrated Intra-Chunk Attention with Rotated Phase Geometry
        ...
        s_matrix = torch.matmul(q_rot, k_rot.transpose(-1, -2)) * decay_weights
        y_intra = torch.matmul(s_matrix, v)

        # 4. Inter-Chunk Retrieval
...
# =============================================================================
# MODULE 3: EXPANDED SWIGLU BLOCK (1536 EXPANSION)
# =============================================================================

class ExpandedSwiGLUBlock(nn.Module):
    def __init__(self, hidden_dim=512, expand_dim=1536):
        super().__init__()
    ...
    def forward(self, x_flat: torch.Tensor) -> torch.Tensor:
        ...
        up = self.w_up(x_flat)
        return self.norm(x_flat + self.w_down(gate * up))
...
class BaselineAgent(nn.Module):
    """Baseline Agent (Production v18: Standard SSD without RoPE, SwiGLU=1024, High DFET)."""
    def __init__(self, config):
        ...
        self.channel_mixer = ParallelSwiGLUBlock(hidden_dim=self.hidden_dim, expand_dim=1024, device=device_str)
...
class ProposedRoPESwiGLUAgent(nn.Module):
    """Proposed Agent (RoPE-infused SSD + Expanded SwiGLU=1536 + Precision DFET)."""
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.hidden_dim = config.net.hidden_dim
        ...
        self.pos_embeddings = OffsetPositionalByteEmbedding(self.text_gen_dim, self.text_dim, device_str=device_str)
        # Rotary Parallel SSD Core
        self.ssd_core = RotaryParallelSSDCore(
            self.text_dim, self.unified_dim, self.hidden_dim, 8, 32, 64
        ).to(device)
        # Expanded 1536-dim SwiGLU Block
        self.channel_mixer = ExpandedSwiGLUBlock(hidden_dim=self.hidden_dim, expand_dim=1536).to(device)
        self.attractor_head = DesaturatedHopfieldAttractorHead(self.hidden_dim, self.text_gen_dim, device=device_str)
        self.motor_text_proj = nn.Sequential(
            nn.Linear(self.hidden_dim, self.text_dim),
            nn.SiLU(),
    ...
    def forward_chunk(self, chunk_emb, chunk_targets, m_prev, u_t, criterion):
        h_chunk, m_next = self.ssd_core.forward_chunk_rope_ssd(chunk_emb, m_prev, u_t, dt=1.0)
        h_reasoned = self.channel_mixer(h_chunk)
        h_relaxed = self.attractor_head.relax_to_minima(h_reasoned)[0]
        logits = F.linear(self.motor_text_proj(h_relaxed), self.pos_embeddings.byte_embed.weight) * self.inv_sqrt_text_dim
        loss = criterion(logits, chunk_targets.contiguous().view(-1))
...
def run_isolated_benchmark():
    print(f"\n{'='*85}")
    print(f" === RUNNING KEP HYPOTHESIS #27 BENCHMARK (ROPE + SWIGLU 1536): {device_str.upper()} ===")
    print(f"{'='*85}\n")
    ...
    # Realistic conversational multi-sentence corpus
    sample_text = (
    ...
    # -------------------------------------------------------------------------
    # TEST 1: BASELINE (Sinusoidal SSD + SwiGLU 1024)
    # -------------------------------------------------------------------------
    print("[1/2] Evaluating BASELINE (Sinusoidal SSD + SwiGLU 1024)...")
    ...
    # -------------------------------------------------------------------------
    # TEST 2: PROPOSED (RoPE SSD + SwiGLU 1536 + Low DFET)
    # -------------------------------------------------------------------------
    print("[2/2] Evaluating PROPOSED (RoPE SSD + SwiGLU 1536 + Low DFET 0.30)...")
    prop_model = ProposedRoPESwiGLUAgent(config).to(device)
    ...
    print("\n" + "="*90)
    print(" === [KEP RULE #6 TELEMETRY BENCHMARK REPORT: ROPE + SWIGLU 1536] ===")
    print("="*90)
    print(f"{'Performance Metric':<35} | {'Baseline (Sinusoidal)':<22} | {'Proposed (RoPE+SwiGLU)':<22} | {'Delta':<10}")
    print("-" * 95)
    ...
    def generate_eval(model, name, is_rope=False):
        model.eval()
        with torch.no_grad():
            ...
            if not is_rope:
                ...
                h_ssm, m_state = model.ssd_core.forward_chunk_rope_ssd(p_emb, m_state, u_t, dt=1.0)
            ...
            for s in range(60):
                ...
                if not is_rope:
                    ...
                    h_s_out, m_state = model.ssd_core.forward_chunk_rope_ssd(t_emb, m_state, u_t, dt=1.0)
    ...
    generate_eval(base_model, "Baseline (Sinusoidal + SwiGLU 1024)", is_rope=False)
    generate_eval(prop_model, "Proposed (RoPE + SwiGLU 1536)", is_rope=True)
    print("="*90 + "\n")

    if prop_losses[-1] < base_losses[-1] and prop_tok_per_sec > 120000.0:
        print("🟢 KEP VERDICT: POSITIVE (RoPE + SwiGLU 1536 Validated!).")
    else:
