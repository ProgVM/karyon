# experiments/exp_hebbian_fast_weights.py
"""
===============================================================================
KARYON ENGINEERING PROTOCOL (KEP) EXPERIMENT #4
Topic: Local Hebbian Fast-Weights & Predictive Conductance Plasticity (Zero Backprop)
Author: Karyon-CoRE Open-Source Research Team (2026)
===============================================================================

Hypothesis:
    Replacing global Autograd Backpropagation (.backward()) during online streaming 
    with local Hebbian Fast-Weights (W_fast = lambda * W_fast + eta * NA * (post x_pre^T)) 
    updated in O(1) matrix operations during the forward pass will eliminate 
    Autograd VRAM memory graph overhead, accelerate per-step latency, and enable 
    real-time continuous adaptation without traditional gradient calculators.

Control Group: 
    Traditional Autograd Backpropagation (.backward() + Adam Optimizer step).

Experimental Group: 
    Local Hebbian Fast-Weights Plasticity (Zero .backward(), Zero Autograd Graph).

Metrics Tracked:
    1. VRAM Memory Allocated (MB)
    2. Per-Step Execution Latency (ms)
    3. Mean Variational Free Energy (F_t)
    4. Adaptation Efficiency on Novel Byte Sequences
===============================================================================
"""
...
# =============================================================================
# UNCONDITIONAL HOTFIX FOR PYTORCH 2.4/2.5+ PYTHON 3.12 DYNAMO BUG ON KAGGLE
# =============================================================================
...
print(f"[KEP Experiment #4] Execution Context: {device.type.upper()}")


# =============================================================================
# 1. HEBBIAN FAST-WEIGHTS SYNAPTIC CONDUCTANCE LAYER
# =============================================================================

class HebbianFastWeightsLayer(nn.Module):
    """
    Synaptic Conductance Layer combining static base weights (W_base) 
    with dynamic, O(1) locally-updated Hebbian Fast-Weights (W_fast).
    """
    def __init__(self, in_features, out_features, decay_rate=0.95, learning_rate=0.01):
        super().__init__()
    ...
    def update_fast_weights(self, x_pre, post_signal, arousal_na):
        """
        Local Hebbian Update Rule:
        W_fast = decay * W_fast + eta * Noradrenaline * (post_signal x_pre^T)
        post_signal shape: [B, out_features]
        x_pre shape: [B, in_features]
        """
        with torch.no_grad():
            # Outer product of post-synaptic activity and pre-synaptic input
            delta_w = torch.bmm(post_signal.unsqueeze(2), x_pre.unsqueeze(1)).mean(dim=0)
            # In-place Hebbian update matching [out_features, in_features]
...
# =============================================================================
# 2. PROTOTYPE KARYON AGENT WITH FAST-WEIGHT PLASTICITY
# =============================================================================

class PrototypeHebbianAgent(nn.Module):
    """
    Karyon Prototype Agent supporting both Autograd Backpropagation 
    and Zero-Backprop Local Hebbian Fast-Weights Plasticity.
    """
    def __init__(self, vocab_size=258, embed_dim=128, hidden_dim=256, latent_dim=64):
        ...
        # Fast-Weights Synaptic Projection Layer ([256, 128])
        self.sensory_hebbian = HebbianFastWeightsLayer(embed_dim, hidden_dim)
    ...
    def forward(self, input_id, h_prev):
        """Forward pass computing next state, prediction error, and Free Energy."""
        ...
        return h_next, x_emb, x_proj, logits, free_energy
...
def run_hebbian_benchmark(mode="autograd_control", stream=None, seq_len=600):
    """
    Executes streaming session comparing Autograd Backprop vs Zero-Backprop Hebbian Plasticity.
    """
    agent = PrototypeHebbianAgent().to(device)
    ...
    for step in range(seq_len - 1):
        ...
        # 1. Forward Pass
        h_curr, x_emb, x_proj, logits, free_energy = agent(input_id, h_curr)
        ...
        if mode == "autograd_control":
            ...
            # EXPERIMENTAL: Local Hebbian Fast-Weights Update (ZERO Autograd .backward()!)
            arousal_na = min(1.0, max(0.1, fe_val)) # Noradrenaline scales with Free Energy
            agent.sensory_hebbian.update_fast_weights(x_emb.detach(), x_proj.detach(), arousal_na)
            h_curr = h_curr.detach()
...
if __name__ == "__main__":
    ...
    control_res = run_hebbian_benchmark(mode="autograd_control", stream=stream, seq_len=STREAM_LEN)

    print("[KEP Step 2/2] Running EXPERIMENTAL Group (Hebbian Fast-Weights - ZERO .backward())...")
    experimental_res = run_hebbian_benchmark(mode="hebbian_fast_weights", stream=stream, seq_len=STREAM_LEN)
    ...
    print("="*80)
    print(f"{'Metric':<32} | {'Autograd Control':<18} | {'Hebbian Fast-Weights':<18} | {'Delta / Gain':<15}")
    print("-" * 88)
    ...
    if speedup_ratio >= 1.5 and fe_change_pct <= 15.0:
        ...
        print("   Action: Merge Hebbian Fast-Weights Plasticity into production karyon_core!")
