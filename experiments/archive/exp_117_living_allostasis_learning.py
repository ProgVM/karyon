...
class LocalPredictivePlasticityEngine:
    ...
    def evaluate(self, inp_tokens: torch.Tensor, tgt_tokens: torch.Tensor, hu: HomeostaticUnit):
        device = self.agent.device
        b_size, seq_len = inp_tokens.shape
        u_t = hu.state
        ...
        with torch.no_grad():
            ...
            h_s2, m_s2, _ = self.agent.stage2(h_s1, m_s2, u_t, sal_gate, 1.0)
...
def run_experiment():
    ...
    with torch.no_grad():
        ...
        val_loss_prop = plasticity_engine.evaluate(val_inp, val_tgt, hu_prop)
