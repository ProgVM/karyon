# experiments/exp_95_dynamic_entropy_dt_scaling.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-95
Hypothesis:
1. Dynamically scaling the temporal integration rate (dt) step-by-step based on
   local prediction entropy (where dt is compressed during high-predictability
   intra-word steps and expanded at high-entropy word boundaries) prevents
   semantic bleed and accelerates speech loss reduction.
2. Coupling the SDE Langevin noise level directly to step-by-step entropy
   stabilizes the attractor trajectory during predictable sequences while
   allowing exploration at boundary transitions.
...
def prepare_packed_stream(num_batches: int = 100, batch_size: int = 16, seq_len: int = 512):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-95 (S={seq_len}, Steps={num_batches})...")
...
class EntropyDtScalingCoREAgent(CoREAgent):
    """
    Karyon-CoRE Agent with:
    1. Step-by-step Entropy-Adaptive temporal integration rate (dt) scaling.
    2. Dynamic Langevin noise level coupled to local prediction entropy.
    """
    def __init__(self, config, device='cpu'):
        super().__init__(config, device)
        # Lightweight local entropy predictor network
    ...
    def get_all_parameters(self):
        ...
        params.extend(list(self.entropy_predictor.parameters()))
        return params
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, 
        ...
        # 2. Predict step-by-step local entropy from Stage 1 representation
        predicted_entropy = self.entropy_predictor(h_s1) # Shape: [batch_size, seq_len, 1]

        # 3. Compute dynamic step-by-step dt scaling factor:
        # dt is compressed (smaller) during highly predictable steps (low entropy) to prevent semantic bleed,
        # and expanded (larger) at high-entropy word boundaries to allow deep integration.
        dynamic_dt_scale = 0.40 + 1.20 * predicted_entropy # Range: [0.40, 1.60]

        # 4. Route with dynamic dt scaling to Stage 2
        saliency_gate = self.boundary_detector(h_s1, input_seq)
        e1_weighted, _, _ = self.pw_lper(h_s1, h1_prev_last, curr_u_t)

        # We pass the average dynamic dt scale to the C++ stage2 forward pass
        avg_dt_scale = dynamic_dt_scale.mean().item()
        h_s2, m_s2, dt2 = self.stage2(e1_weighted, m_s2, curr_u_t, saliency_gate, avg_dt_scale)
...
def run_exp_95_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-95 (DYNAMIC ENTROPY DT SCALING)] ===")
    print("="*85)
    ...
    # 1. EVALUATE BASELINE (Standard CoREAgent with static dt)
    print("\n" + "-"*85)
    ...
    # 2. EVALUATE PROPOSED (Entropy-Adaptive dt Scaling CoREAgent)
    print("\n" + "-"*85)
    print(" >>> RUNNING PHASE 2: PROPOSED (Entropy-Adaptive dt Scaling) <<<")
    print("-"*85)
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats()

    agent_prop = EntropyDtScalingCoREAgent(config, device=device_str).to(device)
