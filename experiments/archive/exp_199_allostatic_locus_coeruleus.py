# experiments/exp_199_allostatic_locus_coeruleus.py
"""
===============================================================================
EXP-199: Allostatically-Gated Locus Coeruleus Neural Gain Modulation
Grounding: KEP Principle 2 (Biological Realism), Principle 14 (Allostatic Forces: No Static Constants).
===============================================================================
Hypothesis:
Replacing the static linear or partially non-linear Locus Coeruleus (LC) neural gain projections
with an allostatically-gated gain mechanism—where the noradrenergic neural gain and phasic
uncertainty adaptation are dynamically modulated by a non-linear function of Somatic Energy (u_t[1]),
Noradrenaline (u_t[4]), and Variational Free Energy (F_t)—will optimize neural gain control,
prevent cognitive overload, and accelerate loss convergence.
"""
...
logger = logging.getLogger("EXP-199")


class AllostaticLCGating(nn.Module):
    """
    Allostatically-gated Locus Coeruleus Neural Gain Controller.
    """
    def __init__(self, device_str: str = 'cpu'):
        ...
        self.proj = nn.Linear(1, 1, bias=False).to(self.device)
    ...
    def forward(self, phasic_gain: torch.Tensor, u_t: torch.Tensor, fe_loss: torch.Tensor) -> torch.Tensor:
        B = phasic_gain.shape[0]
        
        # Format homeostatic and free energy inputs
        fe_normalized = torch.clamp(fe_loss.detach() / 10.0, 0.0, 1.0).view(-1, 1)
        if u_t.dim() == 2:
            u_expanded = u_t.expand(B, -1)
        else:
            u_expanded = u_t.view(1, -1).expand(B, -1)
            
        fe_expanded = fe_normalized.expand(B, -1)
        ...
        return phasic_gain + gain * self.proj(phasic_gain.view(-1, 1)).view_as(phasic_gain)
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-199: ALLOSTATIC LC GATING BENCHMARK]")
    logger.info("=" * 80)
    ...
    # 1. Baseline Evaluation (EXP-198 Critic Gating)
    ...
    # 2. Proposed Evaluation (EXP-199 Allostatic LC Gating)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain
    brain_prop.allostatic_lc_gating = AllostaticLCGating(
        device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Allostatic LC Gating Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-199 TELEMETRY REPORT ===")
    ...
    with open("experiments/exp_199_results.json", "w") as f:
