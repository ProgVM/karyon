# experiments/exp_hebbian_fast_weights_v3.py
"""
===============================================================================
KARYON ENGINEERING PROTOCOL (KEP) EXPERIMENT #4.2 (ITERATION 3)
Topic: Normalized Dual-Layer Three-Factor Predictive Hebbian Fast-Weights
Author: Karyon-CoRE Open-Source Research Team (2026)
===============================================================================

Hypothesis:
    Applying scale-normalized Three-Factor Predictive Hebbian Updates across 
    both sensory projection and recurrent core transitions will eliminate state 
    drift, reducing Free Energy error degradation under 10% while retaining 
    >3x step speedup and zero Autograd VRAM graph overhead.

Control Group: 
    Traditional Autograd Backpropagation (.backward() + Adam Optimizer step).

Experimental Group: 
    Normalized Dual-Layer Predictive Hebbian Plasticity (Zero .backward()).
...
print(f"[KEP Experiment #4.2] Execution Context: {device.type.upper()}")


# =============================================================================
# 1. NORMALIZED THREE-FACTOR PREDICTIVE HEBBIAN SYNAPTIC LAYER
# =============================================================================

class NormalizedHebbianLayer(nn.Module):
    """
    Scale-Normalized Three-Factor Synaptic Conductance Layer:
    dW_fast = eta * Noradrenaline * (e_hidden x_pre^T) / (||x_pre||^2 + eps)
    """
    def __init__(self, in_features, out_features, decay_rate=0.96, learning_rate=0.02):
        super().__init__()
    ...
    def update_normalized_fast_weights(self, x_pre, err_signal, arousal_na):
        """
        Normalized Three-Factor Update Rule with shape-adaptive error projection.
        """
        with torch.no_grad():
            # Dynamically adapt prediction error vector to layer's hidden space
            if err_signal.size(-1) == self.out_features:
                err_hidden = err_signal
            elif err_signal.size(-1) == self.in_features:
                err_hidden = F.linear(err_signal, self.weight_base) # [B, out_features]
            else:
                # Fallback project via adaptive slice/pad
                err_hidden = torch.zeros(err_signal.size(0), self.out_features, device=err_signal.device)
                min_d = min(err_signal.size(-1), self.out_features)
                err_hidden[:, :min_d] = err_signal[:, :min_d]

            # Outer product normalized by input vector norm
            pre_norm_sq = torch.norm(x_pre, p=2, dim=-1, keepdim=True)**2 + 1e-5
            delta_w = torch.bmm(err_hidden.unsqueeze(2), (x_pre / pre_norm_sq).unsqueeze(1)).mean(dim=0)
            
            # Stable scale-normalized update
...
# =============================================================================
# 2. PROTOTYPE DUAL-LAYER HEBBIAN KARYON AGENT
# =============================================================================

class PrototypeDualLayerAgent(nn.Module):
    """
    Karyon Prototype Agent with Dual-Layer Normalized Predictive Plasticity 
    (Sensory Gateway + Recurrent Cell).
    """
    def __init__(self, vocab_size=258, embed_dim=128, hidden_dim=256, latent_dim=64):
        ...
        # Dual Fast-Weights Layers: Sensory Projection + Recurrent Core
        self.sensory_hebbian = NormalizedHebbianLayer(embed_dim, hidden_dim)
        self.recurrent_hebbian = NormalizedHebbianLayer(hidden_dim, hidden_dim)
    ...
    def forward(self, input_id, h_prev):
        """Forward pass with dual fast-weight conductance."""
        x_emb = self.token_embeddings(input_id)
        x_proj = F.silu(self.sensory_hebbian(x_emb))
        
        # Recurrent state transition with fast-weight adaptation
        h_rec = F.tanh(self.recurrent_hebbian(h_prev) + x_proj)
        h_next = h_rec
...
def run_hebbian_benchmark_v3(mode="autograd_control", stream=None, seq_len=600):
    """
    Executes streaming session comparing Autograd Backprop vs Dual-Layer Normalized Hebbian.
    """
    agent = PrototypeDualLayerAgent().to(device)
    ...
    for step in range(seq_len - 1):
        ...
        # 1. Forward Pass
        h_curr, x_emb, x_proj, h_prev, rec_err, logits, free_energy = agent(input_id, h_curr)
        ...
        if mode == "autograd_control":
            ...
            # EXPERIMENTAL: Dual-Layer Normalized Hebbian Plasticity (ZERO .backward()!)
            arousal_na = min(1.0, max(0.1, fe_val))
            
            # Layer 1: Sensory Gateway Fast-Weights Update
            agent.sensory_hebbian.update_normalized_fast_weights(
                x_pre=x_emb.detach(), 
                err_signal=rec_err.detach(), 
                arousal_na=arousal_na
            )
            # Layer 2: Recurrent Core Fast-Weights Update
            agent.recurrent_hebbian.update_normalized_fast_weights(
                x_pre=h_prev.detach(), 
                err_signal=rec_err.detach(), 
                arousal_na=arousal_na
            )
...
if __name__ == "__main__":
    ...
    control_res = run_hebbian_benchmark_v3(mode="autograd_control", stream=stream, seq_len=STREAM_LEN)

    print("[KEP Step 2/2] Running EXPERIMENTAL Group (Dual-Layer Normalized Hebbian - ZERO .backward())...")
    experimental_res = run_hebbian_benchmark_v3(mode="hebbian_fast_weights_v3", stream=stream, seq_len=STREAM_LEN)
    ...
    print("="*80)
    print(f"{'Metric':<32} | {'Autograd Control':<18} | {'Dual-Layer Hebbian':<18} | {'Delta / Gain':<15}")
    print("-" * 88)
    ...
    if speedup_ratio >= 1.5 and fe_change_pct <= 15.0:
        ...
        print("   Action: Merge Dual-Layer Normalized Fast-Weights into production karyon_core!")
