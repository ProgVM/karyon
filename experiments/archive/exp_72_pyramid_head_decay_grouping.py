# experiments/exp_72_pyramid_head_decay_grouping.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-72 (PYRAMID HEAD DECAY GROUPING & CAUSAL ERROR FILTERING - PHDG-CEF)
Hypothesis: In EXP-70, SSD head decay rates were linearly log-spaced across 12 heads in a single range,
and Stage 1's prediction error e1 contained raw single-byte high-frequency noise when entering Stage 2.
By introducing:
  1. Pyramid Head Decay Grouping (PHDG): Partitioning the 12 SSD heads into 3 functional 
     timescale groups (Group 1 Fast beta in [0.10, 0.40], Group 2 Meso beta in [0.01, 0.10], 
     Group 3 Slow beta in [0.0001, 0.01]), forcing head specialization across phonetic, morphemic, and semantic resolutions.
  2. Causal Error Filtering (CEF): Passing Stage 1's error residual e1 through a causal 
     depthwise K=3 SiLU filter before entering Stage 2, smoothing single-byte noise spikes 
     and feeding structured multi-byte error signals to Stage 2.
This will drive steady-state speech loss below 1.60 nats/byte.
...
# =============================================================================
# 0. MULTI-SCALE MORPHOLOGICAL BYTE PYRAMID RECEPTIVE FIELD (EXP-70)
# =============================================================================
...
# =============================================================================
# 2. PYRAMID HEAD DECAY GROUPED SSD LAYER WITH MAMBA-2 GLU GATING & ROPE
# =============================================================================
class PyramidHeadDecaySSDLayer(nn.Module):
    """
    Partition 12 SSD heads into 3 distinct functional timescale groups:
    - Group 1 (Fast Phonetic, Heads 0..3): beta in [0.10, 0.40] (decay alpha in [0.60, 0.90])
    - Group 2 (Meso Morphemic, Heads 4..7): beta in [0.01, 0.10] (decay alpha in [0.90, 0.99])
    - Group 3 (Slow Semantic, Heads 8..11): beta in [0.0001, 0.01] (decay alpha in [0.99, 0.9999])
    """
    def __init__(self, in_dim: int = 768, out_dim: int = 768, num_heads: int = 12,
                 head_k: int = 64, head_v: int = 128):
        super().__init__()
        self.in_dim = in_dim
        ...
        # Build 3-Group Pyramid Head Decay Initialization over 12 heads
        heads_per_group = num_heads // 3 # 4 heads per group
        
        # Group 1 (Fast): beta in [0.10, 0.40]
        betas_g1 = torch.exp(torch.linspace(math.log(0.40), math.log(0.10), heads_per_group))
        # Group 2 (Meso): beta in [0.01, 0.10]
        betas_g2 = torch.exp(torch.linspace(math.log(0.10), math.log(0.01), heads_per_group))
        # Group 3 (Slow): beta in [0.0001, 0.01]
        betas_g3 = torch.exp(torch.linspace(math.log(0.01), math.log(0.0001), heads_per_group))

        betas = torch.cat([betas_g1, betas_g2, betas_g3], dim=0)
        alphas = 1.0 - betas
...
# =============================================================================
# 4. ENTROPY-ADAPTIVE BOUNDARY DETECTOR
# =============================================================================
...
# =============================================================================
# 5. CAUSAL ERROR FILTERING (CEF) FOR LPER RESIDUAL CHANNEL
# =============================================================================
class CausalErrorFilter(nn.Module):
    """Causal Depthwise K=3 SiLU Filter for LPER Error Channel e1."""
    def __init__(self, hidden_dim: int = 768):
        super().__init__()
        self.conv = nn.Conv1d(hidden_dim, hidden_dim, kernel_size=3, groups=hidden_dim, bias=False)
        self.norm = nn.LayerNorm(hidden_dim)

    def forward(self, e1: torch.Tensor) -> torch.Tensor:
        # e1 shape: [B, L, D]
        e_trans = e1.transpose(1, 2) # [B, D, L]
        e_pad = F.pad(e_trans, (2, 0)) # Causal padding 2 on left
        out_conv = F.silu(self.conv(e_pad)).transpose(1, 2)
        return self.norm(e1 + out_conv)


# =============================================================================
# 6. EXP-72 MASTER AGENT (PHDG-CEF)
# =============================================================================
class Exp72PyramidHeadDecayAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        self.stage1 = CorticalStage(
            ...
            head_k=self.head_k, head_v=self.head_v, device_str=device_str
        ).to(self.device)
        ...
        # Causal Error Filter for LPER Channel e1
        self.error_filter = CausalErrorFilter(hidden_dim=self.hidden_dim).to(self.device)
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        for chunk_idx in range(num_chunks):
            ...
            # --- LPER Error Routing with Causal Error Filter ---
            h1_shifted = torch.cat([h1_prev_last, h_s1[:, :-1, :]], dim=1)
            e1_raw = h_s1 - self.topdown_pred_net(h1_shifted)
            h1_prev_last = h_s1[:, -1:, :].detach()

            # Filter single-byte high-frequency noise from error residual
            e1_filtered = self.error_filter(e1_raw)

            # Stage 2: Slow Semantic Pass on Filtered Error e1_filtered
            h_s2, m_s2, dt2 = self.stage2(e1_filtered, m_s2, curr_u_t, saliency_gate=saliency_gate, dt=1.0)
    ...
    def generate_natural_speech(self, prompt: str, hu_state, max_tokens: int = 75) -> str:
        ...
        for c_idx in range(0, prompt_len, 64):
            ...
            e1_filtered = self.error_filter(e1_raw)
            h_s2, m_s2, _ = self.stage2(e1_filtered, m_s2, hu_state, saliency_gate=sal_gate, dt=1.0)
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-72 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_72_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-72 (PYRAMID HEAD DECAY GROUPING & CAUSAL ERROR FILTERING)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-72 Pyramid Head Decay Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp72PyramidHeadDecayAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            g_rf = 0.0
            for p in agent.pos_embeddings.receptive_field.parameters():
                if p.grad is not None:
                    g_rf = max(g_rf, p.grad.norm().item())
            g_s1 = 0.0
            for p in agent.stage1.parameters():
                if p.grad is not None:
                    g_s1 = max(g_s1, p.grad.norm().item())
            g_ef = 0.0
            for p in agent.error_filter.parameters():
                if p.grad is not None:
                    g_ef = max(g_ef, p.grad.norm().item())
            g_s2 = 0.0
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-72 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | Pyramid RF: {g_rf:.4f} | S1: {g_s1:.4f} | Error Filter: {g_ef:.4f} | S2: {g_s2:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-72 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-72 Pyramid Head Decay Value':<40}")
    print("-" * 95)
