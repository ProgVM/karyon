# experiments/exp_event_driven_dt.py
"""
===============================================================================
KARYON ENGINEERING PROTOCOL (KEP) EXPERIMENT #3
Topic: Event-Driven Recurrent Core Execution & Dynamic Time Integration (dt)
Author: Karyon-CoRE Open-Source Research Team (2026)
===============================================================================

Hypothesis:
    When consecutive sensory inputs exhibit sub-threshold variation (||S_t - S_{t-1}|| < eps), 
    skipping the 2nd-Order Stochastic Heun SDE recurrent step and accumulating 
    the integration time-delta (dt_eff = dt_eff + dt_base) reduces recurrent FLOPs 
    by 30-60% while mathematically preserving long-term temporal hidden dynamics.

Control Group: 
    Fixed-step continuous execution (Full SDE integration at every single step).

Experimental Group: 
    Event-Driven State Skipping with Dynamic dt Accumulation.
...
print(f"[KEP Experiment #3] Execution Context: {device.type.upper()}")


# =============================================================================
# 1. 2ND-ORDER STOCHASTIC HEUN SDE CORE WITH DYNAMIC DT
# =============================================================================

class StochasticHeunSDECore(nn.Module):
    """
    2nd-Order Stochastic Predictor-Corrector Heun SDE Recurrent Engine.
    Supports continuous time-delta (dt) integration.
    """
    ...
    def forward(self, h_fast_prev, h_slow_prev, w_t, dt=1.0):
        """2nd-Order Predictor-Corrector Heun SDE Integration Step."""
        sqrt_dt = math.sqrt(max(0.01, dt))
        ...
        h_slow_pred = h_slow_prev + dt * k1_slow + dW_slow

        k2_slow = -self.gamma * h_slow_pred + self.slow_f(h_slow_pred)
        h_slow_next = torch.tanh(h_slow_prev + 0.5 * dt * (k1_slow + k2_slow) + dW_slow)
        ...
        h_fast_pred = h_fast_prev + dt * k1_fast + dW_fast

        fast_in_2 = torch.cat([h_fast_pred, w_t, h_slow_next], dim=-1)
        k2_fast = -self.gamma * h_fast_pred + self.fast_f(fast_in_2)
        h_fast_next = torch.tanh(h_fast_prev + 0.5 * dt * (k1_fast + k2_fast) + dW_fast)

        return h_fast_next, h_slow_next


class EventDrivenAgent(nn.Module):
    """
    Karyon Agent equipped with Event-Driven Dynamic dt Acceleration.
    """
    def __init__(self, vocab_size=258, text_dim=128, hidden_dim=256, unified_dim=128):
        super().__init__()
        self.hidden_dim = hidden_dim
        ...
        self.sde_core = StochasticHeunSDECore(hidden_dim, unified_dim)
        self.head = nn.Linear(hidden_dim, vocab_size)

    def forward_step(self, input_id, h_fast, h_slow, prev_sensory, accumulated_dt=1.0, event_threshold=0.08, mode="control"):
        """
        Processes single step. 
        If mode='event_driven' and sensory shift ||S_t - S_{t-1}|| < threshold,
        skips SDE execution and accumulates dt.
        """
        ...
        if mode == "control" or sensory_delta >= event_threshold:
            # Execute full 2nd-order SDE integration step with accumulated dt
            h_fast_next, h_slow_next = self.sde_core(h_fast, h_slow, w_t, dt=accumulated_dt)
            effective_dt = 1.0 # Reset accumulated time
            sde_executed = True
        else:
            # EVENT-DRIVEN SKIP: Maintain state and accumulate time delta
            h_fast_next = h_fast
            h_slow_next = h_slow
            effective_dt = accumulated_dt + 1.0

        logits = self.head(h_fast_next + h_slow_next)
        return h_fast_next, h_slow_next, w_t, logits, effective_dt, sde_executed
...
def generate_paused_stream(seq_len=1000):
    ...
    for i in range(seq_len):
        if 200 <= i <= 350 or 500 <= i <= 650:
            # Static repeated byte stream (simulating pause / silence / continuous video frame)
...
def run_event_driven_benchmark(mode="control", stream=None, event_threshold=0.08):
    agent = EventDrivenAgent().to(device)
    agent.eval()
...
if __name__ == "__main__":
    ...
    control_res = run_event_driven_benchmark(mode="control", stream=stream)

    print("[KEP Step 2/2] Running EXPERIMENTAL Group (Event-Driven Dynamic dt Accumulation)...")
    experimental_res = run_event_driven_benchmark(mode="event_driven", stream=stream, event_threshold=0.08)
    ...
    print("="*80)
    print(f"{'Metric':<32} | {'Control Group':<18} | {'Experimental (dt)':<18} | {'Delta / Gain':<15}")
    print("-" * 88)
    ...
    if sde_saved_pct >= 30.0 and state_divergence <= 2.5:
        ...
        print("   Action: Merge Event-Driven Dynamic dt Accumulation into production karyon_core!")
