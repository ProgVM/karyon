# experiments/exp_61_entropy_adaptive_blt_boundary.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-61 (ENTROPY-ADAPTIVE BLT BOUNDARY GATING)
Tests Entropy-Adaptive Word-Boundary Saliency (EABS) in the 2-Stage Cortical Stack
to eliminate pseudo-morphemic drift and additive memory bleed in byte space (V=258).
...
# =============================================================================
# 1. SPECIALIZED BALANCED SSD LAYER WITH BOUNDARY SALIENCY GATING
# =============================================================================
class BoundaryGatedSSDLayer(nn.Module):
    ...
    def forward(self, x_seq: torch.Tensor, m_prev: torch.Tensor, u_t: torch.Tensor, 
        ...
        # Modulate Beta update by Saliency Gate if provided (Stage 2 semantic filtering)
        # saliency_gate shape: [B, 1, chunk_len, 1]
        if saliency_gate is not None:
            beta = beta * saliency_gate
...
# =============================================================================
# 2. CORTICAL STAGE 1 (FAST MORPHO-SYNTACTIC SHEET) & STAGE 2 (SEMANTIC SHEET)
# =============================================================================
class CorticalStage(nn.Module):
    def __init__(self, hidden_dim: int = 512, expand_dim: int = 2048, num_heads: int = 8,
        ...
        self.ssd = BoundaryGatedSSDLayer(
...
# =============================================================================
# 3. ENTROPY-ADAPTIVE BOUNDARY DETECTOR (BLT MECHANISM)
# =============================================================================
class EntropyAdaptiveBoundaryDetector(nn.Module):
    """
    Computes dynamic word/morpheme boundary saliency based on Stage 1 hidden features
    and UTF-8 character boundary tokens (spaces, newlines, punctuation).
    """
    def __init__(self, hidden_dim: int = 512):
        ...
        # Explicit Boundary Byte IDs: Space(32), NL(10), Comma(44), Dot(46), Colon(58), Semi(59), Query(63), Excl(33), Quotes(34, 39)
    ...
    def forward(self, h_stage1: torch.Tensor, input_ids: torch.Tensor) -> torch.Tensor:
        """
        Returns saliency gate gamma in range [0.10, 1.00] with shape [B, 1, L, 1].
        """
        batch_size, seq_len, _ = h_stage1.size()
        
        # Neural boundary prediction
        pred_boundary = self.boundary_gate_net(h_stage1) # [B, L, 1]

        # Prior token boundary mask
        is_token_boundary = torch.isin(input_ids, self.boundary_bytes).float().unsqueeze(-1) # [B, L, 1]

        # Fused Boundary Saliency: Base 0.15 + Neural Pred + Hard Boundary Prior
        saliency = torch.clamp(0.15 + 0.50 * pred_boundary + 0.35 * is_token_boundary, 0.10, 1.00)
        
        # Reshape to [B, 1, L, 1] for broadcasting with SSD beta tensor [B, H, L, 1]
        return saliency.unsqueeze(1) # [B, 1, L, 1]


# =============================================================================
# 4. EXP-61 MASTER AGENT WITH ENTROPY-ADAPTIVE BOUNDARY GATING
# =============================================================================
class Exp61BoundaryGatedCoREAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # Stage 1: Fast Morpho-Syntactic Sheet (Decay 0.005 - 0.15)
        ...
        # Entropy-Adaptive Boundary Detector
        self.boundary_detector = EntropyAdaptiveBoundaryDetector(hidden_dim=self.hidden_dim).to(self.device)

        # Stage 2: Slow Semantic Sheet with Boundary Saliency Modulation (Decay 0.0001 - 0.05)
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        for chunk_idx in range(num_chunks):
            ...
            # Stage 1: Fast Morpho-Syntactic Recurrence
            h_s1, m_s1, dt1 = self.stage1(h_in, m_s1, curr_u_t, dt=1.0)

            # Detect Dynamic Word / Morpheme Boundary Saliency
            saliency_gate = self.boundary_detector(h_s1, chunk_in) # [B, 1, c_len, 1]

            # Stage 2: Slow Semantic Recurrence Gated by Word Boundary Saliency
    ...
    def generate_natural_speech(self, prompt: str, hu_state, max_tokens: int = 75) -> str:
        """Theta-Gamma PAC Decoding with Word Boundary Stability."""
        self.eval()
...
# =============================================================================
# 5. DATASET LOADER & COMPARATIVE BENCHMARK RUNNER
# =============================================================================
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-61 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_61_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-61 (ENTROPY-ADAPTIVE BLT BOUNDARY GATING)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-61 Boundary-Gated 2-Stage Cortical Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp61BoundaryGatedCoREAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-61 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | Boundary Detector: {g_bnd:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-61 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-61 Boundary-Gated Value':<40}")
    print("-" * 95)
