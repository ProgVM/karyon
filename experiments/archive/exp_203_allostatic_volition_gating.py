# experiments/exp_203_allostatic_volition_gating.py
"""
===============================================================================
EXP-203: Allostatically-Gated Hierarchical Volitional Override Module
Grounding: KEP Principle 2 (Biological Realism), Principle 14 (Allostatic Forces: No Static Constants).
===============================================================================
Hypothesis:
Replacing the static linear or partially non-linear Hierarchical Volitional Override projections
with an allostatically-gated volition override mechanism—where the volitional override gate and goal
intensity dynamics are dynamically modulated by a non-linear function of Somatic Energy (u_t[1]),
Noradrenaline (u_t[4]), and Variational Free Energy (F_t)—will optimize volitional override,
prevent volitional overload, and accelerate loss convergence.
"""
...
logger = logging.getLogger("EXP-203")


class AllostaticVolitionGating(nn.Module):
    """
    Allostatically-gated Hierarchical Volitional Override Module.
    """
    ...
    def forward(self, h_s2: torch.Tensor, u_t: torch.Tensor, fe_loss: torch.Tensor) -> torch.Tensor:
        B, S, D = h_s2.shape
        ...
        return h_s2 + gain * self.proj(h_s2)
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-203: ALLOSTATIC VOLITION GATING BENCHMARK]")
    logger.info("=" * 80)
    ...
    # 2. Proposed Evaluation (EXP-203 Allostatic Volition Gating)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain
    brain_prop.allostatic_volition_gating = AllostaticVolitionGating(
        hidden_dim=brain_prop.hidden_dim, device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Allostatic Volition Gating Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-203 TELEMETRY REPORT ===")
    ...
    with open("experiments/exp_203_results.json", "w") as f:
