# experiments/exp_82_fused_parallel_sequence_scan.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-82 (FUSED CHUNK-PARALLEL CORTICAL SCAN)
Hypothesis: Formulating the 2-Stage Cortical Stack with Fused Chunk-Parallel SSD:
  1. Computes intra-chunk projections (Q, K, V, Z, Delta), RoPE, and intra-chunk 
     attention in a single batched GEMM over all chunks [B * num_chunks, H, Q, head_k].
  2. Propagates inter-chunk recurrence m_{c+1} = alpha_c * m_c + kv_chunk_update_c 
     with 100% untruncated BPTT autograd graph across all chunks.
  3. Executes SwiGLU, Boundary Detector, PW-LPER, Hopfield Attractor, and Motor 
     projections as full-sequence batched operations [B, S, D].
This will collapse batch latency, surging throughput to >120,000 tok/s with VRAM < 4 GB,
while providing deep 1024-token temporal gradients to break the speech loss floor.
...
class FusedChunkParallelSSDLayer(nn.Module):
    ...
    def forward(self, x_seq: torch.Tensor, m_prev: torch.Tensor, u_t: torch.Tensor,
        ...
        if seq_len < Q:
            ...
            # Pad sequence on the left if it's not a multiple of Q
        ...
        q_full = self.q_proj(x_seq).view(batch_size, num_chunks, Q, self.num_heads, self.head_k).transpose(2, 3) * self.inv_sqrt_k # [B, C, H, Q, K]
        k_full = self.k_proj(x_seq).view(batch_size, num_chunks, Q, self.num_heads, self.head_k).transpose(2, 3) # [B, C, H, Q, K]
        v_full = self.v_proj(x_seq).view(batch_size, num_chunks, Q, self.num_heads, self.head_v).transpose(2, 3) # [B, C, H, Q, V]
        z_full = F.silu(self.z_proj(x_seq)).view(batch_size * seq_len, self.num_heads * self.head_v)

        # Apply RoPE per chunk
        cos, sin = self._get_rope_cos_sin(Q, x_seq.device, x_seq.dtype)
        q_full, k_full = apply_rotary_pos_emb(q_full, k_full, cos, sin)

        # Delta & Decay computation
        delta_full = F.softplus(self.delta_proj(x_seq)).view(batch_size, num_chunks, Q, self.num_heads).permute(0, 1, 3, 2) # [B, C, H, Q]
        base_alpha = torch.sigmoid(self.decay_logits) # [1, 1, H, 1]
        alpha = torch.pow(base_alpha, (delta_full * eff_dt.squeeze(-1)).clamp(0.1, 10.0)) # [B, C, H, Q]
        ...
        # 2. Intra-chunk exact log-space decay scan (batched over all B * C)
        lambda_t = torch.cumsum(log_alpha, dim=-1) # [B, C, H, Q]
        log_decay_matrix = lambda_t.unsqueeze(-1) - lambda_t.unsqueeze(-2) # [B, C, H, Q, Q]
        ...
        # Intra-chunk attention GEMM
        s_matrix = torch.matmul(q_full, k_full.transpose(-1, -2)) * decay_weights # [B, C, H, Q, Q]
        y_intra = torch.matmul(s_matrix, v_full) # [B, C, H, Q, V]

        # 3. Inter-chunk state propagation (Sequential recurrence over C chunks with full autograd)
        decay_to_start = torch.exp(torch.clamp(lambda_t, -20.0, 0.0)).unsqueeze(-1) # [B, C, H, Q, 1]
        lambda_end = lambda_t[:, :, :, -1:].unsqueeze(-1) # [B, C, H, 1, 1]
        decay_to_end = torch.exp(torch.clamp(lambda_end - lambda_t.unsqueeze(-1), -20.0, 0.0)) # [B, C, H, Q, 1]
        
        k_decayed = k_full.float() * decay_to_end * beta.unsqueeze(-1).float() # [B, C, H, Q, K]
        kv_chunk_updates = torch.matmul(k_decayed.transpose(-1, -2), v_full.float()) # [B, C, H, K, V]
        alpha_chunks = torch.exp(torch.clamp(lambda_t[:, :, :, -1:], -20.0, 0.0)).unsqueeze(-1) # [B, C, H, 1, 1]

        m_curr = m_prev.float() # [B, H, K, V]
        y_inter_list = []

        sigma_somatic = 1e-3 * (0.8 * curiosity.squeeze(1).float() + 0.4 * na.squeeze(1).float() + 0.1) # [B, 1, 1, 1]

        for c in range(num_chunks):
            # Inter-chunk readout for chunk c
            q_c = q_full[:, c] # [B, H, Q, K]
            dec_start_c = decay_to_start[:, c] # [B, H, Q, 1]
            y_inter_c = torch.matmul(q_c.float() * dec_start_c, m_curr).to(q_full.dtype) # [B, H, Q, V]
            y_inter_list.append(y_inter_c)

            # State transition to next chunk
            alpha_c = alpha_chunks[:, c] # [B, H, 1, 1]
            kv_c = kv_chunk_updates[:, c] # [B, H, K, V]
            dW_c = torch.randn_like(m_curr) * torch.sqrt(eff_dt.squeeze(1).float()) * sigma_somatic
            m_curr = alpha_c * m_curr + kv_c + dW_c

        y_inter = torch.stack(y_inter_list, dim=1) # [B, C, H, Q, V]
        ...
        # Slice output to original seq_len if padded
        if pad_len > 0:
            h_seq = h_seq[:, pad_len:, :]
...
# =============================================================================
# 3. EXP-82 FUSED CHUNK-PARALLEL CORE AGENT
# =============================================================================
class Exp82FusedChunkParallelAgent(nn.Module):
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, 
        ...
        saliency_gate = self.boundary_detector(h_s1, input_seq) # [B, 1, S]
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-82 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_82_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-82 (FUSED CHUNK-PARALLEL CORTICAL SCAN)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-82 Fused Chunk-Parallel CoREAgent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp82FusedChunkParallelAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-82 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-82 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-82 Fused Chunk-Parallel Value':<40}")
    print("-" * 95)
