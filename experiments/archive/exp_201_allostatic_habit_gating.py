# experiments/exp_201_allostatic_habit_gating.py
"""
===============================================================================
EXP-201: Allostatically-Gated Subcortical Reflex & Habit Module
Grounding: KEP Principle 2 (Biological Realism), Principle 14 (Allostatic Forces: No Static Constants).
===============================================================================
Hypothesis:
Replacing the static linear or partially non-linear Subcortical Reflex & Habit Module projections
with an allostatically-gated habit policy mechanism—where the conditioned habit policy and basal
ganglia associative mapping dynamics are dynamically modulated by a non-linear function of Somatic Energy (u_t[1]),
Noradrenaline (u_t[4]), and Variational Free Energy (F_t)—will optimize habit selection,
prevent habit overload, and accelerate loss convergence.
"""
...
logger = logging.getLogger("EXP-201")


class AllostaticHabitGating(nn.Module):
    """
    Allostatically-gated Subcortical Reflex & Habit Module.
    """
    def __init__(self, unified_dim: int, device_str: str = 'cpu'):
        super().__init__()
    ...
    def forward(self, w_t: torch.Tensor, u_t: torch.Tensor, fe_loss: torch.Tensor) -> torch.Tensor:
        B, S, D = w_t.shape
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-201: ALLOSTATIC HABIT GATING BENCHMARK]")
    logger.info("=" * 80)
    ...
    # 1. Baseline Evaluation (EXP-200 Motor Gating)
    ...
    # 2. Proposed Evaluation (EXP-201 Allostatic Habit Gating)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain
    brain_prop.allostatic_habit_gating = AllostaticHabitGating(
        unified_dim=brain_prop.unified_dim, device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Allostatic Habit Gating Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-201 TELEMETRY REPORT ===")
    ...
    with open("experiments/exp_201_results.json", "w") as f:
