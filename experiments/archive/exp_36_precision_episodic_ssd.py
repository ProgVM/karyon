"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-36 (PRECISION-GATED EPISODIC SSD)
Evaluation of Active Precision-Gated Episodic Memory (Hippocampal CLS, tau >= 0.80)
+ Cosine Annealing LR vs Baseline Multi-Timescale SSD on vicgalle/alpaca-gpt4.
Protocol: KEP v5.0 (Rules #1, #2, #3, #4, #6, #7).
Biophysical Basis: Complementary Learning Systems (McClelland 1995, Kumaran 2016).
...
print(f"\n[EXP-36] Initializing Precision-Gated Episodic SSD Benchmark on Device: {device_str.upper()}")
...
# =============================================================================
# 4. BASELINE MODEL: STANDALONE MULTI-TIMESCALE SSD (NO EPISODIC INJECTION)
# =============================================================================
class BaselineMultiTimescaleAgent(nn.Module):
    def __init__(self, device_str='cpu'):
        ...
        # Uses Native C++ CalibratedParallelSSDCore with Multi-Timescale Spectrum
...
# =============================================================================
# 5. PROPOSED MODEL: EXP-36 PRECISION-GATED EPISODIC MULTI-TIMESCALE SSD
# =============================================================================
class PrecisionEpisodicSSDAgent(nn.Module):
    def __init__(self, device_str='cpu'):
        super().__init__()
        self.device_str = device_str
        self.device = torch.device(device_str)
        ...
        # 1. Neocortex: Native Multi-Timescale SSD Core (alpha in [0.70, 0.999])
        self.ssd_core = CalibratedParallelSSDCore(
            text_dim=self.text_dim, unified_dim=self.unified_dim, hidden_dim=self.hidden_dim,
            num_heads=self.num_heads, head_k=self.head_k, head_v=self.head_v, device=device_str
        )
        ...
        # 2. Hippocampus: Pattern Separation Projection & Precision Recall Injection
        self.episodic_proj = nn.Linear(self.text_dim, self.unified_dim).to(self.device)
        self.memory_recall_proj = nn.Sequential(
            nn.Linear(self.unified_dim, self.hidden_dim),
            nn.SiLU(),
            nn.LayerNorm(self.hidden_dim)
        ).to(self.device)

        self.fusion_norm = nn.LayerNorm(self.hidden_dim).to(self.device)
    ...
    def get_all_parameters(self) -> List[nn.Parameter]:
        params = (
            ...
            list(self.memory_recall_proj.parameters()) +
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, 
                         ...
                         chunk_size: int = 64, optimizer: torch.optim.Optimizer = None):
        batch_size, seq_len = input_seq.size()
        m_curr = torch.zeros(batch_size, self.num_heads, self.head_k, self.head_v, device=self.device)
        curr_u_t = hu_batch.state.clone().detach()
        ...
        for chunk_idx in range(num_chunks):
            ...
            # 1. Neocortical Scan: Multi-Timescale SSD + SwiGLU Reasoning
            ssd_out = self.ssd_core.forward_chunk_parallel_ssd(chunk_emb, m_curr, curr_u_t, 1.0)
            h_chunk, m_curr = ssd_out[0], ssd_out[1]
            h_reasoned = self.channel_mixer(h_chunk) # [B * c_len, 512]

            # 2. Hippocampal Precision Recall: Active Inference Query
            na = curr_u_t[:, 4:5]
            curiosity = curr_u_t[:, 0:1]
            energy = curr_u_t[:, 1:2]
            volitional_gate = torch.sigmoid(2.0 * na + 1.5 * curiosity - 0.5 * (1.0 - energy)) # [B, 1]

            # Formulate High-Entropy Query from Last Byte Representation
            query_key = self.episodic_proj(chunk_emb[:, -1, :]) # [B, 256]
            
            # Read from Hippocampal Episodic Memory with Precision Threshold tau = 0.80
            retrieved_mem, max_sim = episodic_memory.read(
                query_key.detach(), temperature=0.05, threshold=0.80, sigmoid_beta=15.0
            ) # [B, 256], [B, 1]

            # Expand retrieved memory across chunk tokens with precision gating
            gated_recall = (retrieved_mem * volitional_gate).unsqueeze(1).repeat(1, c_len, 1).view(batch_size * c_len, self.unified_dim)
            h_recalled = self.memory_recall_proj(gated_recall) # [B * c_len, 512]

            # 3. Neocortical-Hippocampal Complementary Fusion
            h_fused = self.fusion_norm(h_reasoned + h_recalled)

            # 4. Attractor Minimization & Readout
            h_relaxed = self.attractor_head.relax_to_minima(h_fused)[0]
            h_proj = self.motor_text_proj(h_relaxed)
            logits_flat = F.linear(h_proj, self.pos_embeddings.byte_embed.weight) * self.inv_sqrt_text_dim
            ...
            # 5. One-Shot Hippocampal Storage: Write on High Surprise (Loss > 1.20)
            with torch.no_grad():
                if chunk_loss.item() > 1.20:
                    episodic_memory.write(query_key.detach(), query_key.detach(), protected_slots=3)
    ...
    def generate_thought_and_speech(self, prompt: str, hu, episodic_memory: BatchedEpisodicMemory,
                                   max_generated_tokens: int = 65, temperature: float = 0.7, top_p: float = 0.90) -> str:
        ...
        for step in range(max_generated_tokens):
            ...
            # Precision Hippocampal Recall during generation
            q_k = self.episodic_proj(ctx_emb.squeeze(1))
            ret_mem, max_sim = episodic_memory.read(q_k, temperature=0.05, threshold=0.80, sigmoid_beta=15.0)
            h_rec = self.memory_recall_proj(ret_mem)
            h_fused = self.fusion_norm(h_out + h_rec)
...
# =============================================================================
# 7. EXP-36 MASTER EXECUTION & AUTOMATED DECISION ENGINE
# =============================================================================
def run_exp_36_benchmark():
    print("\n" + "="*85)
    print(" === EXP-36: PRECISION-GATED EPISODIC MEMORY SSD BENCHMARK ===")
    print("="*85)
    ...
    # -------------------------------------------------------------------------
    # PART A: RUN BASELINE MULTI-TIMESCALE SSD (NO EPISODIC INJECTION)
    # -------------------------------------------------------------------------
    print("\n" + "-"*85)
    print(" [1/2] RUNNING BASELINE: MULTI-TIMESCALE SSD (NO EPISODIC RECALL, FIXED LR)")
    print("-" * 85)

    model_base = BaselineMultiTimescaleAgent(device_str=device_str).to(device)
    ...
    # -------------------------------------------------------------------------
    # PART B: RUN PROPOSED EXP-36 PRECISION EPISODIC SSD + COSINE LR
    # -------------------------------------------------------------------------
    print("\n" + "-"*85)
    print(" [2/2] RUNNING PROPOSED: EXP-36 PRECISION-GATED EPISODIC SSD (tau=0.80, COSINE LR)")
    print("-" * 85)

    model_prop = PrecisionEpisodicSSDAgent(device_str=device_str).to(device)
    ...
    # Cosine Annealing Learning Rate Scheduler (3e-3 -> 1e-4)
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt_prop, T_max=len(loader), eta_min=1e-4)
    ...
    for idx, batch_tokens in enumerate(loader):
        ...
        opt_prop.zero_grad()
        loss = model_prop.forward_sequence(
            in_seq, tgt_seq, hu, criterion, episodic_memory=episodic_mem, 
            chunk_size=64, optimizer=opt_prop
        )
        ...
        if (idx + 1) % 20 == 0 or idx == len(loader) - 1:
            print(f"  Proposed Step [{idx+1:02d}/{len(loader)}] | Loss: {loss:.4f} (PPL: {math.exp(min(loss, 20.0)):.2f}) | LR: {cur_lr:.5f}")
    ...
    sample_1 = model_prop.generate_thought_and_speech(prompt_1, diag_hu, diag_mem, max_generated_tokens=65)
    sample_2 = model_prop.generate_thought_and_speech(prompt_2, diag_hu, diag_mem, max_generated_tokens=65)
    ...
    # -------------------------------------------------------------------------
    print("\n" + "="*85)
    print(" === EXP-36 GRADIENT FLOW INSPECTION (PROPOSED MODEL) ===")
    print("="*85)
    ...
    print("\n" + "="*85)
    print(" === EXP-36 FINAL TELEMETRY AUDIT TABLE ===")
    print("="*85)
    print(f"{'Metric':<32} | {'Baseline (No Episodic, Fixed LR)':<32} | {'Proposed (EXP-36 Precision Episodic)':<34}")
    print("-" * 95)
    print(f"{'Parameter Count':<32} | {param_count_base:<32,} | {param_count_prop:<34,}")
    print(f"{'Episodic Precision Threshold':<32} | {'N/A (Disabled)':<32} | {'tau >= 0.80 (Precision Gated)':<34}")
    print(f"{'LR Schedule':<32} | {'Fixed lr=3e-3':<32} | {'Cosine (3e-3 -> 1e-4)':<34}")
    ...
    print(f"\n[EXP-36 Decision Engine] Delta Loss: {delta_loss:+.4f} | Throughput Retention: {speed_ratio:.1f}%")
    
    if delta_loss <= -0.08 and speed_ratio >= 80.0:
        verdict = "🟢 POSITIVE — Significant complementary learning gain on real dataset with high throughput retention. Ready for production merge."
