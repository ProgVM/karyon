# experiments/exp_73_conv_swiglu_channel_mixing.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-73 (CAUSAL DEPTHWISE CONVSWIGLU CHANNEL-MIXING)
Hypothesis: In EXP-70, ParallelSwiGLUBlock performed non-linear channel mixing 
purely point-wise per byte position (1x1). However, byte-level associative knowledge 
synthesis benefits from local temporal context when gating channel interactions.
By inserting a Causal Depthwise Conv1d (K=3) into the SwiGLU gate projection branch before SiLU:
  gate = SiLU(CausalConv1d_K3(W_gate X))
  up = W_up X
  ffn_out = W_down (gate * up)
the SwiGLU knowledge block acquires local n-gram context during channel synthesis, 
eliminating byte-entropy plateaus and driving steady-state speech loss below 1.60 nats/byte.
...
# =============================================================================
# 3. CAUSAL DEPTHWISE CONVSWIGLU CHANNEL-MIXING BLOCK (EXP-73)
# =============================================================================
class CausalConvSwiGLUBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        raw_gate = self.w_gate(x) # [B, L, E]
        
        # Apply Causal Depthwise Conv1d (K=3) to gate branch (padding=2 on left)
        gate_trans = raw_gate.transpose(1, 2) # [B, E, L]
        gate_pad = F.pad(gate_trans, (2, 0)) # Causal left-padding 2
        conv_gate = self.gate_conv(gate_pad).transpose(1, 2) # [B, L, E]
        
        gate = F.silu(conv_gate)
        up = self.w_up(x) # [B, L, E]
        
        ffn_out = self.w_down(gate * up) # [B, L, D]
        return self.norm(x + ffn_out)


# =============================================================================
# 4. CORTICAL STAGE WITH CONVSWIGLU
# =============================================================================
...
# =============================================================================
# 5. ENTROPY-ADAPTIVE BOUNDARY DETECTOR
# =============================================================================
...
# =============================================================================
# 6. EXP-73 MASTER AGENT WITH CONVSWIGLU
# =============================================================================
class Exp73ConvSwiGLUAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # Stage 1: Fast Morpho-Syntactic Sheet with ConvSwiGLU
        ...
        # Stage 2: Slow Semantic Sheet with ConvSwiGLU
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-73 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_73_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-73 (CAUSAL DEPTHWISE CONVSWIGLU CHANNEL-MIXING)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-73 ConvSwiGLU Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp73ConvSwiGLUAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-73 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | Pyramid RF: {g_rf:.4f} | S1: {g_s1:.4f} | ConvSwiGLU: {g_sw1:.4f} | S2: {g_s2:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-73 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-73 ConvSwiGLU Value':<40}")
    print("-" * 95)
