# experiments/exp_78_gated_afferent_efferent_highway.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-78 (GATED AFFERENT-EFFERENT RESIDUAL HIGHWAY)
Hypothesis: Providing a direct gated afferent skip connection from the sensory byte 
embeddings (x_emb in 256D) to the motor text readout (h_proj in 256D):
  h_proj_final = MotorTextProj(h_relaxed) + gate_skip * LayerNorm(x_emb)
allows direct bigram/morpheme transition modeling in the afferent-efferent space,
relieving the deep 2-Stage Cortical Stack to focus entirely on higher-order semantic reasoning.
...
# =============================================================================
# 7. EXP-78 AGENT WITH GATED AFFERENT-EFFERENT RESIDUAL HIGHWAY (GAE-RH)
# =============================================================================
class Exp78GatedAfferentEfferentAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # Gated Afferent-Efferent Residual Skip Highway (GAE-RH)
        self.sensory_skip_norm = nn.LayerNorm(self.text_dim).to(self.device)
        self.skip_gate = nn.Parameter(torch.tensor([0.20], device=self.device)) # Learnable scalar gate
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        for chunk_idx in range(num_chunks):
            ...
            chunk_emb_raw = full_emb[:, c_start:c_end, :]
            ...
            # --- GATED AFFERENT-EFFERENT RESIDUAL HIGHWAY ---
            h_proj_cortical = self.motor_text_proj(h_relaxed).view(batch_size, c_len, self.text_dim)
            h_proj_skip = self.sensory_skip_norm(chunk_emb_raw)
            h_proj = h_proj_cortical + self.skip_gate * h_proj_skip
    ...
    def generate_natural_speech(self, prompt: str, hu_state, max_tokens: int = 75) -> str:
        ...
        with torch.no_grad():
            for step in range(max_tokens):
                ...
                h_proj_cortical = self.motor_text_proj(h_rel)
                h_proj_skip = self.sensory_skip_norm(t_emb.squeeze(1))
                h_proj = h_proj_cortical + self.skip_gate * h_proj_skip
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-78 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_78_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-78 (GATED AFFERENT-EFFERENT RESIDUAL HIGHWAY)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-78 GAE-RH Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp78GatedAfferentEfferentAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            g_skip = agent.skip_gate.grad.item() if agent.skip_gate.grad is not None else 0.0
            g_s1 = 0.0
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-78 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            print(f"Metrics Progress          : Speech Loss = {speech_loss:.4f} (Avg: {cur_loss:.4f}, PPL: {cur_ppl:.2f}) | LR = {cur_lr:.6f}")
            print(f"Precision Dynamics (Pi_t) : Mean Ascending Error Precision = {avg_pi:.4f} | Skip Gate = {agent.skip_gate.item():.4f}")
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | Skip Gate Grad: {g_skip:.6f} | S1: {g_s1:.4f} | S2: {g_s2:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-78 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-78 GAE-RH Value':<40}")
    print("-" * 95)
