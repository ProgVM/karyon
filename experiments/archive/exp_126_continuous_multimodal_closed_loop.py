...
def run_closed_loop_environment():
    ...
    def generate_environment_step(step_idx):
        # Simulated continuous environmental percepts (D=256 unified embeddings or 3D control)
        # Using embedding layer or 256D text embedding representation
        text_tokens = torch.tensor([[ord(c) % 256 for c in f"Env Step {step_idx}"]], dtype=torch.long, device=device_str)
        text_features = agent.pos_embeddings.byte_embed(text_tokens).mean(dim=1) # [1, 256]
        
        vision_vector = torch.randn(1, 256, device=device_str) + (0.05 * step_idx) # Evolving visual scene
        motor_proprioception = torch.tensor([[0.5 * math.sin(step_idx * 0.2), 0.5 * math.cos(step_idx * 0.2), 0.1 * step_idx]], device=device_str)
        
        return text_features, vision_vector, motor_proprioception
    ...
    for step in range(1, num_env_steps + 1):
        ...
        if chosen_action == "EXPRESS_OUTPUT":
            ...
            # Execute System 2 Mental Rollout in Latent Predictor / World Model
            with torch.no_grad():
                w_pred, kl_div, fe_sim, z_sim = agent.world_model(h_unified, h_unified, vision_vec)
            action_detail = f"Executed 1-step System 2 counterfactual rollout (Sim FE: {fe_sim.mean().item():.4f})"
