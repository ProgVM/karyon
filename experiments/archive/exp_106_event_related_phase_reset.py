# experiments/exp_106_event_related_phase_reset.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-106
Hypothesis:
In EXP-105, Additive Phase-Shift Offset Modulation (APSOM) was gradient-healthy and
14% faster, but yielded neutral loss because the phase was free-running.
By introducing an Event-Related Phase Reset (ERPR) mechanism, we force the theta phase
to reset to its trough (cos(phi_t) = -1.0) at word boundaries (detected via saliency_gate).
This aligns the phase-amplitude modulation directly with the linguistic boundaries,
accelerating convergence and reducing speech loss.
...
def prepare_packed_stream(num_batches: int = 150, batch_size: int = 12, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-106 (S={seq_len}, Steps={num_batches})...")
...
class EventRelatedPhaseResetBridge(nn.Module):
    """
    Event-Related Phase Reset (ERPR) APSOM Module:
    1. Computes theta phase phi_t from Stage 1 hidden states.
    2. Receives saliency_gate (word boundary detector).
    3. Resets phase to trough (cos_phi = -1.0) at boundaries:
       cos_phi_reset = (1.0 - saliency_gate) * cos_phi - saliency_gate * 1.0
    4. Projects learned offset: o_t = cos_phi_reset * W_offset(h_stage1).
    5. Adds offset to Stage 1 output: h_modulated = h_stage1 + o_t.
    """
    ...
    def forward(self, h_s1: torch.Tensor, saliency_gate: torch.Tensor) -> tuple:
        # Compute phase coordinates
        phase_raw = self.phase_net(h_s1)
        phase_norm = F.normalize(phase_raw, p=2, dim=-1)
        cos_phi = phase_norm[..., 0:1]

        # Event-Related Phase Reset: force cos_phi to -1.0 at boundaries
        # saliency_gate shape is [B, 1, S] or [B, S, 1]. Let's ensure it matches cos_phi shape [B, S, 1]
        if saliency_gate.dim() == 3 and saliency_gate.size(1) == 1:
            saliency_gate = saliency_gate.transpose(1, 2) # [B, S, 1]
        
        cos_phi_reset = (1.0 - saliency_gate) * cos_phi - saliency_gate * 1.0

        # Compute learned offset
        raw_offset = self.offset_proj(h_s1)
        modulated_offset = cos_phi_reset * raw_offset

        # Additive modulation
        h_modulated = h_s1 + modulated_offset

        return h_modulated, cos_phi_reset


class ProposedERPRAgent(CoREAgent):
    """Karyon-CoRE Agent integrated with Event-Related Phase Reset APSOM."""
    def __init__(self, config, device='cpu'):
        super().__init__(config, device)
        self.pac_bridge = EventRelatedPhaseResetBridge(hidden_dim=self.hidden_dim, device_str=self.device_str)
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, 
        ...
        # Apply Event-Related Phase Reset APSOM
        h_s1_modulated, cos_phi_reset = self.pac_bridge(h_s1, saliency_gate)
        ...
        return total_loss_tensor, speech_loss_val, fe_loss_val, critic_loss_val, m_s2, h_proxy, curr_u_t, eff_dt, cos_phi_reset.mean().item()


def run_exp_106_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-106 (EVENT-RELATED PHASE RESET APSOM)] ===")
    print("="*85)
    ...
    # 2. EVALUATE PROPOSED (ProposedERPRAgent with Event-Related Phase Reset)
    print("\n" + "-"*85)
    print(" >>> RUNNING PHASE 2: PROPOSED (ProposedERPRAgent) <<<")
    print("-"*85)
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats()

    agent_prop = ProposedERPRAgent(config, device=device_str).to(device)
    ...
    for step, batch_tensors in enumerate(batches):
        ...
        opt_prop.zero_grad()
        with torch.amp.autocast(device_type=device_str, dtype=torch.float16, enabled=use_amp):
            tot_loss, s_loss, fe_loss, r_loss, m_s2, h_p, u_t, eff_dt, cos_phi_mean = agent_prop.forward_sequence(
    ...
    with open("exp_106_metrics.json", "w") as f:
        json.dump(metrics_summary, f, indent=2)


if __name__ == "__main__":
    run_exp_106_benchmark()
