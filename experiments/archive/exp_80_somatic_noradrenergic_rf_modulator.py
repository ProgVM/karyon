# experiments/exp_80_somatic_noradrenergic_rf_modulator.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-80 (SOMATIC NORADRENERGIC RECEPTIVE FIELD MODULATOR - SN-RFM)
Tests Somatic Noradrenergic Receptive Field Modulation (SN-RFM) in the Multi-Scale Pyramid RF:
  g_logits = Linear(x_seq) + Linear_homeo(u_t)
  g_logits[K=2] += 1.5 * NA_t (Scrutinize micro-scale bytes on surprise)
  g_logits[K=8] -= 1.0 * NA_t (Expand macro-scale stems on calm processing)
  gates = Softmax(g_logits)
...
# =============================================================================
# 0. SOMATIC NORADRENERGIC MULTI-SCALE BYTE PYRAMID RECEPTIVE FIELD
# =============================================================================
class SomaticNoradrenergicBytePyramidRF(nn.Module):
    def __init__(self, text_dim: int = 256, homeo_dim: int = 6):
        super().__init__()
        ...
        self.scale_gate_text = nn.Linear(text_dim, 3)
        self.scale_gate_homeo = nn.Linear(homeo_dim, 3, bias=False)
        self.norm = nn.LayerNorm(text_dim)

    def forward(self, x_seq: torch.Tensor, u_t: torch.Tensor = None) -> torch.Tensor:
        ...
        gate_logits = self.scale_gate_text(x_seq) # [B, L, 3]

        if u_t is not None:
            if u_t.size(0) != batch_size:
                u_t = u_t[:batch_size]
            homeo_bias = self.scale_gate_homeo(u_t).unsqueeze(1) # [B, 1, 3]
            na_level = u_t[:, 4:5].unsqueeze(1) # [B, 1, 1]

            # Biplicately bias K=2 (micro-scrutiny) on high NA and K=8 (macro-stems) on low NA
            na_bias = torch.cat([1.5 * na_level, torch.zeros_like(na_level), -1.0 * na_level], dim=-1)
            gate_logits = gate_logits + homeo_bias + na_bias
...
class SomaticPositionalByteEmbedding(nn.Module):
    def __init__(self, vocab_size=258, text_dim=256, max_len=8192):
        super().__init__()
        self.vocab_size = vocab_size
        self.text_dim = text_dim
        self.byte_embed = nn.Embedding(vocab_size, text_dim)
        self.receptive_field = SomaticNoradrenergicBytePyramidRF(text_dim=text_dim)
    ...
    def forward(self, input_ids: torch.Tensor, u_t: torch.Tensor = None, start_pos: int = 0, apply_rf: bool = True) -> torch.Tensor:
        ...
        if apply_rf and seq_len > 1:
            embedded = self.receptive_field(embedded, u_t=u_t)
        return embedded
...
# =============================================================================
# 7. EXP-80 SOMATIC NORADRENERGIC RECEPTIVE FIELD MODULATED AGENT
# =============================================================================
class Exp80SomaticNoradrenergicRFAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        self.pos_embeddings = SomaticPositionalByteEmbedding(
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        # Pass u_t into Somatic Noradrenergic Receptive Field
        full_emb = self.pos_embeddings(input_seq, u_t=curr_u_t, start_pos=0, apply_rf=True)
    ...
    def generate_natural_speech(self, prompt: str, hu_state, max_tokens: int = 75) -> str:
        ...
        prompt_emb = self.pos_embeddings(prompt_t, u_t=hu_state, start_pos=0, apply_rf=True)
        ...
        with torch.no_grad():
            for step in range(max_tokens):
                ...
                win_emb = self.pos_embeddings(win_t, u_t=hu_state, start_pos=len(rolling_ids) - len(context), apply_rf=True)
                t_emb = win_emb[:, -1:, :]
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-80 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_80_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-80 (SOMATIC NORADRENERGIC RECEPTIVE FIELD MODULATOR)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-80 Somatic Noradrenergic Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp80SomaticNoradrenergicRFAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-80 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | Somatic RF Grad: {g_rf:.4f} | S1: {g_s1:.4f} | S2: {g_s2:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-80 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-80 Somatic Noradrenergic Value':<40}")
    print("-" * 95)
