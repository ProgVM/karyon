# experiments/exp_71_gated_cortical_composition.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-71 (HIERARCHICAL GATED CORTICAL COMPOSITION & JOINT TOP-DOWN PREDICTIVE CODING - HGCC-JTPC)
Hypothesis: In the 2-Stage Cortical Stack, simple un-gated addition (h_s1 + h_s2) 
forces equal weighting between fast morpho-syntactic details (h_s1) and slow discourse 
semantics (h_s2). Furthermore, Stage 1's predictor is blind to Stage 2's slow discourse state.
By introducing:
  1. Joint Top-Down Predictive Coding: h1_pred = Pred([h1_prev, h2_prev]), so Stage 1's 
     residual error e1 = h1 - h1_pred represents novelty relative to BOTH local phonetics 
     AND global discourse context.
  2. Hierarchical Gated Cortical Composition: g_laminar = sigmoid(W_gate [h_s1, h_s2]) 
     and h_combined = 2.0 * (g_laminar * h_s1 + (1 - g_laminar) * h_s2), enabling dynamic 
     byte-level routing between local syntax and high-level semantics.
...
# =============================================================================
# 3. CORTICAL STAGE WITH MAMBA-2 GATED SSD
# =============================================================================
...
# =============================================================================
# 5. EXP-71 MASTER AGENT (HGCC-JTPC)
# =============================================================================
class Exp71HierarchicalGatedAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # JOINT TOP-DOWN PREDICTIVE CODING: Predicts Stage 1 next state from BOTH Stage 1 and Stage 2 previous states
        self.joint_topdown_pred_net = nn.Sequential(
        ...
        # HIERARCHICAL GATED CORTICAL COMPOSITION: Dynamic gating between Stage 1 & Stage 2
        self.laminar_composition_gate = nn.Sequential(
            nn.Linear(self.hidden_dim * 2, self.hidden_dim),
            nn.SiLU(),
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.Sigmoid()
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        for chunk_idx in range(num_chunks):
            ...
            # --- JOINT TOP-DOWN PREDICTIVE CODING ---
            # Predict Stage 1 state using joint local (Stage 1) and global (Stage 2) histories
            h1_shifted = torch.cat([h1_prev_last, h_s1[:, :-1, :]], dim=1)
            h2_shifted = h2_prev_last.expand(-1, c_len, -1)
            joint_history = torch.cat([h1_shifted, h2_shifted], dim=-1)
            
            h1_pred = self.joint_topdown_pred_net(joint_history)
            e1 = h_s1 - h1_pred
            h1_prev_last = h_s1[:, -1:, :].detach()

            # Stage 2: Slow Semantic Pass on Joint Error e1
            ...
            # --- HIERARCHICAL GATED CORTICAL COMPOSITION ---
            # Dynamic byte-level gate selecting balance between local morpho-syntax and global semantics
            stacked_laminar = torch.cat([h_s1, h_s2], dim=-1)
            g_laminar = self.laminar_composition_gate(stacked_laminar)
            
            # Scaled convex combination (2.0 factor preserves signal norm)
            h_combined = 2.0 * (g_laminar * h_s1 + (1.0 - g_laminar) * h_s2)
    ...
    def generate_natural_speech(self, prompt: str, hu_state, max_tokens: int = 75) -> str:
        ...
        for c_idx in range(0, prompt_len, 64):
            ...
            e1 = h_s1 - self.joint_topdown_pred_net(joint_history)
            h1_prev_last = h_s1[:, -1:, :].detach()
        ...
        with torch.no_grad():
            for step in range(max_tokens):
                ...
                h1_shifted = torch.cat([h1_prev_last, h_s1[:, :-1, :]], dim=1)
                h2_shifted = h2_prev_last.expand(-1, 1, -1)
                joint_history = torch.cat([h1_shifted, h2_shifted], dim=-1)
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-71 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_71_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-71 (HIERARCHICAL GATED CORTICAL COMPOSITION)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-71 Hierarchical Gated Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp71HierarchicalGatedAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            g_comp = 0.0
            for p in agent.laminar_composition_gate.parameters():
                if p.grad is not None:
                    g_comp = max(g_comp, p.grad.norm().item())

            print("\n" + "="*95)
            print(f" === [KEP EXP-71 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | Pyramid RF: {g_rf:.4f} | S1: {g_s1:.4f} | S2: {g_s2:.4f} | Gate: {g_comp:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-71 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-71 Hierarchical Gated Value':<40}")
    print("-" * 95)
