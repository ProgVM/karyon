# experiments/exp_96_pw_hpc.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-96
Hypothesis:
1. In EXP-94, True Hierarchical Predictive Coding (HPC) provided a 4x reduction in Free Energy
   and a 46% throughput boost, but its speech loss delta (+0.0182) was within noise margin because
   ascending top-down prediction errors were routed without dynamic precision weighting.
2. In EXP-96, Single-Pass Precision-Weighted True Hierarchical Predictive Coding (SP-PW-HPC)
   combines Stage 2 top-down generative prediction (h_s1_hat = f_td(h_s2_prev)), ascending prediction error
   e1 = h_s1 - h_s1_hat, dynamic precision weighting pi_t = 2.0 * sigmoid(W_pi [h_s1, h_s1_hat, NA_t]),
   and EXP-95's dynamic entropy-adaptive dt scaling.
3. This closes the Bastos-Friston canonical microcircuit loop, providing superior semantic
   compression and driving speech loss reduction beyond baseline (>0.08 nats/byte).
...
def prepare_packed_stream(num_batches: int = 150, batch_size: int = 16, seq_len: int = 512):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-96 (S={seq_len}, B={batch_size}, Steps={num_batches})...")
...
class PrecisionWeightedTopDownGenerator(nn.Module):
    """
    Top-Down Generative Projection & Dynamic Precision Estimator:
...
class PWHPC_CoREAgent(CoREAgent):
    """
    Karyon-CoRE Agent featuring Precision-Weighted True Hierarchical Predictive Coding (PW-HPC).
    """
    ...
    def get_all_parameters(self):
        ...
        params.extend(list(self.pw_hpc_generator.parameters()))
        return params
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, 
        ...
        with torch.amp.autocast(device_type=self.device_str, dtype=torch.float16, enabled=(self.device_str == 'cuda')):
            ...
            # EXP-95: Dynamic Entropy-Adaptive dt Scaling
            predicted_entropy = self.entropy_predictor(h_s1)
            dynamic_dt_scale = 0.40 + 1.20 * predicted_entropy

            # Stage 2: Slow Semantic-Discourse Cortical Sheet on Precision-Weighted Prediction Errors
            ...
            # Modern Hopfield Attractor Landscape
...
def run_exp_96_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-96 (PW-HPC HIERARCHICAL PREDICTIVE CODING)] ===")
    print("="*85)
    print(f"Device: {device_str.upper()} | AMP FP16 Enabled: {use_amp}")
    ...
    # 1. EVALUATE BASELINE (Standard CoREAgent with EXP-95 dt scaling)
    print("\n" + "-"*85)
    print(" >>> RUNNING PHASE 1: BASELINE (CoREAgent + EXP-95 dt scaling) <<<")
    print("-"*85)
    ...
    base_final_fe = sum(base_fes[-20:]) / 20.0
    base_ppl = math.exp(min(base_final_loss, 20.0))
    base_tok_per_sec = (num_eval_steps * b_size * seq_len) / base_duration

    print(f"\n[Baseline Summary] Final Loss: {base_final_loss:.4f} | PPL: {base_ppl:.2f} | FE: {base_final_fe:.4f} | Peak VRAM: {base_vram:.1f} MB | Speed: {base_tok_per_sec:.1f} tok/s")

    # 2. EVALUATE PROPOSED (PWHPC_CoREAgent)
    print("\n" + "-"*85)
    print(" >>> RUNNING PHASE 2: PROPOSED (Precision-Weighted PW-HPC CoREAgent) <<<")
    print("-"*85)
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats()

    agent_prop = PWHPC_CoREAgent(config, device=device_str).to(device)
    ...
    prop_final_fe = sum(prop_fes[-20:]) / 20.0
    prop_ppl = math.exp(min(prop_final_loss, 20.0))
    prop_tok_per_sec = (num_eval_steps * b_size * seq_len) / prop_duration

    print(f"\n[Proposed Summary] Final Loss: {prop_final_loss:.4f} | PPL: {prop_ppl:.2f} | FE: {prop_final_fe:.4f} | Peak VRAM: {prop_vram:.1f} MB | Speed: {prop_tok_per_sec:.1f} tok/s")

    # 3. KEP RULE #6: GRADIENT HEALTH INSPECTION
    print("\n" + "="*85)
    print(" === [KEP RULE #6 GRADIENT FLOW AUDIT (PROPOSED PW-HPC MODEL)] ===")
    print("="*85)
    zero_grads = 0
    healthy_grads = 0
    for name, param in agent_prop.named_parameters():
        ...
        print(f"  {name:<56} | Grad Norm: {g_norm if param.grad is not None else 0.0:<12.6f} | {status}")
    ...
    print(f"Delta Loss         : {delta_loss:+.4f} ({'IMPROVEMENT' if delta_loss >= 0.08 else 'NEUTRAL / INCONCLUSIVE' if abs(delta_loss) < 0.05 else 'REGRESSION'})")
    print(f"VRAM Change        : {vram_diff_pct:+.1f}% ({base_vram:.1f} MB -> {prop_vram:.1f} MB)")
