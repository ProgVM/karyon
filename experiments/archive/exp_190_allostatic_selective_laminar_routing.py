# experiments/exp_190_allostatic_selective_laminar_routing.py
"""
===============================================================================
EXP-190: Allostatic Precision-Modulated Laminar Error Routing
Grounding: KEP Principle 1 (LibTorch/GPU Matrix Ops), Principle 2 (Biophysical Realism),
           Principle 8 (Compositional Depth), Principle 14 (Allostatic Forces: No Static Constants).
===============================================================================
Hypothesis:
Replacing static linear error residual projections between Cortical Stage 1 and Stage 2
with allostatically modulated precision gating (where noradrenaline NA_t sharpens error gain
and curiosity_t increases bottom-up novelty transmission) dynamically balances sensory prediction
errors and top-down priors, accelerating loss convergence and reducing Free Energy.
"""
...
logger = logging.getLogger("EXP-190")


class AllostaticLaminarPrecisionRouting(nn.Module):
    """
    Allostatically-modulated precision-weighted laminar error routing between cortical stages.
    """
    def __init__(self, hidden_dim: int, device_str: str = 'cpu'):
        super().__init__()
        self.device = torch.device('cuda' if 'cuda' in device_str else 'cpu')
        self.hidden_dim = hidden_dim
        self.error_proj = nn.Linear(hidden_dim, hidden_dim, bias=False).to(self.device)
        self.precision_gate = nn.Linear(hidden_dim + 6, hidden_dim).to(self.device)
        self.norm = nn.LayerNorm(hidden_dim).to(self.device)

    def forward(self, h_s1: torch.Tensor, h_s2_pred: torch.Tensor, u_t: torch.Tensor) -> torch.Tensor:
        # Prediction error: bottom-up actual (Stage 1) minus top-down prior prediction (Stage 2)
        error = h_s1 - h_s2_pred
        
        # Format homeostatic tensor
        B, S, D = h_s1.shape
        if u_t.dim() == 2:
        ...
        gate_input = torch.cat([error, u_expanded], dim=-1)
        precision = torch.sigmoid(self.precision_gate(gate_input))
        
        # Allostatic gain: Noradrenaline enhances error precision gain
        na_gain = 1.0 + 1.5 * u_expanded[..., 4:5]
        curiosity_gain = 1.0 + 0.8 * u_expanded[..., 0:1]
        
        gated_error = self.error_proj(error * precision * na_gain * curiosity_gain)
        return self.norm(h_s1 + gated_error)
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-190: ALLOSATIC LAMINAR PRECISION ROUTING BENCHMARK]")
    logger.info("=" * 80)
    ...
    brain_prop.allostatic_laminar_router = AllostaticLaminarPrecisionRouting(
        hidden_dim=brain_prop.hidden_dim, device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Allostatic Laminar Routing Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-190 TELEMETRY REPORT ===")
    ...
    with open("experiments/exp_190_results.json", "w") as f:
