...
- Affective Russell Space (Valence, Arousal, Dominance) & Panksepp Drives
...
def main():
    ...
    for turn_idx, user_turn in enumerate(dialogue_script):
        ...
        with torch.no_grad():
            ...
            arousal = float(affective_state.get("arousal", 0.15))
            valence = float(affective_state.get("valence", 0.0))

        logger.info(f"--- Turn {turn_idx + 1}: Human: \"{user_turn if user_turn else '[Spontaneous Turn]'}\" ---")
        logger.info(f"Karyon: \"{speech_out}\"")
        logger.info(f"📊 Telemetry | Latency: {t_lat:.1f}ms | Energy: {energy:.3f} | Health: {health:.3f} | Valence: {valence:+.3f} | Arousal: {arousal:.3f} | Curiosity: {curiosity:.3f}\n")
