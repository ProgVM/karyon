...
class ProposedFastWeightHebbianPlasticity(nn.Module):
    ...
    """
    def __init__(self, hidden_dim: int, key_dim: int = 64, value_dim: int = 64, device_str: str = 'cpu'):
        super().__init__()
...
def main():
    ...
    proposed_hebbian = ProposedFastWeightHebbianPlasticity(
        ...
        key_dim=brain_p.fast_weight_hebbian.key_dim,
        value_dim=brain_p.fast_weight_hebbian.value_dim,
    ...
    logger.info(f"⏱️ Proposed Run Duration         : {p_results['elapsed_sec']:.2f}s ({speedup_pct:+.2f}% speedup)")
    logger.info("=" * 80)
