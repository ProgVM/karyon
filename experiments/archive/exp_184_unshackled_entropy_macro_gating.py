...
"""
EXP-184: Unshackled Entropy Macro Gating & Multi-Layer Boundary Projection
...
2. Bottleneck Elimination (KEP Principle 7): In production EntropyMacroGating (`entropy_macro_gate`),
   ...
   Expanding to a 2-layer MLP [768D -> 256D -> 1D] with SiLU activation preserves representational rank
   and non-linear boundary detection capability.
3. Fully Backward-Compatible API:
   Signature maintains `forward(self, h_s1: torch.Tensor)` for zero-overhead drop-in integration.

Telemetry Captured:
- Pre/Post Speech Cross-Entropy Loss (nats)
- Pre/Post Variational Free Energy (F_t)
- Boundary Gate Dynamics
"""
...
class ProposedEntropyMacroGating(nn.Module):
    """
    Proposed KEP Principle 7 Compliant Entropy Macro Gating.
    2-layer 256D non-linear boundary projection.
    """
    def __init__(self, hidden_dim: int, vocab_size: int = 258, device_str: str = 'cpu'):
        ...
        # Expanded from Linear(hidden_dim, 1) to 2-layer MLP (KEP Principle 7)
...
def main():
    ...
    logger.info("\n--- 2. Evaluating Proposed (Unshackled 256D Boundary Projection) ---")
    ...
    summary = {
        "exp_id": "EXP-184",
        "verdict": verdict,
        "hypothesis": "Unshackling macro boundary projection (Linear -> 256D MLP) preserves non-linear boundary features and improves convergence.",
        "architecture_delta": "Expanded EntropyMacroGating boundary projection from single linear layer to 2-layer 256D SiLU MLP.",
        "metrics": {
