...
def create_synthetic_nlp_stream(num_batches=20, batch_size=2, seq_len=64, device='cuda'):
...
def run_experiment():
    ...
    stream_data = create_synthetic_nlp_stream(num_batches=num_steps, batch_size=2, seq_len=64, device=device_str)
    ...
    moving_fe = float(baseline_fe_list[0].item()) if hasattr(baseline_fe_list[0], 'item') else float(baseline_fe_list[0])
    ...
    for step, (inp, tgt) in enumerate(stream_data):
        ...
        if step % 5 == 0 and step > 0:
            hu_dynamic.state[:, 4] = 0.50 # Noradrenaline spike on surprise
        else:
        ...
        fe_val_float = float(fe_val.item()) if hasattr(fe_val, 'item') else float(fe_val)
        fe_delta = fe_val_float - moving_fe
        moving_fe = 0.85 * moving_fe + 0.15 * fe_val_float
        ...
        if (na_val > 0.35 or fe_delta > 0.05) and len(agent_dynamic.dynamic_graph.bricks) < agent_dynamic.dynamic_graph.max_bricks:
            ...
            if success:
                ...
                # Smoothly add new brick parameters into optimizer
        ...
        # Periodic Sleep Consolidation & Pruning at step 12 (Run AFTER optimizer step)
        if step == 12:
            logger.info("  🌙 [Step 12] Entering Sleep Phase: Pruning inactive operator bricks...")
