# experiments/exp_60_production_2stage_cortical_integration.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-60 (PRODUCTION 2-STAGE CORTICAL INTEGRATION)
Validates the full integration of the 2-Stage Cascaded Cortical Stack,
Theta-Gamma PAC Entropy-Adaptive Decoding, and Natural Global Workspace Gateway
into the master karyon_agent.py production CoREAgent.
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-60 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_60_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-60 (PRODUCTION 2-STAGE CORTICAL INTEGRATION)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing Master CoREAgent with Integrated 2-Stage Cortical Stack (300 Steps)...")
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-60 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            print(f"Metrics Progress          : Speech Loss = {speech_loss:.4f} (Avg: {cur_loss:.4f}, PPL: {cur_ppl:.2f}) | LR = {cur_lr:.6f}")
            print(f"Active Inference (F_t)    : Total F_t = {cur_fe:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-60 FINAL INTEGRATION TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-60 Master CoREAgent Value':<40}")
    print("-" * 95)
