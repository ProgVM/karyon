# experiments/exp_76_topdown_film_cortical_modulation.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-76 (HIERARCHICAL TOP-DOWN FILm CORTICAL MODULATION)
Hypothesis: Higher cortical areas (Stage 2 Slow Semantic Sheet) provide top-down
contextual feedback to lower cortical areas (Stage 1 Fast Morpho-Syntactic Sheet)
via Feature-wise Linear Modulation (FiLM: h_in_mod = LayerNorm(h_in) * (1 + gamma) + beta).
Identity-initialized top-down FiLM modulation allows high-level discourse state to
dynamically adapt byte receptive fields to active semantic goals without gradient interference.
...
# =============================================================================
# 7. HIERARCHICAL TOP-DOWN FILm CORTICAL MODULATION MODULE
# =============================================================================
class TopDownFiLMCorticalModulation(nn.Module):
    """
    Feature-wise Linear Modulation (FiLM) projecting Stage 2 discourse state (h_s2_prev)
    into multiplicative (gamma) and additive (beta) modulation parameters for Stage 1:
    h_in_mod = LayerNorm(h_in) * (1.0 + gamma_td) + beta_td
    Identity-initialized (gamma=0, beta=0) to prevent gradient disruption.
    """
    def __init__(self, hidden_dim: int = 768):
        super().__init__()
        self.hidden_dim = hidden_dim
        ...
        nn.init.zeros_(self.film_net[2].weight)
        nn.init.zeros_(self.film_net[2].bias)
        self.norm = nn.LayerNorm(hidden_dim)

    def forward(self, h_in: torch.Tensor, h_s2_prev_last: torch.Tensor) -> torch.Tensor:
        # h_in: [B, L, D]
        # h_s2_prev_last: [B, 1, D]
        film_params = self.film_net(h_s2_prev_last) # [B, 1, D*2]
        gamma_td, beta_td = film_params.chunk(2, dim=-1) # [B, 1, D], [B, 1, D]
        
        norm_in = self.norm(h_in)
        h_in_mod = norm_in * (1.0 + gamma_td) + beta_td
        return h_in_mod


# =============================================================================
# 8. EXP-76 TOP-DOWN FILm CORTICAL AGENT
# =============================================================================
class Exp76TopDownFiLMAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # Top-Down FiLM Cortical Modulation Module
        self.topdown_film = TopDownFiLMCorticalModulation(hidden_dim=self.hidden_dim).to(self.device)
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        h2_prev_last = torch.zeros(batch_size, 1, self.hidden_dim, device=self.device)
        ...
        for chunk_idx in range(num_chunks):
            ...
            # --- 1. HIERARCHICAL TOP-DOWN FILm MODULATION ---
            h_in_mod = self.topdown_film(h_in, h2_prev_last)

            # Stage 1: Fast Morpho-Syntactic Pass
            h_s1, m_s1, dt1 = self.stage1(h_in_mod, m_s1, curr_u_t, dt=1.0)
            ...
            # --- 2. PRECISION-WEIGHTED LPER ROUTING ---
    ...
    def generate_natural_speech(self, prompt: str, hu_state, max_tokens: int = 75) -> str:
        ...
        h2_prev_last = torch.zeros(1, 1, self.hidden_dim, device=self.device)
        ...
        for c_idx in range(0, prompt_len, 64):
            ...
            h_s1, m_s1, _ = self.stage1(h_in_mod, m_s1, hu_state, dt=1.0)
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-76 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_76_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-76 (TOP-DOWN FILm CORTICAL MODULATION)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-76 Top-Down FiLM Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp76TopDownFiLMAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            g_film = 0.0
            for p in agent.topdown_film.parameters():
                if p.grad is not None:
                    g_film = max(g_film, p.grad.norm().item())
            g_s1 = 0.0
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-76 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | TopDown FiLM: {g_film:.4f} | S1: {g_s1:.4f} | S2: {g_s2:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-76 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-76 Top-Down FiLM Value':<40}")
    print("-" * 95)
