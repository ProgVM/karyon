# experiments/exp_165_normalized_allostatic_precision.py
"""
EXP-165: Normalized Allostatic Precision Weighting & Hierarchical Predictive Coding

Hypothesis:
Unit-normalizing the neuromodulatory precision gain:
   Pi_raw(u_t) = sigmoid(W_prec * h_s2 + V_homeo * u_t)
   gain = (1.0 + 2.5 * NA_t) * (1.0 + 1.5 * DA_t) * clamp(1.2 * Energy_t, 0.2, 1.0)
   gain_normalized = gain / (mean(gain) + 1e-6)
   Pi(u_t) = Pi_raw(u_t) * gain_normalized
will prevent gradient scale distortion, stabilize hierarchical prediction error (error_magnitude),
and outperform the unmodulated baseline by reducing Free Energy (F_t) and speech cross-entropy loss.
...
logger = logging.getLogger("EXP-165")
...
class NormalizedAllostaticPredictiveResidualRouting(nn.Module):
    """
    Proposed KEP Principle 14 Compliant Normalized Precision Router.
    """
    def __init__(self, hidden_dim: int, homeo_dim: int = 6, device_str: str = 'cpu'):
        super().__init__()
        self.device = torch.device('cuda' if 'cuda' in device_str else 'cpu')
    ...
    def forward(self, h_s1: torch.Tensor, h_s2: torch.Tensor, u_t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # Dynamic Neuromodulated Precision with Unit-Mean Normalization (KEP Principle 14)
        base_precision = torch.sigmoid(self.precision_gate(h_s2) + u_t_proj)
        
        gain = (1.0 + 2.5 * na_t) * (1.0 + 1.5 * da_t) * torch.clamp(1.2 * energy_t, min=0.2, max=1.0)
        gain_normalized = gain / (gain.mean(dim=-1, keepdim=True) + 1e-6)
        
        precision = base_precision * gain_normalized
        weighted_error = precision * error_s1
        error_magnitude = torch.mean(weighted_error ** 2)
        return weighted_error, error_magnitude
...
def main():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-165: NORMALIZED ALLOSTATIC PRECISION WEIGHTING BENCHMARK]")
    logger.info("=" * 80)
    ...
    logger.info("\n--- Evaluating Proposed (Normalized Allostatic Precision Router) ---")
    
    proposed_router = NormalizedAllostaticPredictiveResidualRouting(
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-165 EMPIRICAL TELEMETRY COMPARISON ===")
    ...
    with open("experiments/exp_165_results.json", "w") as f:
        json.dump(summary, f, indent=2)

    logger.info("✅ EXP-165 execution complete. Results saved to experiments/exp_165_results.json.")
