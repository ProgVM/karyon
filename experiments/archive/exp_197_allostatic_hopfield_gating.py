# experiments/exp_197_allostatic_hopfield_gating.py
"""
===============================================================================
EXP-197: Allostatically-Gated Modern Hopfield Attractor Relaxation
Grounding: KEP Principle 2 (Biological Realism), Principle 14 (Allostatic Forces: No Static Constants).
===============================================================================
Hypothesis:
Replacing the static softmax-based Hopfield attractor relaxation with an allostatically-gated
relaxation mechanism—where the attractor activation and relaxation dynamics are dynamically
...
logger = logging.getLogger("EXP-197")


class AllostaticHopfieldGating(nn.Module):
    """
    Allostatically-gated Modern Hopfield Attractor relaxation.
    """
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-197: ALLOSTATIC HOPFIELD GATING BENCHMARK]")
    logger.info("=" * 80)
    ...
    # 1. Baseline Evaluation (EXP-196 Motor Gateway)
    ...
    # 2. Proposed Evaluation (EXP-197 Allostatic Hopfield Gating)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain
    brain_prop.allostatic_hopfield_gating = AllostaticHopfieldGating(
        hidden_dim=brain_prop.hidden_dim, device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Allostatic Hopfield Gating Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-197 TELEMETRY REPORT ===")
    ...
    with open("experiments/exp_197_results.json", "w") as f:
