...
class ProposedDynamicHopfieldAttractorHead(nn.Module):
    ...
    def compute_pattern_separation_loss(self) -> torch.Tensor:
        ...
        cosine_matrix = torch.matmul(norm_basins, norm_basins.t())
        eye = torch.eye(self.num_attractors, device=self.device)
...
def main():
    ...
    base_params = dict(brain_p.attractor_head.named_parameters())
    with torch.no_grad():
        proposed_attractor.attractor_basins.copy_(base_params['attractor_basins'])
        proposed_attractor.norm.weight.copy_(base_params['norm.weight'])
        proposed_attractor.norm.bias.copy_(base_params['norm.bias'])
