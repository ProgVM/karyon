# experiments/exp_52_balanced_long_horizon_ssd.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-52 (BALANCED LONG-HORIZON SSD)
True Mamba-2 EMA-Normalized State Tracking (O(1) Bounded State) + FP32 MatMul +
GroupNorm Head Equalization + Linguistic Decay Spectrum (alpha in [0.92, 0.9995]).
...
# =============================================================================
# 1. BALANCED MAMBA-2 SSD CORE (O(1) BOUNDED EMA + GROUPNORM EQUALIZATION)
# =============================================================================
class BalancedMamba2SSDCore(nn.Module):
    """
    Balanced Mamba-2 SSD Core:
    - O(1) Bounded EMA State Tracking: beta = (1 - alpha) guarantees no blowup.
    - GroupNorm Head Equalization: Head 7 (long memory) and Head 0 have equal variance 1.0.
    - FP32 State MatMul: Prevents any FP16 overflow (>65504).
    """
    def __init__(self, text_dim: int = 256, unified_dim: int = 256, hidden_dim: int = 512,
        ...
        # GroupNorm equalizes fast and slow memory heads to unit variance (sigma=1.0)
    ...
    def forward_chunk_parallel_ssd(self, chunk_emb: torch.Tensor, m_prev: torch.Tensor, u_t: torch.Tensor, dt: float = 1.0):
        ...
        # 3. Head GroupNorm equalizes variances across heads
        y_normed = self.head_norm(y_total)
        h_chunk = self.norm(self.out_proj(y_normed))

        # 4. State accumulation with EMA beta scaling: bounds state to O(1)
...
# =============================================================================
class EXP52Agent(nn.Module):
    def __init__(self, use_mamba2_balanced: bool = True, device_str: str = 'cpu'):
        super().__init__()
...
def prepare_packed_batches(num_batches: int = 150, batch_size: int = 32, seq_len: int = 512):
    logger.info("Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-52...")
...
# =============================================================================
# 4. RUN EXP-52 PARITY BENCHMARK
# =============================================================================
def run_exp_52_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-52 (BALANCED LONG-HORIZON SSD)] ===")
    print("="*85)
    ...
    # -------------------------------------------------------------------------
    # 1. BASELINE: OLD UN-EQUALIZED DAMPED SSD
    # -------------------------------------------------------------------------
    print("\n[1/2] Benchmarking Model A: Baseline Old Un-equalized SSD...")
    torch.manual_seed(42)
    model_a = EXP52Agent(use_mamba2_balanced=False, device_str=device_str).to(device)
    ...
    # -------------------------------------------------------------------------
    # 2. PROPOSED: EXP-52 BALANCED MAMBA-2 SSD
    # -------------------------------------------------------------------------
    print("\n[2/2] Benchmarking Model B: EXP-52 Balanced Mamba-2 SSD (EMA + GroupNorm)...")
    torch.manual_seed(42)
    model_b = EXP52Agent(use_mamba2_balanced=True, device_str=device_str).to(device)
    ...
    print("="*85)
    print(f"{'Performance Metric':<32} | {'Model A (Old SSD)':<22} | {'Model B (EXP-52 Balanced)':<22} | {'Delta':<12}")
    print("-" * 85)
