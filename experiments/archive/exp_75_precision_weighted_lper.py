# experiments/exp_75_precision_weighted_lper.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-75 (PRECISION-WEIGHTED LAMINAR ERROR ROUTING)
Tests Karl Friston's Active Inference Precision-Weighted Laminar Error Routing (PW-LPER):
  h1_pred = topdown_pred_net(h1_shifted)
  e1_raw = h1 - h1_pred
  Pi_t = 2.0 * Sigmoid(Precision_Net([h1, h1_pred, NA_level]))
  e1_weighted = Pi_t * e1_raw
  h2, m_s2 = Stage2(e1_weighted, m_s2)
By modulating ascending prediction errors via state-dependent & noradrenergic precision,
routine intra-morpheme noise is suppressed while novel boundary signals are amplified.
...
class PrecisionWeightedLPER(nn.Module):
    """
    Friston Active Inference Precision-Weighted Laminar Error Routing (PW-LPER):
    ...
    def forward(self, h_s1: torch.Tensor, h1_prev_last: torch.Tensor, u_t: torch.Tensor) -> tuple:
        ...
        na_level = u_t[:, 4:5].unsqueeze(1).expand(-1, chunk_len, -1) # [B, L, 1]
        prec_in = torch.cat([h_s1, h1_pred, na_level], dim=-1)
        pi_t = 2.0 * self.precision_net(prec_in) # [B, L, D] in range [0, 2.0]
...
# =============================================================================
# 7. EXP-75 PRECISION-WEIGHTED LAMINAR AGENT
# =============================================================================
class Exp75PrecisionWeightedAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # Stage 1: Fast Morpho-Syntactic Sheet (ConvSwiGLU K=3)
        ...
        # Stage 2: Slow Semantic Sheet (ConvSwiGLU K=7)
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        for chunk_idx in range(num_chunks):
            ...
            # Stage 2: Slow Semantic Pass on Precision-Weighted Error e1_weighted
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-75 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_75_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-75 (PRECISION-WEIGHTED LAMINAR ERROR ROUTING)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-75 Precision-Weighted Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp75PrecisionWeightedAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-75 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-75 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-75 Precision-Weighted LPER Value':<40}")
    print("-" * 95)
