# experiments/exp_event_gated_learning_v2.py
"""
===============================================================================
KARYON ENGINEERING PROTOCOL (KEP) EXPERIMENT #2.1 (ITERATION 2)
Topic: Statistical Dynamic Free Energy Thresholding (DFET v2: Mean + Sigma Gating)
Author: Karyon-CoRE Open-Source Research Team (2026)
===============================================================================

Hypothesis:
    Gating gradient backpropagation updates using Statistical Variance 
    (F_t > Mean + k * Sigma) combined with Metabolic Somatic Recovery will 
    achieve >50-70% FLOPs reduction during online streaming (N=1) while 
    maintaining high somatic energy reserves and <10% prediction error increase.

Control Group: 
    Traditional Continuous Online Learning (Unconditional Backprop at EVERY step).

Experimental Group: 
    Statistical Event-Gated Plasticity (Backprop triggered strictly on outlier surprise).

Metrics Tracked:
    1. Mean Variational Free Energy (F_t)
    2. Total Gradient Backprop Steps (FLOPs Proxy)
    3. FLOPs Reduction Ratio (%)
    4. Final Somatic Energy Level (Metabolic Conservation)
...
print(f"[KEP Experiment #2.1] Execution Context: {device.type.upper()}")


# =============================================================================
# 1. STANDALONE PROTOTYPE MODULES (With Somatic Metabolic Recovery)
# =============================================================================

class SyntheticSomaticUnitV2:
    """
    Homeostatic Somatic Controller with Metabolic Recovery.
    ...
    def update(self, free_energy: float, was_adapted: bool):
        ...
        if was_adapted:
            # Backpropagation consumes active energy
            energy = max(0.0, energy - 0.006)
        else:
            # Passive metabolic recovery when predicting smoothly without Backprop
            energy = min(1.0, energy + 0.0012)

        # Noradrenaline (Arousal) tracks unexpected surprise
        na = min(1.0, max(0.0, 0.6 * free_energy + 0.4 * na))

        # Dopamine signals efficient homeostatic balance
...
def generate_streaming_data(seq_len=800, vocab_size=258):
    ...
    for i in range(seq_len):
        if i % 100 > 85:
            # High-entropy novelty burst
            stream.append(torch.randint(0, 255, (1,)).item())
        else:
            # Structured repeating byte sequence
...
# =============================================================================
# 3. EXPERIMENTAL BENCHMARK ENGINE (DFET v2 Statistical Variance Gating)
# =============================================================================

def run_experiment_session_v2(mode="control", data_stream=None, seq_len=800):
    """Runs single-pass continuous learning with Statistical Variance Gating."""
    torch.set_grad_enabled(True)
    ...
    somatic_unit = SyntheticSomaticUnitV2(device=device)
    ...
    # Statistical Variance Tracking (Moving Mean & Moving Variance)
    moving_mean_fe = 0.15
    moving_var_fe = 0.01
    alpha_ma = 0.05
    ...
    for step in range(seq_len - 1):
        ...
        # Update statistical variance parameters
        ...
        # 2. Statistical Plasticity Gating Decision
        was_adapted = False

        if mode == "control":
            should_backprop = True
        else:
            # DFET v2: Trigger Backprop ONLY when Free Energy is an outlier (F_t > Mean + k * Std)
            # Higher Noradrenaline (Arousal) slightly lowers the k-multiplier threshold
            k_sigma = max(0.3, 0.75 - 0.35 * na_level)
        ...
        # 4. Somatic Physiology Update (With Energy Recovery)
...
if __name__ == "__main__":
    ...
    control_results = run_experiment_session_v2(mode="control", data_stream=stream_data, seq_len=STREAM_LENGTH)

    print("[KEP Step 2/2] Running EXPERIMENTAL Group (DFET v2: Statistical Mean + Sigma Gating)...")
    experimental_results = run_experiment_session_v2(mode="event_gated", data_stream=stream_data, seq_len=STREAM_LENGTH)
    ...
    if flops_saved_pct >= 50.0 and fe_change_pct <= 10.0:
        ...
        print("   Action: Merge Statistical DFET v2 into production runtime!")
