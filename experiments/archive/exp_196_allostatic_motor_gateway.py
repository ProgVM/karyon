# experiments/exp_196_allostatic_motor_gateway.py
"""
===============================================================================
EXP-196: Allostatically-Modulated Multimodal Motor Gateway & Dynamic Routing
Grounding: KEP Principle 2 (Biological Realism), Principle 14 (Allostatic Forces: No Static Constants).
===============================================================================
Hypothesis:
Replacing the static or partially linear multimodal motor gateway projections with an
allostatically-modulated dynamic motor gateway—where the projection weights and channel
routing gains are dynamically scaled by a non-linear function of Somatic Energy (u_t[1]),
Noradrenaline (u_t[4]), and Variational Free Energy (F_t)—will optimize multimodal motor integration,
prevent motor overload, and accelerate loss convergence.

Refined Approach (Identity-Preserving Residual Routing):
We use an identity-preserving residual routing formulation:
...
logger = logging.getLogger("EXP-196")


class AllostaticMotorGateway(nn.Module):
    """
    Allostatically-modulated multimodal motor gateway and dynamic routing.
    """
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-196: ALLOSTATIC MOTOR GATEWAY BENCHMARK]")
    logger.info("=" * 80)
    ...
    # 1. Baseline Evaluation (EXP-195 Multimodal Gateway)
    ...
    # 2. Proposed Evaluation (EXP-196 Allostatic Motor Gateway)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain
    brain_prop.allostatic_motor_gateway = AllostaticMotorGateway(
        hidden_dim=brain_prop.hidden_dim, device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Allostatic Motor Gateway Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-196 TELEMETRY REPORT ===")
    ...
    with open("experiments/exp_196_results.json", "w") as f:
