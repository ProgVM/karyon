# experiments/exp_70_multiscale_byte_pyramid_rf.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-70 (MULTI-SCALE MORPHOLOGICAL BYTE PYRAMID RF)
Hypothesis: In raw UTF-8 byte space (V=258), linguistic and semantic patterns 
span multiple scales: single bytes (K=2), sub-word morphemes (K=4), and full word 
stems (K=8). Replacing the single K=4 Conv1d with a Multi-Scale Causal Depthwise 
Pyramid with parallel branches (K=2, 4, 8) and dynamic softmax channel gating will 
enrich the morphological representations fed into the 2-Stage Cortical SSD Core, 
breaking below 1.70 nats/byte.
...
# =============================================================================
# 0. MULTI-SCALE MORPHOLOGICAL BYTE PYRAMID RECEPTIVE FIELD (K=2, 4, 8)
# =============================================================================
class MultiScaleBytePyramidReceptiveField(nn.Module):
    ...
    Merged via dynamic softmax scale gating.
    """
    def __init__(self, text_dim: int = 256):
        ...
        # Scale Selection Gate: [B, L, text_dim] -> [B, L, 3]
    ...
    def forward(self, x_seq: torch.Tensor) -> torch.Tensor:
        ...
        x_trans = x_seq.transpose(1, 2) # [B, D, L]

        # Branch 1: K=2 with causal padding (padding=1 on left)
        x_pad_k2 = F.pad(x_trans, (1, 0))
        out_k2 = F.silu(self.conv_k2(x_pad_k2)).transpose(1, 2) # [B, L, D]

        # Branch 2: K=4 with causal padding (padding=3 on left)
        x_pad_k4 = F.pad(x_trans, (3, 0))
        out_k4 = F.silu(self.conv_k4(x_pad_k4)).transpose(1, 2) # [B, L, D]

        # Branch 3: K=8 with causal padding (padding=7 on left)
        x_pad_k8 = F.pad(x_trans, (7, 0))
        out_k8 = F.silu(self.conv_k8(x_pad_k8)).transpose(1, 2) # [B, L, D]

        # Dynamic Scale Gating
        gates = F.softmax(self.scale_gate(x_seq), dim=-1) # [B, L, 3]
        g2 = gates[..., 0:1]
        g4 = gates[..., 1:2]
        g8 = gates[..., 2:3]
...
# =============================================================================
# 5. EXP-70 MASTER AGENT WITH MULTI-SCALE BYTE PYRAMID
# =============================================================================
class Exp70MultiScalePyramidAgent(nn.Module):
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-70 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_70_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-70 (MULTI-SCALE MORPHOLOGICAL BYTE PYRAMID RF)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-70 Multi-Scale Pyramid Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp70MultiScalePyramidAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-70 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | Pyramid RF: {g_rf:.4f} | S1: {g_s1:.4f} | S2: {g_s2:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-70 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-70 Multi-Scale Pyramid Value':<40}")
    print("-" * 95)
