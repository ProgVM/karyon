# experiments/exp_191_allostatic_predictive_residual_routing.py
"""
===============================================================================
EXP-191: Bidirectional Allostatic Predictive Residual Routing & Error Minimization
...
===============================================================================
Hypothesis:
Extending the allostatic precision-modulated laminar error routing (EXP-190) into a
fully bidirectional predictive residual routing mechanism—where Cortical Stage 2 projects
top-down prior predictions back to Stage 1 to compute precision-weighted prediction errors,
and only the unresolved residuals are routed forward—will significantly reduce Variational
Free Energy (F_t) and accelerate loss convergence while maintaining high GPU throughput.
"""
...
logger = logging.getLogger("EXP-191")


class BidirectionalAllostaticPredictiveRouter(nn.Module):
    """
    Bidirectional predictive residual routing with allostatic precision weighting.
    Stage 2 predicts Stage 1 activations; Stage 1 computes prediction error;
    unresolved residuals are precision-weighted and routed forward to Stage 2.
    """
    def __init__(self, hidden_dim: int, device_str: str = 'cpu'):
        ...
        # Bottom-up residual projection
        self.residual_proj = nn.Linear(hidden_dim, hidden_dim, bias=False).to(self.device)
        
        # Allostatic precision estimator
        self.precision_gate = nn.Linear(hidden_dim * 2 + 6, hidden_dim).to(self.device)
    ...
    def forward(self, h_s1: torch.Tensor, h_s2: torch.Tensor, u_t: torch.Tensor) -> torch.Tensor:
        ...
        # 1. Top-down prediction of Stage 1 activations
        h_s1_pred = self.topdown_predictor(h_s2)
        
        # 2. Compute raw prediction error
        error = h_s1 - h_s1_pred
        
        # 3. Format homeostatic allostatic tensor
        if u_t.dim() == 2:
            u_expanded = u_t.unsqueeze(1).expand(B, S, -1)
        else:
            u_expanded = u_t.view(1, 1, -1).expand(B, S, -1)
            
        gate_input = torch.cat([h_s1, h_s1_pred, u_expanded], dim=-1)
        precision = torch.sigmoid(self.precision_gate(gate_input))
        
        # Noradrenaline enhances error precision gain; Curiosity boosts novelty transmission
        na_gain = 1.0 + 1.5 * u_expanded[..., 4:5]
        curiosity_gain = 1.0 + 0.8 * u_expanded[..., 0:1]
        
        # 4. Precision-weighted residual routing
        weighted_residual = self.residual_proj(error * precision * na_gain * curiosity_gain)
        return self.norm(h_s2 + weighted_residual)
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-191: BIDIRECTIONAL ALLOSATIC PREDICTIVE ROUTING BENCHMARK]")
    logger.info("=" * 80)
    ...
    # 1. Baseline Evaluation (EXP-190 Allostatic Precision Routing)
    ...
    # 2. Proposed Evaluation (EXP-191 Bidirectional Predictive Routing)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain
    brain_prop.bidirectional_allostatic_router = BidirectionalAllostaticPredictiveRouter(
        hidden_dim=brain_prop.hidden_dim, device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Bidirectional Predictive Routing Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-191 TELEMETRY REPORT ===")
    ...
    # POSITIVE if loss converges faster or throughput is maintained with lower Free Energy
    ...
    with open("experiments/exp_191_results.json", "w") as f:
