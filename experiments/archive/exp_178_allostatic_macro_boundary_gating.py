...
class ProposedEntropyMacroGating(nn.Module):
    ...
    def forward(self, h_s1: torch.Tensor, u_t: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if u_t is not None and u_t.dim() == 2 and u_t.size(0) > 0:
            curiosity_t = u_t[:, 0:1] # [B, 1]
            stability_t = u_t[:, 2:3] # [B, 1]
            na_t = u_t[:, 4:5]        # [B, 1]
        else:
        ...
        boundary_logits = self.macro_boundary_proj(h_s1).squeeze(-1) # [B, S] or [B]
        boundary_gate = torch.sigmoid(boundary_logits + beta_boundary * (entropy - h_thresh))
