...
class CombinatorialBiophysicalAgent(nn.Module):
    def __init__(self, vocab_size=258, dim=256, use_a=False, use_b=False, use_c=False, device="cuda"):
        ...
        self.node_fast = karyon_core.OmniMorphicNode(dim, 128, 8, device)
        ...
        self.node_slow = karyon_core.OmniMorphicNode(dim, 128, 8, device)
    ...
    def forward(self, x):
        ...
        # Dummy homeostatic vector for node operators
        u_t = torch.zeros(batch, 6, device=self.device)

        # Stage 1: Fast dynamics via native node
        # node expects signal_manifold: [batch, num_signals, seq_len, dim]
        m1 = h0.unsqueeze(1)
        h1, _ = self.node_fast(m1, u_t)
        ...
        if self.use_b:
            ...
            # Route prediction error into Stage 2 manifold
            m2 = torch.stack([h1, residual_error], dim=1)
        else:
            m2 = h1.unsqueeze(1)

        # Stage 2: Slow contextual dynamics via native node
        h2, _ = self.node_slow(m2, u_t)
