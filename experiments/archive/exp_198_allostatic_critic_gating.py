# experiments/exp_198_allostatic_critic_gating.py
"""
===============================================================================
EXP-198: Allostatically-Gated Temporal-Difference Free Energy Critic
Grounding: KEP Principle 2 (Biological Realism), Principle 14 (Allostatic Forces: No Static Constants).
===============================================================================
Hypothesis:
Replacing the static linear or partially non-linear TD Free Energy Critic projections with an
allostatically-gated critic mechanism—where the value estimation and temporal-difference
...
logger = logging.getLogger("EXP-198")


class AllostaticCriticGating(nn.Module):
    """
    Allostatically-gated TD Free Energy Critic.
    """
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-198: ALLOSTATIC CRITIC GATING BENCHMARK]")
    logger.info("=" * 80)
    ...
    # 1. Baseline Evaluation (EXP-197 Hopfield Gating)
    ...
    # 2. Proposed Evaluation (EXP-198 Allostatic Critic Gating)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain
    brain_prop.allostatic_critic_gating = AllostaticCriticGating(
        hidden_dim=brain_prop.hidden_dim, device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Allostatic Critic Gating Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-198 TELEMETRY REPORT ===")
    ...
    with open("experiments/exp_198_results.json", "w") as f:
