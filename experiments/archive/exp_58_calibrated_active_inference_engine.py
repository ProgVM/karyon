# experiments/exp_58_calibrated_active_inference_engine.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-58 (CALIBRATED ACTIVE INFERENCE ENGINE)
Stable D_KL Bounds (eps=0.01) + Calibrated Loss0=5.55 + Printable Speech Filter +
2-Stage Compositional Cortical Stack on Real Alpaca-GPT4 Stream (300 Steps).
...
# =============================================================================
# 3. STABLE ACTIVE INFERENCE LATENT WORLD MODEL
# =============================================================================
class StableLatentPredictor(nn.Module):
    """
    Active Inference Latent World Model with bounded KL-divergence.
    Guarantees stable Free Energy F_t without numerical blowup.
    """
    ...
    def forward(self, h_fast_prev, h_slow_curr, w_t):
        ...
        # Normalized Gaussian KL Divergence
...
# =============================================================================
# 4. CRISP CONTINUOUS HOPFIELD ATTRACTOR HEAD
# =============================================================================
...
# =============================================================================
# 5. EXP-58 CALIBRATED ACTIVE INFERENCE AGENT
# =============================================================================
class CalibratedActiveAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # Standard Xavier Normal Initialization for stable loss0 = 5.55
        ...
        # Active Inference Latent World Model
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        for chunk_idx in range(num_chunks):
            ...
            w_slice = chunk_emb[:, -1, :]
            h_last = h_s2[:, -1, :]
            w_pred, kl_div, fe, _ = self.world_model(h_prev, h_last, w_slice)
    ...
    def generate_speech(self, prompt: str, max_tokens: int = 70) -> str:
        ...
        with torch.no_grad():
            for step in range(max_tokens):
                ...
                # Mask non-printable control tokens and PAD
                raw_logits[:, 256] = -1e9
                raw_logits[:, :9] = -1e9
...
# =============================================================================
# 6. REAL DATASET & RUNNER (WSD SCHEDULE)
# =============================================================================
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-58 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_58_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-58 (CALIBRATED ACTIVE INFERENCE ENGINE)] ===")
    print("="*85)
    ...
    print("\n[1/1] Training Calibrated Active Inference Engine (300 Steps on S=1024)...")
    torch.manual_seed(42)
    agent = CalibratedActiveAgent(config, device_str=device_str).to(device)
    ...
    print("\n" + "="*85)
    print(" === [KEP EXP-58 CALIBRATED ENGINE TELEMETRY DASHBOARD] ===")
    print("="*85)
    print(f"{'Performance Metric':<36} | {'EXP-58 Calibrated Engine Value':<40}")
    print("-" * 85)
