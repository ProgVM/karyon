    ...
    for step in range(max_steps):
        ...
        if mode == "system2_episodic":
            ...
            for act_cand in range(3):
                ...
                # Query episodic memory using memory.read method
                q_key = w_sim.clone()
                retrieved_val, retrieved_sim = memory.read(q_key, temperature=0.05, threshold=0.5, sigmoid_beta=15.0)
                
                # If similarity is high to a remembered hazard hit, apply heavy epistemic penalty
                max_sim = retrieved_sim.mean().item()
                if max_sim > 0.85:
        ...
        if info == "Food Consumed" or info == "Hazard Hit":
            # Value must match key shape (256D) for memory.write
            v_t = torch.zeros_like(w_t)
            v_t[0, :6] = hu.state
            memory.write(w_t, v_t)
