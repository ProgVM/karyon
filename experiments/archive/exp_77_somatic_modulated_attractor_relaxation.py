# experiments/exp_77_somatic_modulated_attractor_relaxation.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-77 (SOMATIC-MODULATED ATTRACTOR RELAXATION)
Hypothesis: Snapping continuous neural trajectories into discrete attractor basins
should be dynamically modulated by somatic state (Dopamine and Noradrenaline):
  eta = 0.10 + 0.30 * DA - 0.15 * NA (Relaxation Force)
High dopamine (high confidence) snaps states firmly into basins, while high
noradrenaline (high surprise) relaxes the force to allow flexible exploration.
...
# =============================================================================
# 7. SOMATIC-MODULATED ATTRACTOR RELAXATION HEAD (SMAR)
# =============================================================================
class SomaticModulatedHopfieldHead(nn.Module):
    """
    Continuous Hopfield Attractor Head with Somatic-Modulated Attractor Relaxation (SMAR):
    - Softmax temperature (beta) scaled by Dopamine (DA): beta = 1.0 + 1.5 * DA
    - Relaxation force (eta) modulated by DA and Noradrenaline (NA):
      eta = 0.10 + 0.30 * DA - 0.15 * NA
    High DA snaps states firmly into basins, while high NA relaxes the force to allow exploration.
    """
    def __init__(self, hidden_dim: int = 768, num_attractors: int = 256, device_str: str = 'cpu'):
        super().__init__()
        self.hidden_dim = hidden_dim
    ...
    def relax_to_minima(self, h_state: torch.Tensor, u_t: torch.Tensor) -> tuple:
        batch_size = h_state.size(0)
        
        # Extract Dopamine (DA) and Noradrenaline (NA)
        da_val = u_t[:, 5:6] # [B, 1]
        na_val = u_t[:, 4:5] # [B, 1]
        
        # Match batch size if h_state is flattened over sequence length
        if h_state.size(0) != u_t.size(0):
            seq_len = h_state.size(0) // u_t.size(0)
            da_val = da_val.unsqueeze(1).expand(-1, seq_len, -1).reshape(h_state.size(0), -1)
            na_val = na_val.unsqueeze(1).expand(-1, seq_len, -1).reshape(h_state.size(0), -1)

        # Dynamic beta (softmax temperature scaling)
        beta = 1.0 + 1.5 * da_val # [B, 1]
        
        # Dynamic relaxation force eta
        eta = 0.10 + 0.30 * da_val - 0.15 * na_val # [B, 1]
        eta = torch.clamp(eta, 0.05, 0.50)
        ...
        # Apply modulated relaxation force
        h_relaxed = self.norm(h_state + eta * attractor_shift)
...
# =============================================================================
# 8. EXP-77 SOMATIC-MODULATED ATTRACTOR AGENT
# =============================================================================
class Exp77SomaticModulatedAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        # Somatic-Modulated Attractor Relaxation Head (SMAR)
        self.attractor_head = SomaticModulatedHopfieldHead(
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        for chunk_idx in range(num_chunks):
            ...
            # Somatic-Modulated Attractor Relaxation (SMAR)
        ...
        return total_loss, avg_loss.item(), m_s2, h_s2[:, -1, :], curr_u_t, avg_eff_dt, avg_pi_val
    ...
    def generate_natural_speech(self, prompt: str, hu_state, max_tokens: int = 75) -> str:
        ...
        for c_idx in range(0, prompt_len, 64):
            ...
            h_s2, m_s2, _ = self.stage2(e1_weighted, m_s2, hu_state, saliency_gate=sal_gate, dt=1.0)
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-77 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_77_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-77 (SOMATIC-MODULATED ATTRACTOR RELAXATION)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-77 Somatic-Modulated Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp77SomaticModulatedAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        with torch.amp.autocast(device_type=device_str, dtype=torch.float16, enabled=use_amp):
            tot_loss, speech_loss, m_curr, h_proxy, curr_u_t, eff_dt, avg_pi = agent.forward_sequence(
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            g_hop = 0.0
            for p in agent.attractor_head.parameters():
                if p.grad is not None:
                    g_hop = max(g_hop, p.grad.norm().item())
            g_s1 = 0.0
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-77 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            print(f"Metrics Progress          : Speech Loss = {speech_loss:.4f} (Avg: {cur_loss:.4f}, PPL: {cur_ppl:.2f}) | LR = {cur_lr:.6f}")
            print(f"Precision Dynamics (Pi_t) : Mean Ascending Error Precision = {avg_pi:.4f}")
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | Attractor Head: {g_hop:.4f} | S1: {g_s1:.4f} | S2: {g_s2:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-77 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-77 Somatic-Modulated Attractor Value':<40}")
    print("-" * 95)
