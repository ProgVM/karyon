"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-38 (WIDE MULTI-SCALE SSD + RESPONSE FOCUS)
Evaluation of 12-Head Multi-Timescale SSD (D=768, SwiGLU 2048, alpha in [0.65, 0.9995])
+ Response-Weighted Loss vs Baseline 8-Head SSD (D=512) on vicgalle/alpaca-gpt4.
...
print(f"\n[EXP-38] Initializing Wide Multi-Scale SSD + Response Focus Benchmark on: {device_str.upper()}")
...
# =============================================================================
# 4. CUSTOMIZABLE PARALLEL SSD CORE (SUPPORTING H=8 OR H=12 HEADS)
# =============================================================================
class ConfigurableParallelSSDCore(nn.Module):
    def __init__(self, text_dim=128, unified_dim=256, num_heads=12, head_k=32, head_v=64, 
                 beta_min=0.0005, beta_max=0.35, device_str='cpu'):
        super().__init__()
        self.text_dim = text_dim
        ...
        # Geometric Multi-Timescale Spectrum
        betas = torch.exp(torch.linspace(math.log(beta_max), math.log(beta_min), num_heads))
        alphas = 1.0 - betas
...
# =============================================================================
# 5. EXP-38 ARCHITECTURES: BASELINE (H=8, D=512) VS WIDE PROPOSED (H=12, D=768)
# =============================================================================
class ConfigurableAgent(nn.Module):
    def __init__(self, num_heads=8, head_v=64, expand_dim=1536, beta_min=0.001, beta_max=0.30, device_str='cpu'):
        super().__init__()
        ...
        self.ssd_core = ConfigurableParallelSSDCore(
            text_dim=self.text_dim, unified_dim=self.unified_dim, num_heads=self.num_heads,
            head_k=self.head_k, head_v=self.head_v, beta_min=beta_min, beta_max=beta_max, device_str=device_str
        ).to(self.device)
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, 
                         token_weights: torch.Tensor = None, chunk_size: int = 64, optimizer: torch.optim.Optimizer = None):
        ...
        for chunk_idx in range(num_chunks):
            ...
            # Compute Unreduced Per-Token Loss
            targets_flat = chunk_tgt.contiguous().view(-1)
            loss_unreduced = F.cross_entropy(logits_flat, targets_flat, ignore_index=256, reduction='none')

            # Apply Response-Weighted Loss Masking (Safe, No 0/0 Division)
            if token_weights is not None:
                chunk_w = token_weights[:, c_start:c_end].contiguous().view(-1)
                valid_mask = (targets_flat != 256).float() * chunk_w
                denom = valid_mask.sum().clamp_min(1.0)
                chunk_loss = (loss_unreduced * valid_mask).sum() / denom
            else:
                valid_mask = (targets_flat != 256).float()
                denom = valid_mask.sum().clamp_min(1.0)
                chunk_loss = (loss_unreduced * valid_mask).sum() / denom
...
# =============================================================================
# 6. DATASET WITH PER-TOKEN RESPONSE WEIGHTING
# =============================================================================
class WeightedParityDataset(Dataset):
    def __init__(self, hf_data, tokenizer, max_samples=100, max_len=512):
        ...
        for item in hf_data:
            ...
            if inst and out:
                prompt_prefix = f"User: {inst}\nKaryon: "
                prompt_ids = tokenizer.encode(prompt_prefix)[:-1] # Remove EOS from prefix
                response_ids = tokenizer.encode(out)
                ...
                if len(full_ids) > 16:
                    # Token weights: 0.2 on prompt, 1.0 on response
                    weights = [0.2] * min(len(prompt_ids), len(full_ids))
                    weights += [1.0] * (len(full_ids) - len(weights))
...
def collate_weighted_fn(batch):
    ids_list = [item["ids"] for item in batch]
    w_list = [item["weights"] for item in batch]
    padded_ids = pad_sequence(ids_list, batch_first=True, padding_value=256)
    padded_w = pad_sequence(w_list, batch_first=True, padding_value=0.0)
    return padded_ids, padded_w


# =============================================================================
# 7. EXP-38 MASTER EXECUTION & AUTOMATED DECISION ENGINE
# =============================================================================
def run_exp_38_benchmark():
    print("\n" + "="*85)
    print(" === EXP-38: WIDE MULTI-SCALE SSD (H=12, D=768) + RESPONSE FOCUS BENCHMARK ===")
    print("="*85)
    ...
    BATCH_SIZE = 32
    NUM_EVAL_BATCHES = 80
    dataset = WeightedParityDataset(raw_dataset, tokenizer, max_samples=NUM_EVAL_BATCHES, max_len=512)
    loader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate_weighted_fn, drop_last=True)

    logger.info(f"Parity Dataset Loaded. Total Batches: {len(loader)} | Batch Size: {BATCH_SIZE}")

    # -------------------------------------------------------------------------
    # PART A: RUN BASELINE (H=8, D=512, UNIFORM WEIGHTING)
    # -------------------------------------------------------------------------
    print("\n" + "-"*85)
    print(" [1/2] RUNNING BASELINE: 8-HEAD MULTI-SCALE SSD (D=512, UNIFORM LOSS WEIGHTING)")
    print("-" * 85)

    model_base = ConfigurableAgent(num_heads=8, head_v=64, expand_dim=1536, beta_min=0.001, beta_max=0.30, device_str=device_str).to(device)
    ...
    for idx, (batch_tokens, batch_weights) in enumerate(loader):
        ...
        opt_base.zero_grad()
        # Uniform weighting (token_weights=None)
        loss = model_base.forward_sequence(in_seq, tgt_seq, hu, token_weights=None, chunk_size=64, optimizer=opt_base)
    ...
    # -------------------------------------------------------------------------
    # PART B: RUN PROPOSED EXP-38 (H=12, D=768, RESPONSE-WEIGHTED LOSS)
    # -------------------------------------------------------------------------
    print("\n" + "-"*85)
    print(" [2/2] RUNNING PROPOSED: EXP-38 WIDE 12-HEAD SSD (D=768, RESPONSE-WEIGHTED LOSS)")
    print("-" * 85)

    model_prop = ConfigurableAgent(num_heads=12, head_v=64, expand_dim=2048, beta_min=0.0005, beta_max=0.35, device_str=device_str).to(device)
    ...
    for idx, (batch_tokens, batch_weights) in enumerate(loader):
        ...
        in_seq = batch_tokens[:, :-1]
        tgt_seq = batch_tokens[:, 1:]
        tgt_weights = batch_weights[:, 1:]
        hu = HomeostaticUnit(batch_size=BATCH_SIZE, device=device_str)

        opt_prop.zero_grad()
        # Response-weighted loss active (token_weights=tgt_weights)
        loss = model_prop.forward_sequence(in_seq, tgt_seq, hu, token_weights=tgt_weights, chunk_size=64, optimizer=opt_prop)
    ...
    # -------------------------------------------------------------------------
    print("\n" + "="*85)
    print(" === EXP-38 GRADIENT FLOW INSPECTION (PROPOSED MODEL) ===")
    print("="*85)
    ...
    print("\n" + "="*85)
    print(" === EXP-38 FINAL TELEMETRY AUDIT TABLE ===")
    print("="*85)
    print(f"{'Metric':<32} | {'Baseline (H=8, D=512, Uniform)':<32} | {'Proposed (EXP-38 H=12, D=768, Focus)':<34}")
    print("-" * 95)
    print(f"{'Parameter Count':<32} | {param_count_base:<32,} | {param_count_prop:<34,}")
    print(f"{'State Matrix Capacity (M_t)':<32} | {'16,384 floats (8 heads)':<32} | {'24,576 floats (12 heads)':<34}")
    print(f"{'Loss Weighting':<32} | {'Uniform (1.0 prompt & resp)':<32} | {'Response Focus (0.2 pr / 1.0 resp)':<34}")
    ...
    print(f"\n[EXP-38 Decision Engine] Delta Loss: {delta_loss:+.4f} | Throughput Retention: {speed_ratio:.1f}%")
    
    if delta_loss <= -0.08 and speed_ratio >= 75.0:
        verdict = "🟢 POSITIVE — Significant capacity & response-focus gain on real dataset with high throughput retention. Ready for production merge."
    elif delta_loss <= 0.05 and speed_ratio >= 65.0:
