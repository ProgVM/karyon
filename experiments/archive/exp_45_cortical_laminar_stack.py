"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-45 (CORTICAL LAMINAR HIERARCHY STACK)
3-Tier Canonical Cortical Laminar Architecture (Sensory L-IV -> Associative L-II/III -> Motor L-V)
evaluated against Established Canonical Baseline on vicgalle/alpaca-gpt4 (S=2048).
Protocol: KEP v5.2 (Rules #1, #2, #3, #4, #6, #7).
Biophysical Grounding: Canonical Cortical Microcircuits (Douglas & Martin 2004, Bastos & Friston 2012).
...
print(f"\n[EXP-45] Initializing Cortical Laminar Hierarchy Benchmark on: {device_str.upper()}")


# =============================================================================
# 3. MODERN CONTINUOUS HOPFIELD ATTRACTOR HEAD (512D)
# =============================================================================
...
# =============================================================================
# 5. 3-TIER CORTICAL LAMINAR AGENT (SENSORY L-IV -> ASSOCIATIVE L-II/III -> MOTOR)
# =============================================================================
class CorticalLaminarAgent(nn.Module):
    """3-Tier Canonical Cortical Microcircuit Architecture (v18.0)."""
    def __init__(self, device_str='cpu'):
        ...
        # ---------------------------------------------------------------------
        # STAGE 1: Primary Sensory Cortex (Layer IV - Fast Phonemic/Morphemic SSD)
        # ---------------------------------------------------------------------
        self.stage1_sensory_ssd = CalibratedParallelSSDCore(
        ...
        self.stage1_sensory_swiglu = ParallelSwiGLUBlock(
            hidden_dim=self.hidden_dim, expand_dim=1024, device=device_str
        )

        # ---------------------------------------------------------------------
        # STAGE 2: Deep Association Cortex (Layers II/III - Slow Cognitive SSD)
        # ---------------------------------------------------------------------
        self.stage2_assoc_ssd = CalibratedParallelSSDCore(
            text_dim=self.hidden_dim, unified_dim=self.unified_dim, hidden_dim=self.hidden_dim,
            num_heads=self.num_heads, head_k=self.head_k, head_v=self.head_v, device=device_str
        )
        self.stage2_assoc_swiglu = ParallelSwiGLUBlock(
            hidden_dim=self.hidden_dim, expand_dim=2048, device=device_str
        )

        # ---------------------------------------------------------------------
        # STAGE 3: Inter-Laminar Fusion & Motor-Articulatory Cortex (Layer V)
        # ---------------------------------------------------------------------
        self.laminar_fusion_norm = nn.LayerNorm(self.hidden_dim).to(self.device)
    ...
    def get_all_parameters(self) -> List[nn.Parameter]:
        params = (
            ...
            list(self.laminar_fusion_norm.parameters()) +
            list(self.attractor_head.parameters()) + 
            list(self.motor_text_proj.parameters())
        )
        for sub in [self.stage1_sensory_ssd, self.stage1_sensory_swiglu, 
                    self.stage2_assoc_ssd, self.stage2_assoc_swiglu]:
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, 
        ...
        # Dual-Laminar Memory States
        m1_sensory = torch.zeros(batch_size, self.num_heads, self.head_k, self.head_v, device=self.device)
        m2_assoc   = torch.zeros(batch_size, self.num_heads, self.head_k, self.head_v, device=self.device)
        ...
        for chunk_idx in range(num_chunks):
            ...
            # === STAGE 1: Primary Sensory Processing (Layer IV) ===
            ssd1_out = self.stage1_sensory_ssd.forward_chunk_parallel_ssd(chunk_emb, m1_sensory, curr_u_t, 1.0)
            h1_ssm, m1_sensory = ssd1_out[0], ssd1_out[1]
            h1_sensory = self.stage1_sensory_swiglu(h1_ssm) # [B * c_len, 512]

            # === STAGE 2: Deep Association Processing (Layers II/III) ===
            h1_seq = h1_sensory.view(batch_size, c_len, self.hidden_dim)
            ssd2_out = self.stage2_assoc_ssd.forward_chunk_parallel_ssd(h1_seq, m2_assoc, curr_u_t, 1.0)
            h2_ssm, m2_assoc = ssd2_out[0], ssd2_out[1]
            h2_assoc = self.stage2_assoc_swiglu(h2_ssm) # [B * c_len, 512]

            # === STAGE 3: Laminar Residual Fusion & Motor Readout ===
            h_cortical = self.laminar_fusion_norm(h1_sensory + h2_assoc) # [B * c_len, 512]

            h_relaxed, _ = self.attractor_head.relax_to_minima(h_cortical)
            h_proj = self.motor_text_proj(h_relaxed) # [B * c_len, 256]
            ...
            # Event Boundary Theta Phase Reset across both laminar layers
            with torch.no_grad():
                has_eos = (chunk_in == 257).any(dim=-1).view(batch_size, 1, 1, 1).float()
                m1_sensory = m1_sensory * (1.0 - has_eos)
                m2_assoc   = m2_assoc * (1.0 - has_eos)

            m1_sensory = m1_sensory.detach()
            m2_assoc   = m2_assoc.detach()
    ...
    def generate_thought_and_speech(self, prompt: str, hu, max_generated_tokens: int = 75,
        ...
        # Process Prompt through 3-Stage Laminar Circuit
        ssd1_out = self.stage1_sensory_ssd.forward_chunk_parallel_ssd(prompt_embs, m1, hu.state, 1.0)
        h1_ssm, m1 = ssd1_out[0], ssd1_out[1]
        h1_r = self.stage1_sensory_swiglu(h1_ssm)

        h1_seq = h1_r.view(1, prompt_tokens.size(1), self.hidden_dim)
        ssd2_out = self.stage2_assoc_ssd.forward_chunk_parallel_ssd(h1_seq, m2, hu.state, 1.0)
        h2_ssm, m2 = ssd2_out[0], ssd2_out[1]
        h2_r = self.stage2_assoc_swiglu(h2_ssm)
        ...
        for step in range(max_generated_tokens):
            ...
            ssd1_out = self.stage1_sensory_ssd.forward_chunk_parallel_ssd(ctx_emb, m1, hu.state, 1.0)
            h1_s, m1 = ssd1_out[0], ssd1_out[1]
            h1_out = self.stage1_sensory_swiglu(h1_s)

            h1_step = h1_out.view(1, 1, self.hidden_dim)
            ssd2_out = self.stage2_assoc_ssd.forward_chunk_parallel_ssd(h1_step, m2, hu.state, 1.0)
            h2_s, m2 = ssd2_out[0], ssd2_out[1]
            h2_out = self.stage2_assoc_swiglu(h2_s)

            h_cort = self.laminar_fusion_norm(h1_out + h2_out)
            h_rel, _ = self.attractor_head.relax_to_minima(h_cort)
...
# =============================================================================
# 7. EXP-45 MASTER EXECUTION & AUTOMATED DECISION ENGINE
# =============================================================================
def run_exp_45_benchmark():
    print("\n" + "="*85)
    print(" === EXP-45: CORTICAL LAMINAR HIERARCHY STACK (v18.0) BENCHMARK ===")
    print("="*85)
    ...
    logger.info(f"Laminar Setup Ready. Train Steps: {NUM_STEPS} (~20M tokens) | S={SEQ_LEN} | Val Batches: {len(loader_val)}")
    ...
    # -------------------------------------------------------------------------
    # RUN CORTICAL LAMINAR MODEL (3 STAGES, 8.4M PARAMS)
    # -------------------------------------------------------------------------
    model = CorticalLaminarAgent(device_str=device_str).to(device)
    ...
    print(f"  Cortical Laminar Parameters: {param_count:,} (3 Stages, Sensory + Associative + Motor)")
    print("-" * 85)
    ...
    for idx, batch_tokens in enumerate(loader_train):
        ...
        # High-Density 25-Step Logging
        if (idx + 1) % 25 == 0 or idx == NUM_STEPS - 1:
            avg_loss_25 = sum(loss_history[-25:]) / min(len(loss_history), 25)
            print(f"  Step [{idx+1:04d}/{NUM_STEPS}] | Loss: {loss.item():.4f} (Avg25: {avg_loss_25:.4f}) | PPL: {math.exp(min(loss.item(), 20.0)):.2f} | LR: {cur_lr:.6f} | Step: {step_ms:.1f}ms")

        # Periodic Conversational Speech Auditing
        if (idx + 1) % 150 == 0:
            sample_short = model.generate_thought_and_speech("User: Hello!\nKaryon:", diag_hu, max_generated_tokens=50)
            sample_long = model.generate_thought_and_speech("User: What is the primary source of energy for Earth?\nKaryon:", diag_hu, max_generated_tokens=65)
            print(f"  💬 [Laminar Speech Audit @ Step {idx+1:04d}]")
            print(f"     • 'Hello!': \"{sample_short}\"")
            print(f"     • 'Energy': \"{sample_long}\"")
            print("-" * 85)
    ...
    print(f"  Held-Out Validation Loss (EXP-45 Laminar): {final_val_loss:.4f} (PPL: {math.exp(min(final_val_loss, 20.0)):.2f})")
    print("="*85)

    # -------------------------------------------------------------------------
    # FINAL TELEMETRY COMPARISON AGAINST CANONICAL KNOWN BASELINES (KEP RULE #7)
    # -------------------------------------------------------------------------
    known_1layer_val_loss = 1.2536 # Documented 600-step baseline for 1-layer D=512
    known_base_speed = 150000.0
    delta_val_loss = final_val_loss - known_1layer_val_loss
    speed_ratio = (tok_per_sec / known_base_speed) * 100.0

    print("\n" + "="*85)
    print(" === EXP-45 FINAL TELEMETRY AUDIT TABLE (600 STEPS) ===")
    print("="*85)
    print(f"{'Metric':<34} | {'1-Layer Baseline (D=512)':<28} | {'Proposed (EXP-45 Laminar 3-Tier)':<32}")
    print("-" * 102)
    print(f"{'Parameter Count':<34} | {'4,069,384 (1 Layer)':<28} | {f'{param_count:,} (3 Stages)':<32}")
    print(f"{'Cortical Depth':<34} | {'1 Shot Reasoning':<28} | {'3-Tier Laminar Microcircuit':<32}")
    print(f"{'SwiGLU Reasoning Structure':<34} | {'1x 2048 Dim':<28} | {'Sensory(1024) + Assoc(2048)':<32}")
    print(f"{'Final Train Loss':<34} | {'~1.2500':<28} | {final_train_loss:<32.4f}")
    print(f"{'Held-Out Validation Loss':<34} | {known_1layer_val_loss:<28.4f} | {final_val_loss:<32.4f}")
    print(f"{'Validation Perplexity (PPL)':<34} | {math.exp(min(known_1layer_val_loss, 20.0)):<28.2f} | {math.exp(min(final_val_loss, 20.0)):<32.2f}")
    ...
    print(f"\n[EXP-45 Decision Engine] Delta Validation Loss: {delta_val_loss:+.4f} | Speed Ratio: {speed_ratio:.1f}%")
    
    if delta_val_loss <= -0.08 and speed_ratio >= 70.0:
        verdict = "🟢 POSITIVE — Significant laminar depth gain on held-out validation with high throughput. Ready for production merge."
