# experiments/exp_79_active_inference_world_model_error_injection.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-79 (ACTIVE INFERENCE WORLD-MODEL ERROR INJECTION - AI-WMEI)
Tests Friston's Active Inference World-Model Error Injection (AI-WMEI):
  1. Latent World Model computes reconstruction error vector e_world = w_t - w_pred in 256D
  2. e_latent_proj = Linear_wm(e_world) in 768D
  3. Combined Stage 2 input: e1_combined = e1_weighted + gamma_wm * e_latent_proj
By injecting global Active Inference world-model surprise directly into the Stage 2
slow semantic sheet, Stage 2 memory states are grounded in top-down world-model errors.
...
# =============================================================================
# 0. MULTI-SCALE MORPHOLOGICAL BYTE PYRAMID RECEPTIVE FIELD
# =============================================================================
...
class MultiScalePositionalByteEmbedding(nn.Module):
    def __init__(self, vocab_size=258, text_dim=256, max_len=8192):
        super().__init__()
...
# =============================================================================
# 1. ROTARY POSITION EMBEDDING (RoPE) UTILITIES
# =============================================================================
...
# =============================================================================
# 2. MAMBA-2 SSD LAYER WITH ELEMENTWISE SILU OUTPUT GATING & ROPE
# =============================================================================
...
# =============================================================================
# 4. CAUSAL DEPTHWISE CONVSWIGLU CHANNEL-MIXING BLOCK
# =============================================================================
...
# =============================================================================
# 5. CORTICAL STAGE WITH CONVSWIGLU
# =============================================================================
...
# =============================================================================
# 6. PRECISION-WEIGHTED LAMINAR ERROR ROUTING MODULE (PW-LPER)
# =============================================================================
class PrecisionWeightedLPER(nn.Module):
    ...
    def forward(self, h_s1: torch.Tensor, h1_prev_last: torch.Tensor, u_t: torch.Tensor) -> tuple:
...
# =============================================================================
# 7. ACTIVE INFERENCE WORLD MODEL WITH SENSORY ERROR PROJECTION
# =============================================================================
class ActiveInferenceWorldModel(nn.Module):
    def __init__(self, hidden_dim: int = 768, text_dim: int = 256, latent_dim: int = 128):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.text_dim = text_dim
        ...
        self.posterior_net = nn.Linear(hidden_dim + text_dim, latent_dim * 2)
        self.decoder_net = nn.Sequential(
            nn.Linear(latent_dim + hidden_dim, text_dim * 2),
            nn.SiLU(),
            nn.Linear(text_dim * 2, text_dim)
        )
        # World Model Error Projection: Maps 256D sensory reconstruction error into 768D cortical space
        self.error_proj = nn.Linear(text_dim, hidden_dim)

    def forward(self, h_prev_fast: torch.Tensor, h_curr_fast: torch.Tensor, w_current: torch.Tensor):
        prior_out = self.prior_net(h_prev_fast)
        mu_prior, logvar_prior = prior_out.chunk(2, dim=-1)
        logvar_prior = torch.clamp(logvar_prior, -10.0, 10.0)

        post_out = self.posterior_net(torch.cat([h_prev_fast, w_current], dim=-1))
        ...
        w_pred = self.decoder_net(torch.cat([z_t, h_curr_fast], dim=-1))
        ...
        e_world = w_current - w_pred # Sensory error vector [B, 256]
        rec_loss = torch.mean(e_world**2, dim=-1, keepdim=True)
        free_energy = kl_div + rec_loss

        e_wm_proj = self.error_proj(e_world) # Project into [B, 768]
        return w_pred, kl_div, free_energy, e_wm_proj


# =============================================================================
# 8. EXP-79 AGENT WITH WORLD-MODEL ERROR INJECTION (AI-WMEI)
# =============================================================================
class Exp79WorldModelErrorAgent(nn.Module):
    def __init__(self, config: CoREConfig, device_str: str = 'cpu'):
        ...
        self.pos_embeddings = MultiScalePositionalByteEmbedding(
            vocab_size=self.text_gen_dim, text_dim=self.text_dim, max_len=8192
        ).to(self.device)
        ...
        self.stage1 = CorticalStage(
            ...
            swiglu_kernel_size=3, device_str=device_str
        ).to(self.device)

        # Boundary Detector
        self.boundary_detector = EntropyAdaptiveBoundaryDetector(hidden_dim=self.hidden_dim).to(self.device)

        # Precision-Weighted LPER Module
        self.pw_lper = PrecisionWeightedLPER(hidden_dim=self.hidden_dim).to(self.device)

        # Active Inference World Model
        self.world_model = ActiveInferenceWorldModel(hidden_dim=self.hidden_dim, text_dim=self.text_dim, latent_dim=128).to(self.device)
        self.wm_error_gate = nn.Parameter(torch.tensor([0.10], device=self.device)) # Learnable gain

        # Stage 2: Slow Semantic Sheet
        self.stage2 = CorticalStage(
            hidden_dim=self.hidden_dim, expand_dim=self.expand_dim, num_heads=self.num_heads,
            head_k=self.head_k, head_v=self.head_v, min_beta=0.0001, max_beta=0.05,
            swiglu_kernel_size=7, device_str=device_str
        ).to(self.device)
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        for chunk_idx in range(num_chunks):
            ...
            # --- PRECISION-WEIGHTED LPER ROUTING ---
            e1_weighted, h1_prev_last, avg_pi = self.pw_lper(h_s1, h1_prev_last, curr_u_t)
            precisions.append(avg_pi)

            # --- ACTIVE INFERENCE WORLD MODEL ERROR INJECTION ---
            w_curr = full_emb[:, c_end - 1, :]
            h_curr_fast = h_s1[:, -1, :]
            w_pred, kl_div, fe, e_wm_proj = self.world_model(h_prev_fast, h_curr_fast, w_curr)
            h_prev_fast = h_curr_fast.detach()
            fe_losses.append(fe.mean())

            # Inject projected world-model error vector into Stage 2 input
            e1_combined = e1_weighted + self.wm_error_gate * e_wm_proj.unsqueeze(1)

            # Stage 2: Slow Semantic Pass
            h_s2, m_s2, dt2 = self.stage2(e1_combined, m_s2, curr_u_t, saliency_gate=saliency_gate, dt=1.0)
        ...
        avg_pi_val = sum(precisions) / len(precisions)
        avg_eff_dt = sum(eff_dts) / len(eff_dts)

        total_loss = avg_loss + 0.05 * avg_fe + 0.05 * avg_commit
        return total_loss, avg_loss.item(), avg_fe.item(), m_s2, h_s2[:, -1, :], curr_u_t, avg_eff_dt, avg_pi_val
    ...
    def generate_natural_speech(self, prompt: str, hu_state, max_tokens: int = 75) -> str:
        ...
        h_prev_fast = torch.zeros(1, self.hidden_dim, device=self.device)
        ...
        for c_idx in range(0, prompt_len, 64):
            ...
            w_curr = c_emb[:, -1, :]
            h_curr_fast = h_s1[:, -1, :]
            _, _, _, e_wm_proj = self.world_model(h_prev_fast, h_curr_fast, w_curr)
            h_prev_fast = h_curr_fast.detach()

            e1_combined = e1_weighted + self.wm_error_gate * e_wm_proj.unsqueeze(1)
            h_s2, m_s2, _ = self.stage2(e1_combined, m_s2, hu_state, saliency_gate=sal_gate, dt=1.0)
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-79 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_79_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-79 (ACTIVE INFERENCE WORLD-MODEL ERROR INJECTION)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-79 World-Model Error Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp79WorldModelErrorAgent(config, device_str=device_str).to(device)
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            for p in agent.world_model.parameters():
                if p.grad is not None:
                    g_wm = max(g_wm, p.grad.norm().item())
            g_gate = agent.wm_error_gate.grad.item() if agent.wm_error_gate.grad is not None else 0.0
            g_s1 = 0.0
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-79 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
            print(f"Metrics Progress          : Speech Loss = {speech_loss:.4f} (Avg: {cur_loss:.4f}, PPL: {cur_ppl:.2f}) | LR = {cur_lr:.6f}")
            print(f"Active Inference (F_t)    : Free Energy = {fe_val:.4f} | WM Error Gate = {agent.wm_error_gate.item():.4f}")
            ...
            print(f"Gradient Flow Inspection  : Total: {grad_norm_total:.4f} | Emb: {g_emb:.4f} | WM Grad: {g_wm:.4f} | WM Gate Grad: {g_gate:.6f} | S1: {g_s1:.4f} | S2: {g_s2:.4f}")
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-79 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-79 World-Model Error Value':<40}")
    print("-" * 95)
