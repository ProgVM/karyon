# experiments/exp_94_hierarchical_predictive_coding.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-94
Hypothesis:
1. True Hierarchical Predictive Coding (HPC) where Stage 2 (higher semantic sheet)
   generates a top-down prediction of Stage 1 (lower morpho-syntactic sheet)
   and routes only the precision-weighted prediction error upwards significantly
   accelerates speech loss reduction compared to temporal self-prediction.
2. Precision weighting modulated by somatic noradrenaline (NA) dynamically
   scales error sensitivity during high-surprise transitions.
...
def prepare_packed_stream(num_batches: int = 100, batch_size: int = 16, seq_len: int = 512):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-94 (S={seq_len}, Steps={num_batches})...")
...
class TrueHierarchicalPredictiveCoREAgent(CoREAgent):
    """
    Karyon-CoRE Agent implementing True Hierarchical Predictive Coding (HPC):
    1. Stage 2 generates top-down predictive feedback to Stage 1.
    2. Ascending prediction error is precision-weighted by somatic arousal (NA) and routed upwards.
    """
    def __init__(self, config, device='cpu'):
        super().__init__(config, device)
        # Top-down generative prediction network (Stage 2 -> Stage 1 prediction)
        self.topdown_generator = nn.Sequential(
            nn.Linear(self.hidden_dim, self.hidden_dim),
            nn.SiLU(),
            nn.Linear(self.hidden_dim, self.hidden_dim)
        ).to(self.device)
        
        # Precision weighting network based on error and noradrenaline (NA)
        self.precision_weight_net = nn.Sequential(
    ...
    def get_all_parameters(self):
        ...
        params.extend(list(self.topdown_generator.parameters()))
        params.extend(list(self.precision_weight_net.parameters()))
        return params
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, 
        ...
        # 1. Stage 1: Fast Morpho-Syntactic Cortical Pass
        h_s1, m_s1, dt1 = self.stage1(full_h_in, m_s1, curr_u_t, torch.Tensor(), 1.0)

        # 2. Compute Top-Down Generative Prediction from Stage 2 (using previous state proxy or zero for step 0)
        h_s2_prev = m_s2.view(batch_size, -1)[:, :self.hidden_dim].unsqueeze(1).expand(-1, seq_len, -1)
        h_s1_pred = self.topdown_generator(h_s2_prev)

        # 3. Compute Prediction Error
        e1_raw = h_s1 - h_s1_pred

        # 4. Compute Precision Weighting (pi_t) modulated by somatic noradrenaline (NA)
        na_expanded = na_level.unsqueeze(1).expand(batch_size, seq_len, 1)
        prec_in = torch.cat([h_s1, h_s1_pred, na_expanded], dim=-1)
        pi_t = 2.0 * self.precision_weight_net(prec_in)

        # 5. Route Precision-Weighted Error upwards to Stage 2
        e1_weighted = pi_t * e1_raw

        # 6. Stage 2: Slow Semantic-Discourse Pass
        saliency_gate = self.boundary_detector(h_s1, input_seq)
        h_s2, m_s2, dt2 = self.stage2(e1_weighted, m_s2, curr_u_t, saliency_gate, 1.0)

        eff_dt = (dt1 + dt2) / 2.0
        
        # Combined Representation (Stage 1 + Stage 2)
        ...
        critic_loss = F.none if v_current.numel() == 0 else F.mse_loss(v_current, td_targets)
        ...
        total_loss_tensor = (
            ...
            0.05 * commit_loss + 
            0.01 * ortho_loss + 
            0.02 * (critic_loss if isinstance(critic_loss, torch.Tensor) else 0.0)
        )
...
def run_exp_94_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-94 (TRUE HIERARCHICAL PREDICTIVE CODING)] ===")
    print("="*85)
    ...
    # 1. EVALUATE BASELINE (Standard CoREAgent with temporal self-prediction)
    print("\n" + "-"*85)
    ...
    # 2. EVALUATE PROPOSED (True Hierarchical Predictive Coding CoREAgent)
    print("\n" + "-"*85)
    print(" >>> RUNNING PHASE 2: PROPOSED (True Hierarchical Predictive Coding) <<<")
    print("-"*85)
    torch.cuda.empty_cache()
    torch.cuda.reset_peak_memory_stats()

    agent_prop = TrueHierarchicalPredictiveCoREAgent(config, device=device_str).to(device)
    ...
    print(f"VRAM Change        : {prop_vram - base_vram:+.1f} MB")
