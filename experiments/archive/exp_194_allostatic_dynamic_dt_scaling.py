# experiments/exp_194_allostatic_dynamic_dt_scaling.py
"""
===============================================================================
EXP-194: Allostatically-Modulated Dynamic dt Scaling & Temporal Integration
Grounding: KEP Principle 2 (Biological Realism), Principle 14 (Allostatic Forces: No Static Constants).
===============================================================================
Hypothesis:
Replacing the static or partially linear dt scaling with a fully allostatically-modulated
dynamic temporal integration mechanism—where the time-delta (dt) is dynamically scaled
by a non-linear function of Somatic Energy (u_t[1]), Noradrenaline (u_t[4]), and
Variational Free Energy (F_t)—will optimize temporal integration, prevent semantic drift,
...
logger = logging.getLogger("EXP-194")


class AllostaticTemporalIntegrator(nn.Module):
    """
    Allostatically-modulated dynamic temporal integration (dt scaling).
    """
    def __init__(self, hidden_dim: int, device_str: str = 'cpu'):
        ...
        # Non-linear temporal scaling estimator
    ...
    def forward(self, h_state: torch.Tensor, u_t: torch.Tensor, fe_loss: torch.Tensor) -> torch.Tensor:
        ...
        outputs = self.temporal_net(scaler_input)
        
        # Map output to biophysically realistic dt range (0.20 to 2.50)
        dynamic_dt = 0.20 + 2.30 * outputs
        
        # Apply temporal integration scaling
        return h_state * dynamic_dt
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-194: ALLOSTATIC TEMPORAL INTEGRATION BENCHMARK]")
    logger.info("=" * 80)
    ...
    # 1. Baseline Evaluation (EXP-193 Hopfield Gating)
    ...
    # 2. Proposed Evaluation (EXP-194 Allostatic Temporal Integration)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain
    brain_prop.allostatic_temporal_integrator = AllostaticTemporalIntegrator(
        hidden_dim=brain_prop.hidden_dim, device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Allostatic Temporal Integration Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-194 TELEMETRY REPORT ===")
    ...
    with open("experiments/exp_194_results.json", "w") as f:
