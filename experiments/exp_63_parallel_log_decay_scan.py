# experiments/exp_63_parallel_log_decay_scan.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-63 (PARALLEL LOG-SPACE CUMULATIVE DECAY SCAN)
Implements exact token-specific parallel retention decay scan via log-space cumulative sum:
Lambda_t = cumsum(log(alpha_m))
decay(i, j) = exp(Lambda_i - Lambda_j) * causal_mask(i, j)
This preserves exact, non-diluted token-specific gradients for the Entropy-Adaptive Boundary Detector.
...
class ParallelLogDecaySSDLayer(nn.Module):
    ...
    def forward(self, x_seq: torch.Tensor, m_prev: torch.Tensor, u_t: torch.Tensor, 
        ...
        if saliency_gate is not None:
            # At word boundaries, alpha is suppressed (decay is accelerated)
            alpha = alpha * (1.0 - 0.80 * saliency_gate)

        # Clamp alpha for numerical stability in log space: [B, H, chunk_len]
        alpha = torch.clamp(alpha, 1e-4, 0.9999)
        log_alpha = torch.log(alpha)
        beta = 1.0 - alpha

        # Exact Parallel Log-Space Cumulative Scan:
        # Lambda_t = sum_{m=1}^t log(alpha_m)
        lambda_t = torch.cumsum(log_alpha, dim=-1) # [B, H, chunk_len]

        # Intra-chunk decay matrix: exp(Lambda_i - Lambda_j) for i >= j
        # lambda_t.unsqueeze(-1) has shape [B, H, chunk_len, 1] (index i)
        # lambda_t.unsqueeze(-2) has shape [B, H, 1, chunk_len] (index j)
        log_decay_matrix = lambda_t.unsqueeze(-1) - lambda_t.unsqueeze(-2)
        decay_matrix = torch.exp(torch.clamp(log_decay_matrix, -20.0, 0.0))

        # Apply Causal Mask: [1, 1, chunk_len, chunk_len]
        ...
        # State decay from token j to the end of the chunk: exp(Lambda_{chunk_len-1} - Lambda_j)
        ...
        # Total chunk decay for m_prev: exp(Lambda_{chunk_len-1})
...
class EntropyAdaptiveBoundaryDetector(nn.Module):
    ...
    def forward(self, h_stage1: torch.Tensor, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        # Fused Boundary Saliency in [0.00, 1.00] with shape [B, 1, L]
        saliency = torch.clamp(0.05 + 0.60 * pred_boundary + 0.35 * is_token_boundary, 0.0, 1.0)
        return saliency.squeeze(-1).unsqueeze(1) # [B, 1, L]


# =============================================================================
# 4. EXP-63 AGENT WITH EXACT PARALLEL LOG-DECAY SCAN
# =============================================================================
class Exp63ParallelLogDecayAgent(nn.Module):
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        for chunk_idx in range(num_chunks):
            ...
            # Stage 2: Slow Semantic Pass with Exact Parallel Log-Decay Scan
...
def prepare_packed_stream(num_batches: int = 300, batch_size: int = 32, seq_len: int = 1024):
    logger.info(f"Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-63 (S={seq_len}, Steps={num_batches})...")
...
def run_exp_63_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-63 (PARALLEL LOG-SPACE CUMULATIVE DECAY SCAN)] ===")
    print("="*85)
    ...
    print("\n[1/1] Initializing EXP-63 Parallel Log-Decay Agent (300 Steps)...")
    torch.manual_seed(42)
    agent = Exp63ParallelLogDecayAgent(config, device_str=device_str).to(device)
    ...
    for step in range(num_eval_steps):
        ...
        if (step + 1) % 15 == 0 or step == num_eval_steps - 1:
            ...
            print("\n" + "="*95)
            print(f" === [KEP EXP-63 PROCESS DIAGNOSTICS DASHBOARD | STEP {step+1:03d}/{num_eval_steps}] ===")
            print("="*95)
    ...
    print("\n" + "="*95)
    print(" === [KEP EXP-63 FINAL TELEMETRY DASHBOARD] ===")
    print("="*95)
    print(f"{'Performance Metric':<36} | {'EXP-63 Parallel Log-Decay Value':<40}")
    print("-" * 95)
