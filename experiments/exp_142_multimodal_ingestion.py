...
motor actions (Text speech, Vision latent prediction, Audio generation, and Volitional Motor Efference).
...
- Multi-modal Motor Gateway outputs (Speech logits, Vision prediction, Audio generation, Motor efference)
...
def run_multimodal_ingestion_benchmark():
    ...
    for step in range(seq_len):
        ...
        with torch.no_grad():
            ...
            # Motor Gateway Multimodal Synthesis
        ...
        # Track cross-modal attention
        attn_dict = {f"ch_{i}": round(v.item(), 3) for i, v in enumerate(attn_weights.squeeze(0))} if attn_weights.numel() > 0 else {}
        attn_history.append(attn_dict)

        logger.info(
            f" Step {step+1:02d}/20 | Latency: {step_ms:.2f}ms | Free Energy: {fe_val:.4f} | "
            f"Attn: {attn_dict} | VisGen: {vis_gen.norm().item():.3f} | AudGen: {aud_gen.norm().item():.3f}"
        )
    ...
    logger.info(f"Speech Logits Shape : {text_logits.shape} (V=258)")
    logger.info(f"Vision Output Shape : {vis_gen.shape} (Dim=256)")
    logger.info(f"Audio Output Shape  : {aud_gen.shape} (Dim=256)")
    logger.info(f"Motor Action Vector : {actions.squeeze().cpu().numpy().round(4)}")
    logger.info(f"Cognitive Action Vec: {cog_actions.squeeze().cpu().numpy().round(4)}")
    logger.info(f"Epistemic Entropy   : {epistemic_entropy.mean().item():.4f}")
