# experiments/exp_200_allostatic_motor_gating.py
"""
===============================================================================
EXP-200: Allostatically-Gated Volitional Motor Head Readout
Grounding: KEP Principle 2 (Biological Realism), Principle 14 (Allostatic Forces: No Static Constants).
===============================================================================
Hypothesis:
Replacing the static linear or partially non-linear Volitional Motor Head projections with an
allostatically-gated motor readout mechanism—where the motor text projection and causal motor
receptive field (CPG) dynamics are dynamically modulated by a non-linear function of Somatic Energy (u_t[1]),
Noradrenaline (u_t[4]), and Variational Free Energy (F_t)—will optimize motor output selection,
...
logger = logging.getLogger("EXP-200")


class AllostaticMotorGating(nn.Module):
    """
    Allostatically-gated Volitional Motor Head.
    """
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-200: ALLOSTATIC MOTOR GATING BENCHMARK]")
    logger.info("=" * 80)
    ...
    # 1. Baseline Evaluation (EXP-199 LC Gating)
    ...
    # 2. Proposed Evaluation (EXP-200 Allostatic Motor Gating)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain
    brain_prop.allostatic_motor_gating = AllostaticMotorGating(
        hidden_dim=brain_prop.hidden_dim, device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Allostatic Motor Gating Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-200 TELEMETRY REPORT ===")
    ...
    with open("experiments/exp_200_results.json", "w") as f:
