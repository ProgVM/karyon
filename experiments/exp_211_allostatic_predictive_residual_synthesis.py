# experiments/exp_211_allostatic_predictive_residual_synthesis.py
"""
===============================================================================
EXP-211: Allostatically-Modulated Predictive Residual Coding Synthesis Engine
Grounding: KEP Principle 1 (C++20 as Engine), Principle 2 (Biological Realism),
           Principle 8 (Compositional Depth), Principle 14 (Axiom of Allostatic Dynamic Forces),
           Principle 15 (Net2Net Smooth Grafting: Strict Identity at Birth).
===============================================================================
Hypothesis:
In the Grand Synthesis of Neo-Cortical vectors, the bottom-up predictive error residual
`weighted_error` (computed by `PredictiveResidualRouting`: `precision * (h_s1 - topdown_pred(h_s2))`)
communicates the unpredicted sensory remainder from Stage 1 to Stage 2.
Currently in `PredictiveResidualRouting`, the precision weighting `precision = sigmoid(precision_gate(u_t))`
is computed via a single linear projection without non-linear interaction or residual adaptation,
and `weighted_error` is directly added into `h_combined` with a fixed unit scale (`+ weighted_error`).
Under KEP Principle 14 (Axiom of Allostatic Dynamic Forces), precision weighting of prediction errors
in biological active inference (Friston, 2010; Feldman & Friston, 2010) is a highly non-linear,
multi-timescale neuromodulatory process governed jointly by somatic stability, metabolic energy,
and cognitive urgency (Noradrenaline / Dopamine).
Replacing the single linear precision gate with an Allostatically-Modulated Deep Precision & Error Synthesis Engine:
  Pi(u_t) = sigmoid(W_base * u_t + W_nl * SiLU(W_hidden * u_t))
coupled with a zero-initialized Net2Net error refinement projection (KEP Principle 15) that strictly
preserves zero-delta function identity at birth t_0, will dynamically sharpen error precision under high arousal,
suppress sensory noise under low stability, and accelerate loss convergence (Loss Delta >= 0.08).
"""

import sys
import os
import time
import math
import json
import logging
from typing import Tuple
import torch
import torch.nn as nn
import torch.nn.functional as F

sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

from karyon_entity import KaryonEntity
from karyon_hardware import get_hardware_engine

logging.basicConfig(level=logging.INFO, format="%(module)-15s | %(levelname)-8s | %(asctime)s | %(message)s")
logger = logging.getLogger("EXP-211")


class AllostaticPredictiveResidualRouter(nn.Module):
    """
    Allostatically-Modulated Deep Precision & Error Synthesis Engine (EXP-211).
    Dynamically modulates prediction error precision with non-linear allostatic gating
    and zero-initialized Net2Net refinement projection (preserving strict identity at birth).
    """
    def __init__(self, base_router: nn.Module, hidden_dim: int, homeo_dim: int = 6, device_str: str = 'cpu'):
        super().__init__()
        self.device = torch.device('cuda' if 'cuda' in device_str else 'cpu')
        self.base_router = base_router
        self.hidden_dim = hidden_dim

        # Deep non-linear allostatic precision enhancement
        self.allostatic_precision_net = nn.Sequential(
            nn.Linear(homeo_dim, 32),
            nn.SiLU(),
            nn.Linear(32, hidden_dim),
            nn.Tanh()
        ).to(self.device)
        nn.init.zeros_(self.allostatic_precision_net[2].weight)
        nn.init.zeros_(self.allostatic_precision_net[2].bias)

        # Zero-initialized Net2Net refinement projection (KEP Principle 15)
        self.refine_proj = nn.Linear(hidden_dim, hidden_dim, bias=False).to(self.device)
        nn.init.zeros_(self.refine_proj.weight)

    def forward(self, h_s1: torch.Tensor, h_s2: torch.Tensor, u_t: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        hat_h_s1 = self.base_router.topdown_pred(h_s2)
        error_s1 = h_s1 - hat_h_s1

        B = h_s1.size(0)
        if u_t.dim() == 2:
            u_flat = u_t if u_t.size(0) == B else u_t[0:1].expand(B, -1)
        else:
            u_flat = u_t.view(1, -1).expand(B, -1)

        base_prec_logits = self.base_router.precision_gate(u_flat) # [B, hidden_dim]
        # Modulated precision logits with zero-init non-linear enhancement
        enhanced_prec_logits = base_prec_logits + 0.20 * self.allostatic_precision_net(u_flat)
        precision = torch.sigmoid(enhanced_prec_logits)

        if h_s1.dim() == 3:
            precision = precision.unsqueeze(1) # [B, 1, hidden_dim]

        weighted_error = precision * error_s1
        refined = self.refine_proj(weighted_error)
        final_weighted_error = weighted_error + refined

        error_magnitude = torch.mean(final_weighted_error ** 2)
        return final_weighted_error, error_magnitude


def evaluate_model(brain, entity, text_samples, num_steps=25, lr=1e-3):
    hw = get_hardware_engine()
    criterion = nn.CrossEntropyLoss(ignore_index=256)
    optimizer = torch.optim.AdamW(brain.parameters(), lr=lr, weight_decay=1e-4)

    step_losses = []
    step_fe_losses = []

    start_time = time.perf_counter()

    for step in range(num_steps):
        text = text_samples[step % len(text_samples)]
        prompt_ids = brain.tokenizer.encode(text)
        seq_t = torch.tensor([prompt_ids[:-1]], dtype=torch.long, device=hw.device)
        target_t = torch.tensor([prompt_ids[1:]], dtype=torch.long, device=hw.device)

        optimizer.zero_grad()
        tot_loss, speech_loss, fe_loss, _, _, _, _ = brain.forward_sequence(
            seq_t, target_t, entity.hu, criterion, chunk_size=seq_t.size(1), use_checkpointing=False
        )
        tot_loss.backward()
        torch.nn.utils.clip_grad_norm_(brain.parameters(), 1.0)
        optimizer.step()

        step_losses.append(float(speech_loss))
        step_fe_losses.append(float(fe_loss))

    elapsed = time.perf_counter() - start_time
    total_tokens = sum(len(brain.tokenizer.encode(t)) - 1 for t in text_samples[:num_steps])
    tok_per_sec = total_tokens / elapsed if elapsed > 0 else 0.0

    return {
        "final_loss": step_losses[-1],
        "initial_loss": step_losses[0],
        "mean_loss": sum(step_losses) / len(step_losses),
        "final_fe": step_fe_losses[-1],
        "initial_fe": step_fe_losses[0],
        "tok_per_sec": tok_per_sec,
        "elapsed": elapsed
    }


def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-211: ALLOSTATIC PREDICTIVE RESIDUAL SYNTHESIS BENCHMARK]")
    logger.info("=" * 80)

    hw = get_hardware_engine()
    logger.info(f"Target Accelerator: {hw.device_str.upper()}")

    text_samples = [
        "The quick brown fox jumps over the lazy dog near the riverbank with high agility.",
        "Active Inference formulates brain dynamics as continuous minimization of variational free energy.",
        "Homeostasis and allostasis regulate physiological variables through predictive bodily setpoints.",
        "Neural state space duality enables zero-loop associative parallel scans across deep cortical layers.",
        "Continuous Hopfield attractors snap neural trajectories into discrete conceptual semantic basins.",
        "Cortical laminar hierarchy routes top-down predictions and bottom-up precision-weighted error residuals."
    ] * 5

    # 1. Baseline Evaluation
    entity_base = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_base = entity_base.brain
    logger.info("Running Baseline Evaluation...")
    b_results = evaluate_model(brain_base, entity_base, text_samples, num_steps=25)

    # 2. Proposed Evaluation (EXP-211 Allostatic Predictive Residual Synthesis)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain

    allostatic_router = AllostaticPredictiveResidualRouter(
        base_router=brain_prop.predictive_residual_router,
        hidden_dim=brain_prop.hidden_dim,
        homeo_dim=6,
        device_str=hw.device_str
    ).to(hw.device)

    brain_prop.predictive_residual_router = allostatic_router

    logger.info("Running Proposed Allostatic Predictive Residual Synthesis Evaluation...")
    p_results = evaluate_model(brain_prop, entity_prop, text_samples, num_steps=25)

    loss_delta = b_results['final_loss'] - p_results['final_loss']
    fe_delta = b_results['final_fe'] - p_results['final_fe']

    logger.info("=" * 80)
    logger.info("📊 === EXP-211 TELEMETRY REPORT ===")
    logger.info(f"  - Baseline Final Loss : {b_results['final_loss']:.4f} nats | Throughput: {b_results['tok_per_sec']:.1f} tok/s")
    logger.info(f"  - Proposed Final Loss : {p_results['final_loss']:.4f} nats | Throughput: {p_results['tok_per_sec']:.1f} tok/s")
    logger.info(f"  - Loss Delta (B - P)  : {loss_delta:.4f} nats")
    logger.info(f"  - Free Energy Delta   : {fe_delta:.6f}")
    logger.info(f"  - Baseline Duration   : {b_results['elapsed']:.3f} s")
    logger.info(f"  - Proposed Duration   : {p_results['elapsed']:.3f} s")

    verdict = "POSITIVE" if (loss_delta >= 0.08 or (loss_delta >= 0.02 and p_results['tok_per_sec'] >= 0.90 * b_results['tok_per_sec'])) else "NEUTRAL"

    results = {
        "exp_id": "EXP-211",
        "verdict": verdict,
        "base_initial_loss": b_results['initial_loss'],
        "base_final_loss": b_results['final_loss'],
        "proposed_initial_loss": p_results['initial_loss'],
        "proposed_final_loss": p_results['final_loss'],
        "loss_delta": loss_delta,
        "fe_delta": fe_delta,
        "throughput_tok_per_sec": p_results['tok_per_sec'],
        "execution_time_s": p_results['elapsed']
    }

    with open("experiments/exp_211_results.json", "w") as f:
        json.dump(results, f, indent=2)

    logger.info(f"🏆 Final Verdict: 🟢 {verdict}" if verdict == "POSITIVE" else f"🏆 Final Verdict: ⚪ {verdict}")
    return results


if __name__ == "__main__":
    run_benchmark()
