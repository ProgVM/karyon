# experiments/exp_53_crisp_attractors_and_morphemic_snapping.py
"""
===============================================================================
KARYON EXPERIMENTAL BENCHMARK: EXP-53 (CRISP ATTRACTORS & MORPHEMIC SNAPPING)
Evaluating High-Contrast Continuous Hopfield Attractor Landscape (beta=12.0) +
Entropy-Adaptive Morphemic Sampling + Balanced Mamba-2 SSD Engine.
...
# =============================================================================
# 1. CRISP CONTINUOUS HOPFIELD ATTRACTOR HEAD (RAMSAUER ET AL. 2020)
# =============================================================================
class CrispContinuousHopfieldHead(nn.Module):
    """
    High-Contrast Continuous Hopfield Attractor Basin Landscape:
    - Unit-sphere normalized attractor basins (||b_i|| = 1.0).
    - Sharp inverse temperature (beta = 12.0) with dopamine signal-to-noise modulation.
    - Pattern separation loss enforcing orthogonal attractor concepts.
    """
    ...
    def relax_to_minima(self, h_state: torch.Tensor, u_t: torch.Tensor):
        ...
        # Normalize basins to unit sphere
...
# =============================================================================
# 2. BALANCED MAMBA-2 SSD CORE
# =============================================================================
class BalancedMamba2SSDCore(nn.Module):
...
# =============================================================================
# 3. EXPERIMENTAL AGENT CONTAINER
# =============================================================================
class EXP53Agent(nn.Module):
    def __init__(self, use_crisp_attractors: bool = True, device_str: str = 'cpu'):
        super().__init__()
        ...
        self.ssd_core = BalancedMamba2SSDCore(
    ...
    def forward_sequence(self, input_seq: torch.Tensor, target_seq: torch.Tensor, hu_batch, criterion, chunk_size: int = 64):
        ...
        ortho_loss = self.attractor_head.compute_pattern_separation_loss() if hasattr(self.attractor_head, 'compute_pattern_separation_loss') else torch.tensor(0.0, device=self.device)
        total_loss = avg_loss + 0.05 * avg_commit + 0.01 * ortho_loss
        return total_loss, avg_loss.item()

    def generate_adaptive_sample(self, prompt: str, max_tokens: int = 60) -> str:
        """Generates speech with Entropy-Adaptive Morphemic Sampling."""
        self.eval()
        ...
        with torch.no_grad():
            for step in range(max_tokens):
                ...
                # Measure local byte entropy
                p_dist = F.softmax(raw_logits, dim=-1)
                local_entropy = -(p_dist * torch.log(p_dist + 1e-9)).sum(dim=-1).item()

                # Entropy-Adaptive temperature: high at word start, sharp inside word
                last_char_is_space = (len(rolling_ids) > 0 and rolling_ids[-1] in [32, 10, 44, 46])
                if last_char_is_space or local_entropy > 0.60:
                    temp = 0.45
                    top_p = 0.90
                else:
                    temp = 0.08  # Crisp MAP spelling inside morphemes
                    top_p = 0.99
...
# =============================================================================
# 4. REAL DATASET & PARITY BENCHMARK
# =============================================================================
def prepare_packed_batches(num_batches: int = 150, batch_size: int = 32, seq_len: int = 512):
    logger.info("Loading Real Dataset (vicgalle/alpaca-gpt4) for EXP-53...")
...
def run_exp_53_benchmark():
    print("\n" + "="*85)
    print(" === [KEP EXPERIMENTAL BENCHMARK: EXP-53 (CRISP ATTRACTORS & MORPHEMIC SNAPPING)] ===")
    print("="*85)
    ...
    # 1. BASELINE: EXP-52 (DESATURATED ATTRACTORS)
    print("\n[1/2] Benchmarking Model A: Baseline EXP-52 (Desaturated Attractors)...")
    torch.manual_seed(42)
    model_a = EXP53Agent(use_crisp_attractors=False, device_str=device_str).to(device)
    ...
    sample_a = model_a.generate_adaptive_sample(diag_prompt, max_tokens=60)

    # 2. PROPOSED: EXP-53 (CRISP HOPFIELD ATTRACTORS & MORPHEMIC SNAPPING)
    print("\n[2/2] Benchmarking Model B: EXP-53 (Crisp Attractor Basins beta=12.0 + Morphemic Snapping)...")
    torch.manual_seed(42)
    model_b = EXP53Agent(use_crisp_attractors=True, device_str=device_str).to(device)
    ...
    sample_b = model_b.generate_adaptive_sample(diag_prompt, max_tokens=60)
    ...
    print("="*85)
    print(f"{'Performance Metric':<32} | {'Model A (EXP-52)':<22} | {'Model B (EXP-53)':<22} | {'Delta':<12}")
    print("-" * 85)
