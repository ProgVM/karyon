...
"""
================================================================================
EXP-303: THE SOVEREIGN THERMODYNAMIC MORPHIC ENGINE
================================================================================
Implements the full canonical manifesto:
  1. Adaptive Thermodynamic Noise:
     sigma_noise(t) = sigma_base + gamma * tanh(F_t)
     Injects physical noise into the latent thinking space to melt frozen states,
     escape local minima, and foster genuine non-deterministic creative search.
  2. Pure Modality-Agnostic Substrate:
     Zero text-specific heuristics, zero ASCII-specific biases, zero discrete pointer hacks.
  3. Deliberative Thinking Latent Settling (KEP Principle 21):
     Decouples problem settling from reaction time.
  4. Continuous C-SSD Soliton Attractor Gaze:
     Differentiable continuous resonant wave dynamics.
  5. Two-Tier Genesis:
     - Macro: Evolutionary Graph Topology & Epigenetics.
     - Micro: Forward-only Local Predictive Coding with Synaptic Normalization.
================================================================================
"""
...
# =====================================================================
# 1. SOVEREIGN THERMODYNAMIC ENGINE ARCHITECTURE
# =====================================================================

class SovereignThermodynamicEngine:
    """
    Continuous Modality-Agnostic Cognitive Substrate with
    Adaptive Thermodynamic Annealing and Deliberative Latent Settling.
    """
    def __init__(
        self,
        pop_size: int = 32,
        ...
        # Macro-Genome (Discrete Topologies & Epigenetic Locks)
        ...
        # Micro-Synaptic Plastic Substrate
        ...
        # Continuous C-SSD Soliton Matrices
    ...
    def forward_graph_cycle(
        ...
        free_energy_stress: Optional[torch.Tensor] = None, # [P]
        cycles: int = 1
    ) -> Tuple[torch.Tensor, torch.Tensor, List[torch.Tensor], List[torch.Tensor]]:
        """
        Executes internal recurrent deliberation cycles with adaptive thermodynamic noise.
        """
        P, K, B, D = current_states.shape
        gated_route = torch.tanh(self.w_route)
        graft_gate = torch.tanh(self.alpha_epi).unsqueeze(-1).unsqueeze(-1)

        # Compute dynamic noise standard deviation based on somatic stress
        if free_energy_stress is not None:
            sigma_eff = self.base_noise + self.noise_gain * torch.tanh(free_energy_stress).view(P, 1, 1, 1)
        else:
            sigma_eff = self.base_noise
        ...
        for _ in range(cycles):
            ...
            # Inject thermodynamic noise into pre-activation stream
            if self.base_noise > 0:
                noise = torch.randn_like(agg_inputs) * sigma_eff
                agg_inputs = agg_inputs + noise

            recorded_inputs.append(agg_inputs)

            projected_in = torch.einsum('pkbd,pkde->pkbe', agg_inputs, self.w_node_in)

            # Atomic Dynamic Equations (Universal Basis)
    ...
    def execute_latent_settling_phase(
        ...
        prompt_field: torch.Tensor,
        cycles: int = 4
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Phase 1.5: Decoupled Thinking Latent Settling (KEP Principle 21).
        """
    ...
    def compute_continuous_soliton_gaze(
        self,
        h_deliberation: torch.Tensor,
        prompt_field: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        100% Continuous Differentiable C-SSD Soliton Resonant Gaze.
        """
        P, B, L, D = prompt_field.shape
        q = torch.einsum('pbd,pde->pbe', h_deliberation, self.w_focus_q).unsqueeze(2)
        k = torch.einsum('pblm,pme->pble', prompt_field, self.w_content_k)
        
        mean_field = prompt_field.mean(dim=2, keepdim=True)
        contrast = torch.norm(prompt_field - mean_field, dim=-1)
        
        raw_energy = torch.einsum('pbxd,pbyd->pbxy', q, k).squeeze(2) / (D ** 0.5)
        soliton_logits = (raw_energy + contrast) * self.soliton_sharpness
        continuous_gaze = F.softmax(soliton_logits, dim=-1)
        
        p_copy = torch.sigmoid(torch.einsum('pbd,pdm->pbm', h_deliberation, self.w_copy_gate)).squeeze(-1)
    ...
    def process_and_evolve_stream(
        ...
        curr_token = p_pop[:, :, -1]
        active_stress = torch.zeros(P, device=self.device)
        ...
        for t_step in range(t_max):
            ...
            h_int, node_states, rec_in, rec_act = self.forward_graph_cycle(
                node_states, x_sens, free_energy_stress=active_stress, cycles=thinking_cycles
            )
            ...
            step_energy = step_nll.mean(dim=1)
            total_free_energy += step_energy
            active_stress = step_energy # Drives dynamic noise temperature in next step
            
            pred_tok = fused_probs.argmax(dim=-1)
            correct_token_count += (pred_tok == target_tok).float().sum(dim=1)
            
            # Local Predictive Step
            with torch.no_grad():
...
# =====================================================================
# 2. RUN EXPERIMENT EXP-303
# =====================================================================

def evaluate_exact_matches_by_domain(engine: SovereignThermodynamicEngine, suite: Dict[str, List]) -> Dict[str, Dict]:
    results = {}
    P = engine.P
    D = engine.D
    K = engine.K
...
def main():
    print("=" * 80)
    print("🔥 EXP-303: SOVEREIGN THERMODYNAMIC MORPHIC BENCHMARK")
    print("   [Adaptive Noise Annealing | Modality-Agnostic Substrate | Soliton Gaze]")
    print("=" * 80)
    ...
    print(f"Total Continuous Stream: {len(stream_data)} events across 5 domains.")
    print(f"Population: P=32 on {device} | Deliberation Cycles: K=4.")

    engine = SovereignThermodynamicEngine(
        pop_size=32,
        dim=64,
        max_nodes=8,
        vocab_size=258,
    ...
    print("\n--- Initiating Thermodynamic Stream Adaptation (N=1 Single Pass) ---")
    ...
    print("=" * 80)
    print(f"✅ Sovereign Thermodynamic Stream Completed in {elapsed:.2f}s!")
