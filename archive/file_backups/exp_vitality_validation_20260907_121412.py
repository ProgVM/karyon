...
def run_ashby_friston_vitality_test():
    ...
    with torch.no_grad():
        for energy_val in [0.9, 0.5, 0.1]:
            ...
            if hasattr(entity.brain, 'efe_action_evaluator') and entity.brain.efe_action_evaluator is not None:
                # Mock sensory inputs
                sensory_in = torch.zeros(1, entity.brain.unified_dim, device=entity.device)
                # Evaluate action probabilities
                action_probs = entity.brain.efe_action_evaluator(entity.h_fast, sensory_in, test_hu_state)
    ...
    # -------------------------------------------------------------------------
    logger.info("\n--- Pillar 5: Somatic Metabolism & Affective Allostasis (M_allostasis) ---")
    # Verify that energy drains during generation and restores during perceptive rest
    initial_energy = entity.hu.state[0, 1].item()
    
    # Run an interaction to drain energy
    list(entity.interact("Analyze the thermodynamic free energy of a closed-loop system", max_tokens=20))
    post_gen_energy = entity.hu.state[0, 1].item()
    
    # Run perceptive rest (long user input) to restore energy
    long_input = "A" * 500
    list(entity.interact(long_input, max_tokens=5))
    restored_energy = entity.hu.state[0, 1].item()
    
    # M_allostasis score: 1.0 if energy drains and restores correctly
    m_allostasis = 0.0
