...
class LocalPredictivePlasticityEngine:
    """
    Implements Local Hierarchical Predictive Error Routing & 3-Factor Plasticity
    (Neuromodulated by Noradrenaline NA_t and Dopamine DA_t without global autograd BPTT).
    """
    def __init__(self, agent: CoREAgent, lr: float = 1e-4):
        self.agent = agent
        self.lr = lr
    ...
    def local_update_step(self, inp_tokens: torch.Tensor, tgt_tokens: torch.Tensor, hu: HomeostaticUnit, episodic_mem: BatchedEpisodicMemory):
        ...
        h_s1, m_s1, _ = self.agent.stage1(h_in, m_s1, u_t, torch.Tensor(), 1.0)
        sal_gate = self.agent.boundary_detector(h_s1, inp_tokens)
        
        # 2. Local Top-Down Prediction & Error Computation
        topdown_pred_s1 = self.agent.topdown_prior_proj(h_s1)
        
        # Local Predictive Error for Layer 1
        eps_1 = h_s1 - topdown_pred_s1.detach()
        
        # 3. Stage 2 Semantic-Discourse Forward (takes eps_1 error residual)
        h_s2, m_s2, _ = self.agent.stage2(eps_1, m_s2, u_t, sal_gate, 1.0)
        
        # 4. Attractor & Volitional Readout
        topdown_s2 = self.agent.topdown_prior_proj(h_s2)
        h_comb = h_s1 + h_s2 + 0.15 * topdown_s2
        ...
        # 5. Local 3-Factor Hebbian Parameter Updates
        eff_lr = self.lr * neuromod
        with torch.no_grad():
            for p in self.agent.volitional_head.parameters():
                if p.grad is not None:
                    p.data -= eff_lr * p.grad
            for p in self.agent.in_proj.parameters():
                if p.grad is not None:
                    p.data -= eff_lr * p.grad

        free_energy = speech_loss.item() * 0.1
        hu.update_somatic_state(speech_loss.item(), free_energy)
...
def run_experiment():
    ...
    texts = [ex["instruction"] + " " + ex["output"] for ex in dataset.select(range(100))]
