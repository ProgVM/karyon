...
if karyon_cpp is None:
    ...
    for path in ["/kaggle/working/karyon/build/karyon_core_jit", "/root/.cache/torch_extensions/py312_cu128/karyon_cpp_ext_v25"]:
        if os.path.exists(path) and path not in sys.path:
            sys.path.append(path)
            
    for candidate_name in ["karyon_cpp_ext_v25", "karyon_cpp_ext_v24", "karyon_core_ext", "karyon_core_ext_v1"]:
        try:
