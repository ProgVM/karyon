...
def test_vector_a(agent: CoREAgent, config: CoREConfig, device: torch.device):
    ...
    episodic_memory = BatchedEpisodicMemory(batch_size=1, max_active_cpu=128, device=str(device))
...
def test_vector_b(agent: CoREAgent, device: torch.device):
    ...
    with torch.no_grad():
        for step in range(10):
            # Generate candidate thought delta via parallel rollout
            w_dummy = torch.zeros(1, agent.unified_dim, device=device)
            w_pred, fe, _, _ = agent.world_model(h_curr, h_curr, w_dummy)
...
def run_survival_simulation(agent: CoREAgent, mode: str, max_steps: int = 40, seed: int = 42, device: str = 'cpu'):
    ...
    episodic_memory = BatchedEpisodicMemory(batch_size=1, max_active_cpu=128, device=device)
        ...
        with torch.no_grad():
            ...
            if mode == 'system1':
                ...
                for cand_idx, cand_act in enumerate(candidate_actions):
                    # Pad candidate to unified dim 256
                    w_cand = torch.zeros(1, 256, device=agent.device)
                    w_cand[:, :4] = cand_act
                    
                    # Episodic Memory Boost if unified mode
                    if mode == 'unified':
                        q_k = agent.episodic_sensory_proj(w_cand)
                        ret_mem, max_sim = episodic_memory.read(q_k, temperature=0.05, threshold=0.10)
                        w_cand = w_cand + 0.15 * ret_mem[:, :256]
...
def main():
    ...
    # System 2 & Unified Imagination should significantly outperform System 1 reactive execution
