...
class AgentDBManager:
    ...
    async def _init_tables(self):
        ...
        async with self.db.cursor() as cursor:
            ...
            # 7. Model Quota Limits & Rate Telemetry (Persisted across sessions)
            await cursor.execute("""
                CREATE TABLE IF NOT EXISTS model_quota_limits (
                    model_name TEXT PRIMARY KEY,
                    quota_metric TEXT DEFAULT NULL,
                    quota_value INTEGER DEFAULT NULL,
                    retry_delay_sec REAL DEFAULT NULL,
                    is_daily INTEGER DEFAULT 0,
                    last_exhausted_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
                )
            """)
            """)
    ...
    # =========================================================================
    # 4b. MODEL QUOTA LIMITS & RATE TELEMETRY PERSISTENCE
    # =========================================================================
    async def record_model_quota_limit(
        self,
        model_name: str,
        ...
        """Saves or updates discovered model quota limit info into SQLite."""
        async with self._lock:
            try:
                if not self.db:
                    await self.connect()
                await self.db.execute("""
                    INSERT INTO model_quota_limits (model_name, quota_metric, quota_value, retry_delay_sec, is_daily, last_exhausted_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
                    ON CONFLICT(model_name) DO UPDATE SET
                        quota_metric = COALESCE(EXCLUDED.quota_metric, model_quota_limits.quota_metric),
                        quota_value = COALESCE(EXCLUDED.quota_value, model_quota_limits.quota_value),
                        retry_delay_sec = COALESCE(EXCLUDED.retry_delay_sec, model_quota_limits.retry_delay_sec),
                        is_daily = EXCLUDED.is_daily,
                        last_exhausted_at = CURRENT_TIMESTAMP,
                        updated_at = CURRENT_TIMESTAMP
                """, (model_name, quota_metric, quota_value, retry_delay_sec, 1 if is_daily else 0))
                await self.db.commit()
            except Exception as e:
                logger.warning(f"Error recording model quota limit for '{model_name}': {str(e)}")

    async def get_model_quota_limits(self, model_name: Optional[str] = None) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
        """Retrieves discovered model quota limit info from SQLite."""
        async with self._lock:
            try:
                if not self.db:
                    await self.connect()
                if model_name:
                    async with self.db.execute(
                        "SELECT model_name, quota_metric, quota_value, retry_delay_sec, is_daily, last_exhausted_at, updated_at FROM model_quota_limits WHERE model_name = ?",
                        (model_name,)
                    ) as cur:
                        row = await cur.fetchone()
                        if row:
                            return {
                                ...
                                "quota_metric": row[1],
                                "quota_value": row[2],
                                "retry_delay_sec": row[3],
                                "is_daily": bool(row[4]),
                                "last_exhausted_at": str(row[5]),
                                "updated_at": str(row[6])
                            }
                        return {}
                else:
                    async with self.db.execute(
                        "SELECT model_name, quota_metric, quota_value, retry_delay_sec, is_daily, last_exhausted_at, updated_at FROM model_quota_limits"
                    ) as cur:
                        rows = await cur.fetchall()
                        return [
                            {
                                ...
                                "retry_delay_sec": r[3],
                                "is_daily": bool(r[4]),
                                "last_exhausted_at": str(r[5]),
                                "updated_at": str(r[6])
                            }
                            for r in rows
                        ]
            except Exception as e:
                logger.warning(f"Error reading model quota limits: {str(e)}")
                return {} if model_name else []
