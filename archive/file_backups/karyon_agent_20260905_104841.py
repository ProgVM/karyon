...
class CoREAgent(nn.Module):
    ...
    def generate_thought_and_speech(
        ...
        for step in range(max_generated_tokens):
            ...
            # sharpens Hopfield attractor relaxation beta and resets the refractory trace
            if entropy_val > 0.65:
                # Phasic ERPR: Sharpen attractor commitment to snap into clean concept basin
                h_relaxed, _ = self.attractor_head.relax_to_minima(h_flat, effective_hu_st, beta_override=18.0)
                raw_logits = self.volitional_head.compute_volitional_logits(h_relaxed, effective_hu_st, self.pos_embeddings.byte_embed.weight)
