...
from karyon_agent_runtime.db_manager import AgentDBManager, clean_for_json
...
class KaggleCoREAgent:
    ...
    async def initialize(self):
        ...
        self.multi_agent_manager = MultiAgentManager(self, self.db)
        try:
    ...
    async def execute_cortical_swarm_cycle(
        ...
        """
        Executes a 4-Stage Distributed Cortical Swarm Pipeline (Principle 2 Laminar Hierarchy):
        Stage 1 (Researcher / Layer IV): Hypothesis & arXiv Literature Priors
        Stage 2 (Coder / Layer V): Isolated Benchmark Implementation
        Stage 3 (Refactorer / Layer II/III): Principle 9 Dual-Refactoring Audit
        Stage 4 (Critic / Apical Dendrites): Telemetry Validation against KEP Rule #2 & Merge Verdict
        """
        ...
        # STAGE 1: RESEARCHER
        if status_callback:
            await status_callback("info", f"🧠 **[Swarm Cycle #{self.autonomous_cycle_count} | Stage 1/4]** Researcher exploring arXiv priors & formulating hypothesis...")

        stage1_prompt = (
            f"=== SWARM RESEARCH CYCLE #{self.autonomous_cycle_count} - STAGE 1: HYPOTHESIS & LITERATURE PRIORS ===\n"
            f"Current Empirical State:\n{latest_status}\n\n"
            f"Operator Strategic Directive:\n{base_agenda or 'Advance Karyon-CoRE biophysical realism and throughput under KEP v9.0.'}\n\n"
        ...
        # STAGE 2: CODER
        if status_callback:
            await status_callback("info", f"⚙️ **[Swarm Cycle #{self.autonomous_cycle_count} | Stage 2/4]** Coder authoring isolated benchmark script...")
        ...
        # STAGE 3: REFACTORER
        if status_callback:
            await status_callback("info", f"🔍 **[Swarm Cycle #{self.autonomous_cycle_count} | Stage 3/4]** Refactorer auditing code for Principle 9 compliance...")
        ...
        # STAGE 4: CRITIC
        if status_callback:
            await status_callback("info", f"📊 **[Swarm Cycle #{self.autonomous_cycle_count} | Stage 4/4]** Critic running KEP pipeline & evaluating telemetry...")
        ...
        synthesis_report = (
            f"## 🧬 Cortical Swarm Cycle #{self.autonomous_cycle_count} Completed\n\n"
        ...
        # Save model turn
        content_obj = types.Content(role="model", parts=[types.Part.from_text(text=synthesis_report)])
        await self.db.save_turn("model", synthesis_report, content_obj=content_obj)
        ...
        while self.autonomous_loop_active and self.autonomous_cycle_count < self.autonomous_max_cycles:
            ...
            try:
                ...
                if is_failed_response:
                    ...
                    self.autonomous_cycle_count = max(0, self.autonomous_cycle_count - 1)
                    backoff_wait = min(120.0, 10.0 * (1.5 ** min(self.autonomous_consecutive_failures, 4)))
                    self.update_heartbeat("BACKOFF_WAIT", f"Backing off {backoff_wait:.0f}s before cycle #{self.autonomous_cycle_count+1}...")
                    if status_callback:
                        await status_callback("info", f"⏳ Backing off for {backoff_wait:.0f}s before recovering next autonomous cycle...")
    ...
    async def process_user_query(
        ...
        try:
            ...
            await self.db.save_turn("user", user_text, content_obj=user_content)
            ...
            for turn in range(max_turns):
                ...
                for attempt in range(max_api_retries):
                    ...
                    # Single-point client acquisition (NO double-advance on retries!)
                    if attempt == 0 or self.key_manager.is_key_exhausted(self.key_manager.current_key_index):
                    ...
                    try:
                        ...
                        if err_code == 429 or "RESOURCE_EXHAUSTED" in err_msg.upper():
                            if event_callback:
                                await event_callback("info", f"Rate limit 429 on Key #{self.key_manager.current_key_index+1} ({active_model}). Failover to next key/model...")
                            ...
                            active_model = self.key_manager.get_model()
                            pacing_sleep = 0.5 + 0.1 * min(attempt, 5)
                        ...
                        elif err_code == 400 and any(w in err_msg.lower() for w in ["function response", "function call", "role", "thinking", "budget"]):
                            logger.warning("400 alignment or thinking config error. Re-sanitizing contents and retrying...")
                            contents = sanitize_and_align_contents(contents)
