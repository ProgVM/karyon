...
def generate_imagined_trajectory(agent: CoREAgent, hu: HomeostaticUnit, episodic_mem: BatchedEpisodicMemory, batch_size: int, seq_len: int):
    """
    Generates a linguistically coherent imagined thought sequence by seeding
    the model with a real prefix from episodic memory and letting it autoregressively
    roll out the continuation (Mental Sandbox).
    """
    device = agent.device
    with torch.no_grad():
        # 1. Sample a real prefix from our memory pool or fallback to a standard cognitive prompt
        # 'The following is a thought:'
        prefix_text = "The cognitive agent perceives "
        prefix_ids = list(prefix_text.encode('utf-8'))
        prefix_tokens = torch.tensor([prefix_ids], dtype=torch.long, device=device).expand(batch_size, -1)
        
        # 2. Autoregressively roll out seq_len tokens to form a coherent imagined trajectory
        ...
        curr_token = prefix_tokens
        for t in range(seq_len - prefix_tokens.size(1)):
            ...
            sal_gate = agent.boundary_detector(h_s1, curr_token[:, -1:])
            ...
            # Nucleus sampling over valid ASCII bytes to maintain coherence
            probs = torch.softmax(logits[:, 32:126] / 0.45, dim=-1)
            if probs.dim() > 2:
        ...
        imagined_seq = torch.cat(tokens, dim=1)
        return imagined_seq
