...
public:
    ...
    std::tuple<torch::Tensor, torch::Tensor> relax_to_minima(torch::Tensor h_state, torch::Tensor u_t) {
        ...
        // Normalized Tsodyks-Markram Synaptic Efficacy (Efficacy = R * u / U0 in [0, 1], Baseline = 1.0)
        auto tm_efficacy = (R_state * u_state / 0.20f).unsqueeze(0);
        
        // Calcium-activated Potassium AHP Fatigue Penalty
        auto ahp_penalty = 1.20f * torch::nan_to_num(ahp_trace, 0.0f).unsqueeze(0);
        
        auto fatigued_sim = sim * tm_efficacy - ahp_penalty;
        
        auto attn_weights = torch::softmax(fatigued_sim, -1);
