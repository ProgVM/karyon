...
class KeyManager:
    ...
    async def handle_quota_exhausted(
        ...
        # === CRITICAL FIX B: EXACT GEMINI RETRY DELAY & ZERO UNNECESSARY PACING ===
        ...
        logger.warning(
            ...
            f"Server retryDelay: {quota_meta.get('retry_delay_sec')}s. Consecutive 429: {self.consecutive_429_count}."
        )

        # 1. Immediate key rotation: attempt to switch instantly to another key that is non-cooldown
        next_key_idx = self.select_best_key(estimated_tokens=estimated_tokens, model=curr_model)
        if next_key_idx != -1:
            logger.info(f"⚡ Instant failover: Rotated Key #{prev_key_idx + 1} -> Key #{next_key_idx + 1} for '{curr_model}'.")
            self.current_key_index = next_key_idx
            self._init_client()
            return self._client

        # 2. Try proxy rotation if IP level rate-limit is suspected
        if proxy_rotator.has_any_healthy_proxy():
            current_p = proxy_rotator.get_healthy_proxy()
            proxy_rotator.mark_proxy_429(current_p, cooldown_sec=20.0)
            next_p = proxy_rotator.get_healthy_proxy()
            if next_p:
        ...
        # 3. All keys exhausted for curr_model: Sleep EXACTLY the retryDelay requested by Gemini API (or exact min remaining cooldown)
        exact_delay = quota_meta.get("retry_delay_sec")
        if exact_delay is not None and 0.0 < float(exact_delay) <= 120.0:
            sleep_wait = float(exact_delay) + 0.5
        else:
            cooldowns = [
                exp_t - now for (k_i, m_n), exp_t in self.exhausted_keys.items()
                if m_n == curr_model and exp_t > now
            ]
            sleep_wait = max(1.0, min(cooldowns)) + 0.5 if cooldowns else 5.0

        wait_msg = f"⏳ All keys in pool exhausted on '{curr_model}'. Sleeping EXACTLY {sleep_wait:.1f}s for Gemini API retryDelay..."
        logger.info(wait_msg)
        if event_callback:
            await event_callback("info", wait_msg)
        if agent and hasattr(agent, "update_heartbeat"):
            agent.update_heartbeat("RATE_PACING", f"Waiting exact API retryDelay {sleep_wait:.1f}s")

        await asyncio.sleep(sleep_wait)
        self.consecutive_429_count = 0

        # Try best key after sleep
        next_key_idx = self.select_best_key(estimated_tokens=estimated_tokens, model=curr_model)
        if next_key_idx != -1:
            self.current_key_index = next_key_idx
            self._init_client()
        ...
        # If still no key, rotate model cascade
        old_m = curr_model
        new_m = self.rotate_to_next_model(reason=f"All ready keys rate-limited on '{old_m}'")
        if new_m != old_m:
            cascade_alert = f"🔄 Cascading model '{old_m}' -> '{new_m}' to acquire fresh rate limits..."
