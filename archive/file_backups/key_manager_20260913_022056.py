...
def parse_gemini_error_cooldown(ex_or_str: Any) -> Tuple[float, bool]:
    ...
    1. Robustly unpacks nested dicts/lists/objects in `ex.details`.
    2. Correctly extracts `RetryInfo.retryDelay` (e.g. '21.439818s').
    3. Detects quota violations (RPD vs RPM/TPM).
    4. Distinguishes true daily quota limits (generateRequestsPerDay, limit: 20)
       from per-minute rate limits (RPM / TPM).
    """
    import re
    ...
    # Helper function to recursively inspect any object / dict / list
    def _inspect_node(node):
        nonlocal is_daily, retry_delay_val
        if isinstance(node, dict):
            t_url = str(node.get("@type", "") or node.get("typeUrl", "")).lower()
            if "retryinfo" in t_url:
            ...
            if "retryDelay" in node:
                try:
                    retry_delay_val = float(str(node["retryDelay"]).strip().rstrip("s"))
                except (ValueError, TypeError):
                    pass
    ...
    if retry_delay_val is not None and retry_delay_val > 0:
        ...
        logger.info(f"Server-specified rate limit delay: {retry_delay_val:.2f}s (applied cooldown: {cooldown:.2f}s)")
...
class GeminiKeyManager:
    ...
    def select_best_key(self, estimated_tokens: int = 35000, model: Optional[str] = None) -> int:
        ...
        # Prioritize round-robin progression across interleaved accounts/projects
    ...
    async def handle_quota_exhausted(
        ...
        logger.warning(
            ...
            f"hit 429 [{rate_type_str}] on '{curr_model}'. Consecutive 429 count: {self.consecutive_429_count}."
        )

        # MANDATORY ALLASTATIC PACING DELAY:
        # When consecutive keys receive 429 within a short window, or when server specifies a retryDelay,
        # DO NOT bombard the next key immediately. Sleeping for the required delay prevents the shared Kaggle IP
        # from instantly exhausting all 36 keys in rapid succession.
        if not is_daily:
            # If server requested a delay or if we have consecutive 429s across keys (shared IP rate limit)
            if cooldown_sec > 1.0 or self.consecutive_429_count >= 2:
                # Progressive adaptive backoff: min 2.0s, capped at server cooldown or 10.0s
                sleep_pace = min(cooldown_sec, max(2.5, 1.5 * self.consecutive_429_count))
                pace_msg = (
                    f"🛑 Pacing throttle active: Backing off for {sleep_pace:.1f}s "
                    f"to prevent IP/key cascade storm before trying next key..."
                )
                logger.info(pace_msg)
                if event_callback:
                    try:
                        await event_callback("info", pace_msg)
                    except Exception:
                        pass
                if agent and hasattr(agent, "update_heartbeat"):
                    agent.update_heartbeat("RATE_PACING", f"Sleeping {sleep_pace:.1f}s for IP quota pacing...")

                p_elapsed = 0.0
                while p_elapsed < sleep_pace:
                    chunk = min(1.0, sleep_pace - p_elapsed)
        ...
        # Require AT LEAST 15 keys exhausted (out of 36) before forcing model downgrade.
        if is_daily:
