...
class GeminiKeyManager:
    ...
    async def handle_quota_exhausted(
        ...
        # If consecutive keys hit 429 on this model (e.g. shared project quotas or global throttling),
        # fail over rapidly to the next model in the cascade instead of exhaustively iterating all 36 keys.
        if is_daily:
            model_switch_threshold = 2
        else:
            model_switch_threshold = min(4, len(self.keys))
