...
class CoREAgent(nn.Module):
    def __init__(self, config, device='cpu'):
        ...
        self.v1_entropy_gate = EntropyMacroGating(self.hidden_dim, vocab_size=self.text_gen_dim, device_str=self.device_str)
        self.v2_thalamic_router = ThalamocorticalGate(self.hidden_dim, homeo_dim=config.net.homeo_dim, device_str=self.device_str)
        self.v3_fast_hebbian = FastWeightHebbianPlasticity(self.hidden_dim, device_str=self.device_str)
        self.v4_predictive_residual = PredictiveResidualRouting(self.hidden_dim, homeo_dim=config.net.homeo_dim, device_str=self.device_str)
