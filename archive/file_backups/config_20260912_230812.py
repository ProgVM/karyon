...
# Flagship Gemini 3.x Model Matrix (Guaranteed 3.x Flash Family Only)
_raw_models = (
...
# Guarantee that 3.x fallback models exist in the cascade
for _fallback_m in ["gemini-3.7-flash", "gemini-3.5-flash"]:
    if _fallback_m not in _parsed_models:
        _parsed_models.append(_fallback_m)
GEMINI_MODELS = _parsed_models

# Dedicated Resilient Summarizer Models Pool (3.x Family Only)
_raw_sum_models = (
...
for _fallback_sm in ["gemini-3.7-flash", "gemini-3.5-flash"]:
    if _fallback_sm not in _parsed_sum_models:
        _parsed_sum_models.append(_fallback_sm)
SUMMARIZER_MODELS = _parsed_sum_models
SUMMARIZER_MODEL = SUMMARIZER_MODELS[0] if SUMMARIZER_MODELS else "gemini-3.8-flash"
