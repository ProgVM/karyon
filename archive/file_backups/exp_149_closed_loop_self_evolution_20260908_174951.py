...
def run_evolution_benchmark():
    ...
    agent_info = load_karyon(agent, kcore_path, device=device_str)
    agent.eval()
    ...
    episodic_mem = BatchedEpisodicMemory(batch_size=1, max_capacity=128, dim=agent.unified_dim, device_str=device_str)
    ...
    with torch.no_grad():
        test_tokens = torch.randint(0, 256, (1, 16), device=device)
        test_emb = expanded_agent.pos_embeddings(test_tokens, start_pos=0, apply_rf=False)
        s_in = {
            'text': test_emb.squeeze(0),
            'vision': torch.zeros(16, expanded_agent.config.net.vision_dim, device=device),
            'motor_efference': torch.zeros(16, expanded_agent.config.net.action_dim, device=device)
        }
