...
def run_morphogenesis_experiment():
    ...
    agent = ZeroStateLegoAgent(vocab_size=258, dim=256, num_blocks=3).to(device) # Expand to dim=256 for capacity
    dataset = NestedSyntaxDataset(vocab_size=258, seq_len=24) # Slightly shorter sequence to accelerate learning
    ...
    for epoch in range(25): # Increase to 25 epochs for deeper convergence
        agent.train()
        epoch_loss = 0.0
        num_batches = 50
