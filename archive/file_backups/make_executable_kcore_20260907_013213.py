...
LOADER_SCRIPT = """#!/usr/bin/env bash
_SELF="$0"
...
magic_offset = data.find(b"KCORE\\x05\\x00\\x00")
if magic_offset == -1:
    magic_offset = data.find(b"KCORE")
...
def make_executable(filepath="karyon_soul.kcore"):
    ...
    idx = data.find(b"KCORE\x05\x00\x00")
    if idx == -1:
        idx = data.find(b"KCORE")
    if idx == -1:
