...
def run_experiment():
    ...
    with torch.no_grad():
        ...
        # episodic_sensory_proj maps text_dim (256) -> unified_dim (256)
    ...
    # Test Memory Read Directly
    with torch.no_grad():
        q_test = agent.episodic_sensory_proj(fact_embs.mean(dim=1))
        ret_val, max_sim = memory.read(q_test, temperature=0.05, threshold=0.30, sigmoid_beta=10.0)
        sim_score = max_sim.item()
        print(f"\nDirect Memory Retrieval Cosine Similarity: {sim_score:.4f}")

    print(f"3-Gram Lexical Diversity: {div_score:.4f}")

    metrics = {
        "memory_items": memory.size.item(),
        "3gram_diversity": div_score,
        "memory_retrieval_similarity": sim_score,
        "noradrenaline_arousal": hu.state[0, 4].item()
    }

    verdict = "🟢 POSITIVE" if (div_score > 0.40 and sim_score > 0.90) else ("⚪ NEUTRAL" if sim_score > 0.70 else "🔴 REJECTED")
