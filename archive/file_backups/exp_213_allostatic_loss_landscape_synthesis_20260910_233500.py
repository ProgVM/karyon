...
Replacing the static loss weights with an Allostatically-Modulated Loss Synthesis Engine:
  w_loss(u_t) = w_base * (1.0 + 0.20 * tanh(W_loss * u_t))
...
def run_benchmark():
    ...
    def forward_multimodal_sequence_with_allostatic_loss(
        ...
        full_h_in, modal_att = brain_prop.gateway(unrolled_inputs, curr_u_t, seq_len)
        ...
        # Latent World Model Active Inference
        z_t, mu_post, logvar_post, mu_prior, logvar_prior, fe_loss_tensor = brain_prop.world_model(
            h_relaxed.view(batch_size, seq_len, -1)[:, -1, :],
            h_s1.mean(dim=1)
        )
