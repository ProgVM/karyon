...
class Vector3FastWeightHebbian(nn.Module):
    ...
    def forward(self, h_seq: torch.Tensor, u_t: torch.Tensor):
        ...
        # Decay matrix: L_{i, j} = lambda^(i - j) for i >= j
        ...
        # Fast weight interaction: Y_fast_i = sum_{j <= i} lambda^(i-j) * eta * (K_j^T Q_i) * V_j
...
def benchmark_variant(variant_name: str, agent: CoREAgent, hu: HomeostaticUnit, 
                      variant_modules: dict, input_stream: torch.Tensor, 
                      steps: int = 25, device_str: str = 'cuda'):
    
    device = torch.device(device_str)
    optimizer = torch.optim.AdamW(agent.parameters(), lr=3e-4, weight_decay=1e-2)
...
def main():
    ...
    hidden_dim = config.network.hidden_dim # 768
    
    # Instantiate Agent Baseline
    agent = CoREAgent(config=config, device=device_str).to(device_str)
    
    results = []
    
    # 1. Baseline
    logger.info("\n--- Benchmarking Configuration 0: Baseline (Master v33.0) ---")
    res_base = benchmark_variant("0. Baseline (Master v33.0)", agent, hu, {}, input_stream, steps=20, device_str=device_str)
    ...
    res_v1 = benchmark_variant("1. Vector 1 (Entropy Gating)", agent, hu, {'v1': v1_mod}, input_stream, steps=20, device_str=device_str)
    ...
    res_v2 = benchmark_variant("2. Vector 2 (Thalamocortical Gate)", agent, hu, {'v2': v2_mod}, input_stream, steps=20, device_str=device_str)
    ...
    res_v3 = benchmark_variant("3. Vector 3 (Fast-Weight Hebbian)", agent, hu, {'v3': v3_mod}, input_stream, steps=20, device_str=device_str)
    ...
    res_v4 = benchmark_variant("4. Vector 4 (Predictive Error Residuals)", agent, hu, {'v4': v4_mod}, input_stream, steps=20, device_str=device_str)
    ...
    v_all = {'v1': v1_mod, 'v2': v2_mod, 'v3': v3_mod, 'v4': v4_mod}
    res_grand = benchmark_variant("5. Grand Synthesis (All 4 Vectors)", agent, hu, v_all, input_stream, steps=20, device_str=device_str)
