...
===============================================================================
Hypothesis:
Coupling online stream adaptation (brief 25-step spatial sensory & world model grounding)
with System 2 Parallel Mental Sandbox (parallel_rollout_search) in CS-World will:
  1. Allow Karyon to dynamically adapt its Motor Gateway and World Model to the 2D spatial environment.
  2. Demonstrate significant survival advantage and Free Energy reduction for System 2 counterfactual look-ahead over System 1.
===============================================================================
"""
...
def quick_spatial_grounding_adaptation(agent, device='cpu', steps=25):
    """
    Brief online stream adaptation (25 steps) to align Motor Gateway and World Model
...
def run_survival_evaluation(agent, mode="system1", max_steps=50, device="cpu"):
    ...
    for step in range(max_steps):
        ...
        if mode == "system2":
            # System 2 Sandbox look-ahead with weighted counterfactual guidance
            best_h, min_efe, dur_ms = agent.world_model.parallel_rollout_search(h_slow, w_t, 3)
            h_slow_input = h_slow + 0.30 * best_h
        else:
            h_slow_input = h_slow
...
def run_experiment():
    ...
    quick_spatial_grounding_adaptation(agent, device=device, steps=25)
    agent.eval()
    ...
    # Positive if FE is reduced and survival/health is non-degraded
    is_positive = (
        sys2_metrics['cumulative_fe'] < sys1_metrics['cumulative_fe'] and
