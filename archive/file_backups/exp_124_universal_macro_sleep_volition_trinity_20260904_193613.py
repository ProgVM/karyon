...
def run_experiment():
    ...
    # C. Continuous Robotics Motor Efference / Bio-Signal Dynamics
    motor_signal = torch.randn(1, agent.unified_dim, device=device_str)
    h_mot, fe_mot, loss_mot, ms_mot = cognitive_engine.process_universal_stream('motor', motor_signal, hu, episodic_mem)
    print(f"[Modality: Motor Efference] Processed physical dynamic signal in {ms_mot:.2f}ms | FE: {fe_mot:.4f}")
