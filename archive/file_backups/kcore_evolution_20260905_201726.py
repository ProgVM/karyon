...
class Net2NetMorphogenesisEngine:
    ...
    def expand_agent_dimensions(
        ...
        w_t = torch.tensor([[65]], device=device)
        ...
        agent.eval()
        with torch.no_grad():
            out_before = agent.forward(w_t, h_fast, h_slow, u_t)
        ...
        with torch.no_grad():
            ...
            if hasattr(agent, 'world_model') and hasattr(agent.world_model, 'latent_predictor'):
                lp = agent.world_model.latent_predictor
                if hasattr(lp, 'prior_mu'):
                    lp.prior_mu = Net2NetMorphogenesisEngine.expand_linear_layer(
                        lp.prior_mu, lp.prior_mu.out_features, new_hidden_dim, device=device
                    )
                    lp.prior_logvar = Net2NetMorphogenesisEngine.expand_linear_layer(
                        lp.prior_logvar, lp.prior_logvar.out_features, new_hidden_dim, device=device
                    )
            ...
            if hasattr(agent, 'attractor_head'):
                if hasattr(agent.attractor_head, 'q_proj'):
                    agent.attractor_head.q_proj = Net2NetMorphogenesisEngine.expand_linear_layer(
                        agent.attractor_head.q_proj, agent.attractor_head.q_proj.out_features, new_hidden_dim, device=device
                    )
                if hasattr(agent.attractor_head, 'out_proj'):
                    agent.attractor_head.out_proj = Net2NetMorphogenesisEngine.expand_linear_layer(
                        agent.attractor_head.out_proj, new_hidden_dim, agent.attractor_head.out_proj.in_features, device=device
                    )

            # 4. Motor Gateway expansion
            if hasattr(agent, 'output_gateway'):
                if hasattr(agent.output_gateway, 'motor_proj'):
                    agent.output_gateway.motor_proj = Net2NetMorphogenesisEngine.expand_linear_layer(
                        agent.output_gateway.motor_proj, agent.output_gateway.motor_proj.out_features, new_hidden_dim, device=device
                    )
        ...
        with torch.no_grad():
            out_after = agent.forward(w_t, h_fast_new, h_slow_new, u_t)
