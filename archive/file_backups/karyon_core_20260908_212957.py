...
if karyon_cpp is None:
    for path in [
        ...
        "/root/.cache/torch_extensions/py312_cu128/karyon_cpp_ext_v36",
        "/root/.cache/torch_extensions/py312_cu128/karyon_cpp_ext_v35"
    ]:
        if os.path.exists(path) and path not in sys.path:
            sys.path.insert(0, path)
            
    for candidate_name in ["karyon_cpp_ext_v36"]:
        try:
...
if karyon_cpp is None:
    print("[C++ JIT] Compiling and linking native Karyon C++20 architecture (v36.0 Master with Energy-Dependent Synaptic Recovery)...")
    try:
        karyon_cpp = load(
            name="karyon_cpp_ext_v36",
            sources=["karyon_core.cpp"],
            extra_cflags=["-O3", "-std=c++20"],
            verbose=False
        )
        sys.modules["karyon_cpp_ext_v36"] = karyon_cpp
        print("[C++ JIT] Native C++20 v36.0 Master architecture successfully compiled and initialized!")
    except Exception as e:
        if "already registered" in str(e):
            print("[C++ JIT] PyBind11 type registration conflict detected. Attempting fallback import...")
            for candidate_name in ["karyon_cpp_ext_v36", "karyon_cpp_ext_v35"]:
                try:
                    mod = importlib.import_module(candidate_name)
                    if _is_valid_karyon_cpp_module(mod):
