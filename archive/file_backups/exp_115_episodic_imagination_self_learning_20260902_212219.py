...
def run_experiment():
    ...
    for txt in real_texts[:4]:
        ...
        if raw_b.size(1) > 0:
            emb = agent_base.pos_embeddings(raw_b).mean(dim=1)
            val = torch.randn(1, 256, device=device_str)
    ...
    # Pre-populate memory identically
    for txt in real_texts[:4]:
        raw_b = torch.tensor([list(txt[:128].encode('utf-8'))], dtype=torch.long, device=device_str)
        if raw_b.size(1) > 0:
            emb = agent_prop.pos_embeddings(raw_b).mean(dim=1)
            val = torch.randn(1, 256, device=device_str)
            for b_i in range(batch_size):
