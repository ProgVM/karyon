...
Hypothesis:
  ...
  zero-weight Net2Net Smooth Grafting alpha_epi=0.0) based on allostatic arousal (NA > 0.40)
...
def create_synthetic_nlp_stream(num_batches=30, batch_size=2, seq_len=64, vocab_size=256, device='cuda'):
    ...
    for b in range(num_batches):
        ...
        # Add surprise shift in the second half of stream
        if b % 6 == 0 and b > 0:
...
def run_experiment():
    ...
    opt_baseline = optim.AdamW(agent_baseline.get_all_parameters(), lr=1e-3, weight_decay=0.01)
    ...
    opt_dynamic = optim.AdamW(agent_dynamic.get_all_parameters(), lr=1e-3, weight_decay=0.01)
    ...
    for step, (inp, tgt) in enumerate(stream_data):
        ...
        # Allostatic trigger calculation
        ...
        if (na_val > 0.35 or fe_delta > 0.05) and len(agent_dynamic.dynamic_graph.bricks) < agent_dynamic.dynamic_graph.max_bricks:
            ...
            agent_dynamic.dynamic_graph.sprout_brick(brick_type)
            sprouted_events += 1
            ...
            # Reconstruct optimizer to include new sprouted parameters cleanly
            opt_dynamic = optim.AdamW(agent_dynamic.get_all_parameters(), lr=1e-3, weight_decay=0.01)
        ...
        if step == 15:
            ...
            # Simulate low-viability for weak paths
            if len(agent_dynamic.dynamic_graph.alpha_epi) > 2:
                with torch.no_grad():
                    agent_dynamic.dynamic_graph.alpha_epi[-1].copy_(torch.tensor(0.0001))
