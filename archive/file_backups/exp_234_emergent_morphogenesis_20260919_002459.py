...
class SproutedBlock(nn.Module):
    ...
    def __init__(self, dim: int, block_id: int):
        ...
        # Epigenetic gates initialized to -3.0 (near-zero shock, allows gradient flow)
        self.epi_gates = nn.ParameterDict({
            name: nn.Parameter(torch.ones(1) * -3.0) for name in self.ops.keys()
        })
...
def run_morphogenesis_experiment():
    ...
    # Optimizer
    optimizer = torch.optim.AdamW(agent.parameters(), lr=0.005, weight_decay=0.01)
    ...
    final_loss = losses[-1]
    assert final_loss < 0.50, f"Morphogenesis failed to achieve low prediction error! Loss: {final_loss:.4f}"
