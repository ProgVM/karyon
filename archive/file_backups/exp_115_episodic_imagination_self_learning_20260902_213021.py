...
def generate_imagined_trajectory(agent: CoREAgent, hu: HomeostaticUnit, episodic_mem: BatchedEpisodicMemory, batch_size: int, seq_len: int):
    ...
    with torch.no_grad():
        ...
        # ASCII character byte subset: letters, digits, spaces (32..126)
        valid_ascii_bytes = torch.arange(32, 127, device=device)

        # Query episodic memory using current homeostatic context
        ...
        if active_slots > 0:
            ret_trace, _ = episodic_mem.read(q_imagination, temperature=0.05, threshold=0.15, sigmoid_beta=10.0)
            seed_latent = ret_trace * (0.6 + 0.4 * lc_gain)
            seed_logits = seed_latent @ agent.pos_embeddings.byte_embed.weight.t()
            # Constrain to valid ASCII range
            seed_sub = torch.argmax(seed_logits[:, 32:127], dim=-1)
            seed_tokens = (seed_sub + 32).unsqueeze(1) # (B, 1)
        else:
            seed_tokens = torch.randint(65, 91, (batch_size, 1), device=device)

        # Build trajectory around episodic seed with structured syntax variations
        noise_offset = torch.randint(-5, 6, (batch_size, seq_len), device=device)
        trajectory_tokens = torch.clamp(seed_tokens + noise_offset, min=32, max=126)
        
        # Ensure proper space boundaries (word boundaries) every ~5-8 bytes
        for i in range(4, seq_len, 6):
            trajectory_tokens[:, i] = 32
