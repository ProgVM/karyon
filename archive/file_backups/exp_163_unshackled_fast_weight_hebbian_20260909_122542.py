...
def benchmark_associative_recall(hebbian_module, device_str='cuda:0'):
    """
    Measures associative recall accuracy on key-value pairs stored in sequence context.
    """
    ...
    # Generate synthetic sequence with repeated key-value patterns
    ...
    # Measure signal-to-noise ratio / magnitude of retrieved fast weights
...
def main():
    ...
    seq_t = torch.tensor([prompt_ids], dtype=torch.long, device=hw.device)
    u_t = torch.tensor([[0.85, 1.0, 0.90, 1.0, 0.10, 0.20]], device=hw.device)
    
    with torch.no_grad():
        tot_loss, speech_loss, fe_loss, _, _, _, _ = brain.forward_sequence(seq_t, u_t)
        base_s1 = brain.fused_stack(brain.in_proj(brain.gateway({'text': brain.pos_embeddings(seq_t)[0].view(seq_t.size(1), -1)}, torch.zeros(seq_t.size(1), 768, device=hw.device), u_t.repeat(seq_t.size(1), 1))[0].view(1, seq_t.size(1), -1)), torch.zeros(1, brain.num_heads, brain.head_k, brain.head_v, device=hw.device), torch.zeros(1, brain.num_heads, brain.head_k, brain.head_v, device=hw.device), u_t, seq_t)[0]
        base_y_fast = baseline_hebbian(base_s1, u_t)
        _, base_err_mag = brain.predictive_residual_router(base_s1[:, -1:, :], base_s1[:, -1:, :], u_t)
        base_err_mag_val = float(base_err_mag.item())

    logger.info(f"Baseline Fast-Weight Signal Norm : {base_signal:.4f}")
    logger.info(f"Baseline Predictive Error Mag    : {base_err_mag_val:.6f}")
    ...
    with torch.no_grad():
        tot_loss_p, speech_loss_p, fe_loss_p, _, _, _, _ = brain.forward_sequence(seq_t, u_t)
        prop_s1 = brain.fused_stack(brain.in_proj(brain.gateway({'text': brain.pos_embeddings(seq_t)[0].view(seq_t.size(1), -1)}, torch.zeros(seq_t.size(1), 768, device=hw.device), u_t.repeat(seq_t.size(1), 1))[0].view(1, seq_t.size(1), -1)), torch.zeros(1, brain.num_heads, brain.head_k, brain.head_v, device=hw.device), torch.zeros(1, brain.num_heads, brain.head_k, brain.head_v, device=hw.device), u_t, seq_t)[0]
        prop_y_fast = proposed_hebbian(prop_s1, u_t)
        _, prop_err_mag = brain.predictive_residual_router(prop_s1[:, -1:, :], prop_s1[:, -1:, :], u_t)
        prop_err_mag_val = float(prop_err_mag.item())

    logger.info("\n>>> Proposed Unshackled Fast-Weight Hebbian Module Evaluation <<<")
    logger.info(f"Proposed Fast-Weight Signal Norm : {prop_signal:.4f}")
    logger.info(f"Proposed Predictive Error Mag    : {prop_err_mag_val:.6f}")
    ...
    summary = {
        ...
        "metrics": {
            ...
            "baseline_err_mag": base_err_mag_val,
            "proposed_err_mag": prop_err_mag_val
        }
    }
