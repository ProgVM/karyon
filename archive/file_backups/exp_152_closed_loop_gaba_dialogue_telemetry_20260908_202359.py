...
def main():
    ...
    for turn_idx, user_turn in enumerate(dialogue_script):
        ...
        # Execute turn via KaryonEntity (handles sensory perception, Free Energy calculation, GABA motor generation, and somatic state updates)
        if user_turn.strip() == "":
            logger.info(f"--- Turn {turn_idx + 1}: [Spontaneous Inner Monologue / Self-Initiated Thought] ---")
            result = entity.interact(user_text="", max_tokens=90)
        else:
            logger.info(f"--- Turn {turn_idx + 1}: Human: \"{user_turn}\" ---")
            result = entity.interact(user_text=user_turn, max_tokens=90)
        ...
        f_human = float(result.get("free_energy_surprise", 0.0))
        speech_out = result.get("speech", "").strip()

        logger.info(f"Karyon: \"{speech_out}\"")
        logger.info(f"📊 Telemetry | Latency: {t_lat:.1f}ms | F_human: {f_human:.4f} | Energy: {energy:.3f} | Health: {health:.3f} | Curiosity: {curiosity:.3f} | NA: {na:.3f} | DA: {da:.3f}\n")
    ...
    mean_f_human = sum(r["f_human_surprise"] for r in telemetry_records) / len(telemetry_records)
    ...
    logger.info(f"⚡ Mean F_human Surprise   : {mean_f_human:.4f}")
    ...
    summary = {
        ...
        "verdict": verdict,
        "metrics": {
            "mean_f_human_surprise": mean_f_human,
