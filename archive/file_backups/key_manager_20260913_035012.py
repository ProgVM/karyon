...
"""
===============================================================================
36-KEY DYNAMIC POOL MANAGER & 250K TPM 36-PROJECT CROSS-ROTATION ENGINE (v32.0)
...
Persistence, Multi-Tier Cascading Model Failover (3.8 -> 3.7 -> 3.5 -> 2.5 -> 2.0),
...
def parse_gemini_error_cooldown(ex_or_str: Any) -> Tuple[float, bool, Dict[str, Any]]:
    ...
    1. Robustly unpacks nested dicts/lists/objects in `ex.details` and raw error string.
    2. Correctly extracts server-specified retry delays from `RetryInfo.retryDelay` (e.g. '7s', '21.439818s')
       or string messages ("Please retry in 7.064074475s").
    3. Detects quota violations (RPD vs RPM/TPM) and extracts quota metrics/values
       (e.g., generativelanguage.googleapis.com/generate_content_free_tier_requests, quotaValue: 20).
    4. Distinguishes true daily quota limits from per-minute rate limits (RPM / TPM).
    ...
    def _inspect_node(node):
        ...
        if isinstance(node, dict):
            ...
            # RetryInfo parsing
            if "retryinfo" in t_url or "retrydelay" in node:
                rd = node.get("retryDelay")
                if rd:
                    try:
                        retry_delay_val = float(str(rd).strip().rstrip("s"))
                    except (ValueError, TypeError):
                        pass

            # QuotaFailure parsing
            if "quotafailure" in t_url or "violations" in node:
                violations = node.get("violations", [])
                if isinstance(violations, list):
                    for v in violations:
                        if isinstance(v, dict):
                            q_m = v.get("quotaMetric")
                            q_i = v.get("quotaId")
                            q_v = v.get("quotaValue")
                            q_dims = v.get("quotaDimensions", {})
                            if q_m:
            ...
            _inspect_node(node.__dict__)

    details = getattr(ex_or_str, "details", None)
    if details:
        _inspect_node(details)

    # Regex extraction fallback for retryDelay if details object missing or non-dict
    if retry_delay_val is None:
        match = re.search(r'["\']?retryDelay["\']?\s*:\s*["\']?([0-9]+(?:\.[0-9]+)?)s?["\']?', ex_str, re.IGNORECASE)
        if match:
            try:
                retry_delay_val = float(match.group(1))
            except (ValueError, TypeError):
                pass

    if retry_delay_val is None:
        # Match pattern: "Please retry in 7.064074475s" or "retry in 12s" or "wait 15 seconds"
        text_matches = re.findall(
            r'(?:retry|wait|try again)\s+(?:in|after)?\s*([0-9]+(?:\.[0-9]+)?)\s*s?\b',
            ex_str,
            re.IGNORECASE
        )
        if text_matches:
            try:
                retry_delay_val = float(text_matches[0])
            except (ValueError, TypeError):
                pass

    # Regex extraction fallback for target_model if missing
    if target_model is None:
        tm_match = re.search(r'["\']?model["\']?\s*:\s*["\']?([a-zA-Z0-9\.\-_]+)["\']?', ex_str, re.IGNORECASE)
        if tm_match:
            target_model = tm_match.group(1)
        else:
            tm_text = re.search(r'model:\s*([a-zA-Z0-9\.\-_]+)', ex_str, re.IGNORECASE)
            if tm_text:
                target_model = tm_text.group(1)
    if quota_metric is None:
        qm_match = re.search(r'["\']?quotaMetric["\']?\s*:\s*["\']?([^"\',\s\}]+)["\']?', ex_str)
        if qm_match:
            quota_metric = qm_match.group(1)
        else:
            qm_text = re.search(r'Quota exceeded for metric:\s*([^\s,]+)', ex_str)
            if qm_text:
                quota_metric = qm_text.group(1)

    if quota_value is None:
        qv_match = re.search(r'["\']?quotaValue["\']?\s*:\s*["\']?([0-9]+)["\']?', ex_str)
        if qv_match:
            try:
                quota_value = int(qv_match.group(1))
            except ValueError:
                pass
        else:
            qv_text = re.search(r'limit:\s*([0-9]+)', ex_str)
            if qv_text:
                try:
                    quota_value = int(qv_text.group(1))
                except ValueError:
                    pass

    daily_indicators = [
        "generatecontentrequestsperday", "generaterequestsperday",
        "requests per day", "requests_per_day", "day-freetier"
    ]
    ...
    if retry_delay_val is not None and retry_delay_val > 0:
        # EXACT SERVER COMPLIANCE: Wait exact requested retryDelay + 1.0s safety buffer
        cooldown = max(2.0, retry_delay_val + 1.0)
        logger.info(f"Server-specified rate limit delay: {retry_delay_val:.2f}s (applied exact cooldown: {cooldown:.2f}s)")
        return cooldown, False, quota_meta

    return 15.0, False, quota_meta


class GeminiKeyManager:
    """
    36-Project Key Pool Coordinator with Multi-Tier Model Failover and RPM Throttling.
    Tracks quotas per (key_index, model) pair, persists discovered model quota limits into DB,
    and enforces exact server-specified retry delays.
    """
    ...
    def __init__(self, db_manager=None):
        ...
        # Discovered model limits cache (loaded from DB on init if available)
        ...
        # Adaptive IP / Burst Rate Pacing State
        ...
        # Load persisted model quota limits asynchronously if db exists
        if self.db:
            try:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    asyncio.create_task(self._load_persisted_quota_limits())
            except Exception:
                pass

    async def _load_persisted_quota_limits(self):
        """Loads discovered model limits from DB into active memory cache."""
        if not self.db:
            return
        try:
            records = await self.db.get_model_quota_limits()
            if isinstance(records, list):
                for rec in records:
                    m_name = rec.get("model_name")
                    if m_name:
                        self.discovered_model_limits[m_name] = rec
            logger.info(f"Loaded {len(self.discovered_model_limits)} model quota limit records from SQLite DB.")
        except Exception as e:
            logger.debug(f"Notice loading model quota limits from DB: {str(e)}")
    ...
    def select_best_key(self, estimated_tokens: int = 35000, model: Optional[str] = None) -> int:
        ...
        Validates both TPM headroom AND RPM headroom before selecting key.
        Uses discovered model limit info if available (e.g. RPD=20 or RPM limits).
        Prioritizes keys that have not been used recently to maximize inter-request gaps per project.
        """
        now = time.time()
        self._prune_sliding_windows(now)
        ...
        for step in range(len(self.interleaved_indices)):
            ...
            # Minimum quiet gap: avoid hitting the exact same key twice within 3.0s unless necessary
            if now - self.last_used_timestamps.get(cand_idx, 0.0) < 3.0 and len(self.keys) > 1:
                continue
        ...
        # 2. Secondary pass: any key not in cooldown with TPM and RPM headroom (ignoring 3.0s quiet gap)
        for step in range(len(self.interleaved_indices)):
            cand_idx = self.interleaved_indices[(self.schedule_cursor + step) % len(self.interleaved_indices)]
        ...
        # 3. Tertiary pass: any key not in cooldown for target_model with strict RPM ceiling
        for step in range(len(self.interleaved_indices)):
            cand_idx = self.interleaved_indices[(self.schedule_cursor + step) % len(self.interleaved_indices)]
        ...
        return -1  # All keys fully saturated or on cooldown for target_model
    ...
    async def get_ready_client_for_request(
        ...
        if best_idx != -1:
            self._init_client()
            # Pre-register request and token footprint immediately to prevent race conditions across parallel calls
    ...
    async def handle_quota_exhausted(
        ...
        if any(w in err_lower for w in ["401", "unauthenticated", "access_token_type_unsupported"]):
            ...
            # Permanently lock this key index across all models (far into future: 10 years)
        ...
        # Update discovered limits cache and persist to DB
        ...
        # Update consecutive 429 tracking for IP/burst pacing
        ...
        # Lock ONLY this key for this specific model for EXACT cooldown_sec
        ...
        # 1. IMMEDIATE FAILOVER CHECK:
        # If this model has other READY keys in the 36-key pool, switch INSTANTLY without multi-minute pauses!
        ...
        # CASCADING MODEL FAILOVER:
        # For true daily caps (RPD), failover quickly.
        # For transient RPM/TPM limits, do NOT failover easily if keys are merely waiting out brief cooldowns!
        if is_daily:
        ...
        # Exact server wait: find the minimum remaining cooldown among keys for target model
        relevant_cooldowns = [
