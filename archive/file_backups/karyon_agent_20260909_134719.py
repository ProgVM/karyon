...
class EntropyMacroGating(nn.Module):
    """
    Entropy-Driven Hierarchical Concept Gating (BLT-Neuro / Multi-timescale Macro-Pulse - EXP-178 Validated 🟢).
    Computes local Shannon entropy from Stage 1 representation and produces an allostatically modulated macro-boundary gate
    to scale Stage 2 semantic processing (KEP Principle 14 Compliant).
    """
...
class CoREAgent(nn.Module):
    ...
    def forward_multimodal_sequence(self, sensor_seq_dict: Dict[str, torch.Tensor], target_seq: torch.Tensor, hu_batch,
        ...
        with torch.amp.autocast(device_type=('cuda' if self.hardware.is_cuda else ('xla' if self.hardware.is_tpu else 'cpu')), dtype=self.hardware.get_autocast_dtype(), enabled=self.hardware.config.enable_amp and not self.hardware.is_cpu):
            ...
            # EXP-136 Neo-Cortical Quad-Vector Grand Synthesis & Allostatic Macro-Gating (EXP-178)
            entropy_s1, boundary_gate = self.entropy_macro_gate(h_s1, effective_u_t)
            da_t_exp = effective_u_t[:, 5:6].unsqueeze(1) if effective_u_t.dim() == 2 else effective_u_t[..., 5:6]
            curiosity_t_exp = effective_u_t[:, 0:1].unsqueeze(1) if effective_u_t.dim() == 2 else effective_u_t[..., 0:1]
            
            g_base = 0.40 + 0.30 * da_t_exp
            g_boundary = (0.80 + 1.20 * curiosity_t_exp) * boundary_gate.unsqueeze(-1)
            h_s2_gated = h_s2 * (g_base + g_boundary)
