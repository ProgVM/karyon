# experiments/exp_195_allostatic_multimodal_gateway.py
"""
===============================================================================
EXP-195: Allostatically-Modulated Multimodal Sensory Gateway & Dynamic Routing
Grounding: KEP Principle 2 (Biological Realism), Principle 14 (Allostatic Forces: No Static Constants).
===============================================================================
Hypothesis:
Replacing the static or partially linear multimodal sensory gateway projections with an
allostatically-modulated dynamic sensory gateway—where the projection weights and channel
...
logger = logging.getLogger("EXP-195")


class AllostaticMultimodalGateway(nn.Module):
    """
    Allostatically-modulated multimodal sensory gateway and dynamic routing.
    """
    def __init__(self, unified_dim: int, hidden_dim: int, device_str: str = 'cpu'):
        super().__init__()
...
def run_benchmark():
    logger.info("=" * 80)
    logger.info("🔬 [STARTING EXP-195: ALLOSTATIC MULTIMODAL GATEWAY BENCHMARK]")
    logger.info("=" * 80)
    ...
    # 1. Baseline Evaluation (EXP-194 Temporal Integration)
    ...
    # 2. Proposed Evaluation (EXP-195 Allostatic Multimodal Gateway)
    entity_prop = KaryonEntity.load("karyon_soul.kcore", device=hw.device_str)
    brain_prop = entity_prop.brain
    brain_prop.allostatic_multimodal_gateway = AllostaticMultimodalGateway(
        unified_dim=brain_prop.unified_dim, hidden_dim=brain_prop.hidden_dim, device_str=hw.device_str
    ).to(hw.device)
    
    logger.info("Running Proposed Allostatic Multimodal Gateway Evaluation...")
    ...
    logger.info("=" * 80)
    logger.info("📊 === EXP-195 TELEMETRY REPORT ===")
    ...
    with open("experiments/exp_195_results.json", "w") as f:
