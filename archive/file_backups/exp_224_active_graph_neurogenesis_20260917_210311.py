...
from karyon_agent import CoREAgent, CoREConfig
from karyon_hardware import HardwareEngine
from karyon_checkpoint import KaryonEntity
...
def run_experiment():
    ...
    agent = entity.agent
    agent.eval()
    logger.info("✅ KaryonEntity successfully loaded.")
    ...
    hu_st = torch.zeros(1, 6, device=device)
    hu_st[0, 1] = 0.8  # Energy
    hu_st[0, 2] = 0.7  # Stability

    with torch.no_grad():
        logits_before, fe_before = agent.forward_sequence(x, hu_st)[:2]
    ...
    with torch.no_grad():
        logits_after, fe_after = agent.forward_sequence(x, hu_st)[:2]

    delta_logits = torch.max(torch.abs(logits_after - logits_before)).item()
    delta_fe = abs(fe_after - fe_before)

    logger.info(f"Max Logit Delta at Birth t0: {delta_logits:.8f}")
    logger.info(f"Free Energy Delta at Birth t0: {delta_fe:.8f}")
    assert delta_logits == 0.0, f"FATAL: Net2Net zero identity violated! Delta = {delta_logits}"
