...
async def launch_widgets_ui(agent: KaggleCoREAgent):
    ...
    # Input area with attached files button
    input_row = widgets.HBox(
        [text_input, file_upload],
        layout=widgets.Layout(width='100%', align_items='center', margin='4px 0')
    )

    top_controls = widgets.VBox([btn_row, widgets.HBox([status_label])])
    ui_box = widgets.VBox([output_area, input_row, top_controls, config_accordion])
    display(ui_box)
