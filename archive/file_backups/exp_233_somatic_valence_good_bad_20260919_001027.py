...
class DamasioSomaticValenceEngine(nn.Module):
    ...
    def forward_candidate_evaluation(
        ...
        q_k = action_cand_emb
        ret_mem, max_sim = episodic_memory.read(q_k, temperature=0.05, threshold=0.15)
        
        somatic_marker_affect = self.affect_projector(ret_mem) * max_sim # Weighted by match similarity
