...
def run_experiment():
    ...
    num_attractors = agent_brain.attractor_head.attractor_basins.size(0)
    ...
    def habituated_relax_to_minima(h_state, u_t=None):
        ...
        scale = 1.0 / math.sqrt(agent_brain.hidden_dim)
        ...
        sim = torch.matmul(h_state, basins.transpose(0, 1)) * (scale * beta)
        
        # Biophysical Habituation: subtract visitation fatigue (synaptic depression)
        fatigue_penalty = 1.20 * visitation_trace.unsqueeze(0)
        habituated_sim = sim - fatigue_penalty
        
        attn_weights = F.softmax(habituated_sim, dim=-1)
        
        # Update visitation trace with passive GABA decay (0.80) + active accumulation
        with torch.no_grad():
            visitation_trace = 0.80 * visitation_trace + attn_weights.detach().mean(dim=0)
