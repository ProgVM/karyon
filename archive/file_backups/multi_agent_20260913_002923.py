        ...
        try:
            for turn in range(max_turns):
                ...
                for attempt in range(max_api_retries):
                    ...
                    if attempt == 0 or self.agent_core.key_manager.is_key_exhausted(self.agent_core.key_manager.current_key_index):
                        gemini_client = await self.agent_core.key_manager.get_ready_client_for_request(
                            estimated_tokens=current_tokens,
                            agent=self.agent_core
                        )
                    else:
                        gemini_client = self.agent_core.key_manager.get_client()
                    ...
                    try:
                        ...
                        self.agent_core.key_manager.record_request_tokens(self.agent_core.key_manager.current_key_index, current_tokens)
                        break
