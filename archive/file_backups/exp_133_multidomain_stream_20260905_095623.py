...
def build_multi_domain_stream(seq_len=1024, max_samples_per_domain=15000):
...
def run_experiment():
    ...
    h_fast_stream = torch.zeros(BATCH_SIZE, agent.hidden_dim, device=device)
    h_slow_stream = torch.zeros(BATCH_SIZE, agent.num_heads, agent.head_k, agent.head_v, device=device)
    ...
    for step, batch in enumerate(loader):
        ...
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            speech_loss, fe, h_fast_out, h_slow_out, action_indices = agent.forward_sequence(
                input_ids,
                target_ids,
                h_fast_stream,
                h_slow_stream,
        ...
        if fe_val > 0.005:
            scaler.scale(speech_loss + 0.10 * fe).backward()
        ...
        h_fast_stream = h_fast_out.detach()
        h_slow_stream = h_slow_out.detach()
        ...
        with torch.no_grad():
            ...
            if mean_energy < 0.25:
                # Sleep cycle: restore energy & downscale redundant synaptic traces (SHY hypothesis)
        ...
        if (step + 1) % 50 == 0 or (step + 1) == benchmark_batches:
            ...
            print(f"[Step {step+1:04d}/{benchmark_batches}] Loss: {loss_val:.4f} | PPL: {ppl:.2f} | FE: {fe_val:.5f} | Adapt: {total_adapted} | Sleep: {total_sleeps} | Speed: {tok_s:.0f} tok/s | VRAM: {vram:.1f} MB")
    ...
    h_fast_save = h_fast_stream[:1].detach()
    h_slow_save = h_slow_stream[:1].detach()
    save_karyon(agent, memory, hu, h_fast_save, h_slow_save, epoch=1, story_idx=benchmark_batches, filepath=kcore_path)

    metrics = {
        "final_speech_loss": loss_val,
        "final_ppl": math.exp(min(10.0, loss_val)),
    ...
    verdict = "🟢 POSITIVE" if (loss_val < 3.5 and overall_div > 0.40) else "⚪ NEUTRAL"
