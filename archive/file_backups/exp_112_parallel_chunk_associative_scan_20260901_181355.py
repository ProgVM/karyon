...
class ParallelChunkAssociativeSSDLayer(nn.Module):
    ...
    def forward(self, x_seq, m_prev, u_t, saliency_gate=None, dt=1.0):
        ...
        # Deterministic zero noise for equivalence test
...
def run_benchmark():
    ...
    layer_base = ParallelLogDecaySSDLayer(768, 768, 12, 64, 128, 0.005, 0.15, 64, device_str)
    ...
    # Copy weights from base to prop
    for p_base, p_prop in zip(layer_base.parameters(), layer_prop.parameters()):
        p_prop.data.copy_(p_base.data)
        
    # Warmup
    for _ in range(10):
        with get_autocast_ctx():
            _ = layer_base.forward(x, m, u)
            _ = layer_prop(x, m, u)
    ...
    with get_autocast_ctx():
        out_base, m_next_base, _ = layer_base.forward(x, m, u)
