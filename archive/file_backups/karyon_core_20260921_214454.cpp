...
public:
    ...
    void add_node(std::string name, std::string op_type, bool is_core = false, float initial_alpha = 0.0f) {
        ...
        if (old_k > 0) {
            torch::NoGradGuard no_grad;
            w_route.slice(0, 0, old_k).narrow(1, old_k, 1).fill_(-100.0f);
            w_route.narrow(0, old_k, 1).slice(1, 0, old_k).fill_(-100.0f);
            w_route.index_put_({old_k, old_k}, -100.0f);
        }
    }
    ...
    torch::Tensor forward(torch::Tensor x_sensory, int64_t thinking_steps = 4) {
        ...
        auto w_softmax = torch::softmax(active_w_route, -1);

        for (int64_t step = 0; step < thinking_steps; ++step) {
            auto aggregated_inputs = torch::einsum("ij,ibd->jbd", {w_softmax, node_states});
