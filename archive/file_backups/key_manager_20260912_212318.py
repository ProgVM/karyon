...
class GeminiKeyManager:
    ...
    async def handle_quota_exhausted(
        ...
        # Note: Do not switch model immediately on transient TPM 429s unless daily RPD is hit.
        # Instead, try remaining keys or enter brief async pause to let 60s TPM sliding window reset.
        if is_daily:
            model_switch_threshold = 3
        else:
            model_switch_threshold = len(self.keys)  # Only switch model when ALL keys hit 429
