...
"""
===============================================================================
EXP-279: Active Inference Variational Free Energy & Latent Attractor Equilibrium Reasoning
===============================================================================
Scientific Hypothesis:
  ...
  Energy collapse, and endogenous recurrent latent recirculation (System 2 thinking)—
  will enable Karyon-CoRE to converge on logical deduction tasks without pseudo-morphemic
  drift or cross-entropy artifact hacking.
...
Biophysical Equations:
  ...
  3. Latent Equilibrium / Thinking Depth: Iterative state recirculation until delta F_t < epsilon
===============================================================================
"""
import os
import sys
...
def compute_hopfield_lyapunov_energy(h_latent: torch.Tensor, basins: torch.Tensor, beta: float = 12.0) -> torch.Tensor:
    """
    ...
    # Cosine similarities: [B, S, D] @ [D, N] -> [B, S, N]
...
def compute_active_inference_kl(latent_pred: nn.Module, h_prev: torch.Tensor, h_curr: torch.Tensor) -> torch.Tensor:
    ...
    # Dimension-normalized Gaussian KL Divergence with numerical epsilon clamping
...
def run_benchmark():
    ...
    # 1. Initialize Sovereign Cognitive Components
    agent = CoREAgent(
        vocab_size=258,
        embed_dim=dim,
        num_cells=2,
    ...
    optimizer = torch.optim.AdamW(all_params, lr=0.0015, weight_decay=1e-4)

    # Extract Hopfield basins tensor from parameters
    hopfield_params = list(agent.hopfield.parameters())
    hopfield_basins = hopfield_params[0] # [num_basins, dim]

    print("\n🧠 Training Karyon via Variational Free Energy (F_t) Minimization (120 epochs)...")
    ...
    for epoch in range(120):
        ...
        for title, prompt, target in tasks:
            ...
            h_latent = agent.forward_latent(tokens) # [1, seq_len, dim]
        ...
        if (epoch + 1) % 30 == 0 or epoch == 0:
            ...
            print(f"  • Epoch {epoch+1:3d}/120 | Free Energy F_t: {final_fe:.4f} (KL: {total_kl.item()/len(tasks):.2f}, Hopfield_E: {total_energy.item()/len(tasks):.3f}, Recon_E: {total_recon.item()/len(tasks):.4f})")

    train_duration = time.perf_counter() - t0

    # 4. Latent Equilibrium Evaluation with Recurrent Thinking Relaxation
    print("\n🔍 Evaluating Logical Reasoning via Latent Attractor Snapping & Thinking Relaxation...")
    correct_count = 0
    ...
    with torch.no_grad():
        for title, prompt, expected in tasks:
            ...
            for _ in range(len(exp_bytes)):
                ...
                # Forward pass through morphic space & Hopfield attractor snapping
                logits = agent(inp)
            ...
            print(f"  {status} [{title:15s}] Expected: '{expected}' | Generated: '{predicted}'")
    ...
    print(f"  • Final Free Energy (F_120) : {final_fe:.4f}")
