...
# =============================================================================
# PROPOSED EXPERIMENTAL MODULE: FP16 Tensor Core Parallel SSD Layer
# =============================================================================
...
def run_benchmark():
    ...
    # Replace standard C++ SSD layers in stage1 and stage2 with FP16 Tensor Core SSD layers
    fp16_ssd_stage1 = TensorCoreParallelSSDLayer(
        in_dim=768, out_dim=768, num_heads=12, head_k=64, head_v=128,
        min_beta=0.005, max_beta=0.15, chunk_size=64, device_str=device_str
    ).to(device)
    fp16_ssd_stage2 = TensorCoreParallelSSDLayer(
        in_dim=768, out_dim=768, num_heads=12, head_k=64, head_v=128,
        min_beta=0.0001, max_beta=0.05, chunk_size=64, device_str=device_str
    ).to(device)
    
    agent_prop.stage1.ssd = fp16_ssd_stage1
    agent_prop.stage2.ssd = fp16_ssd_stage2
