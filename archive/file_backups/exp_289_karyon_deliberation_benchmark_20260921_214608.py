"""
===============================================================================
EXP-289: Pure Karyon-CoRE Recurrent Latent Deliberation vs Shallow Auto-Regressive
Testing:
1. Zero Transformer / Zero Attention: 100% C++20 Causal SSD + Dynamic Morphic Graph.
2. Principle of Delayed Motor Output: Deliberative Recurrent Cycles (K internal thinking steps)
   BEFORE committing to the motor gateway (speaking).
3. Out-of-Distribution (OOD) Algorithmic Logic (Sequence Reversal & Pointer Tracking).
===============================================================================
"""
...
print("=" * 80)
print("EXP-289: KARYON-CORE RECURRENT LATENT DELIBERATION (ZERO TRANSFORMER)")
print(f"Device: {device} | Backend: C++20 LibTorch (CausalParallelSSD + DynamicMorphicGraph)")
print("=" * 80)
...
print(f"Datasets Synthesized:")
print(f"  Training Instances (Short Length 3..5) : {len(train_samples)}")
print(f"  OOD Length Instances (Unseen 7..9)     : {len(ood_length)}")
print(f"  OOD Symbol Instances (Unseen Letters)  : {len(ood_symbols)}")
print(f"  Sample Train: '{train_samples[0][0]} -> {train_samples[0][1]}'")
print(f"  Sample OOD  : '{ood_length[0][0]} -> {ood_length[0][1]}'")

# ---------------------------------------------------------------------------
# 2. Pure Karyon-CoRE Deliberative Architecture
# ---------------------------------------------------------------------------

class KaryonDeliberativeAgent(nn.Module):
    def __init__(self, vocab_size=258, dim=128):
        ...
        # 2. Dynamic Morphic Recirculation Graph (Internal Thinking Space)
        self.graph = karyon_core.DynamicMorphicGraph(dim, str(device))
        
        # Biological functional primitive nodes:
...
def run():
    model = KaryonDeliberativeAgent(dim=128).to(device)
    ...
    print("=" * 80)
    print(f"Architecture                    : 100% C++20 Causal SSD + Morphic Graph")
