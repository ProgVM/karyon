...
def generate_imagined_trajectory(agent: CoREAgent, hu: HomeostaticUnit, episodic_mem: BatchedEpisodicMemory, batch_size: int, seq_len: int):
    ...
    and steered by Panksepp SEEKING drive and PAC Morphemic Decoding.
    """
    device = agent.device
    with torch.no_grad():
        ...
        # Common readable ASCII character byte subset: letters (65-90, 97-122), digits (48-57), space (32), newline (10)
        valid_ascii_bytes = torch.tensor([32, 10] + list(range(65, 91)) + list(range(97, 123)), device=device)

        # 1. Sample cognitive query vector from high-level latent attractor space
        u_t = hu.state
        q_imagination = agent.episodic_sensory_proj(agent.pos_embeddings.byte_embed.weight[valid_ascii_bytes].mean(dim=0, keepdim=True).expand(batch_size, -1))
        
        # 2. Retrieve episodic anchor if memory has active traces
        active_slots = getattr(episodic_mem, 'max_active_cpu', 0)
        if active_slots > 0:
            ret_trace, _ = episodic_mem.read(q_imagination, temperature=0.05, threshold=0.20, sigmoid_beta=10.0)
            seed_latent = ret_trace * (0.6 + 0.4 * lc_gain)
            seed_logits = seed_latent @ agent.pos_embeddings.byte_embed.weight.t()
            # Constrain to valid readable bytes
            seed_sub = torch.argmax(seed_logits[:, valid_ascii_bytes], dim=-1)
            seed_token_ids = valid_ascii_bytes[seed_sub]
        else:
            # Fallback to capital letter start
            seed_token_ids = torch.randint(65, 91, (batch_size,), device=device)
        ...
        for t in range(seq_len):
            ...
            # Entropy-Adaptive PAC decoding:
            # High entropy / word boundary -> higher temperature for conceptual choice
            # Low entropy / inside morpheme -> strict low temperature (MAP)
            pred_entropy = agent.entropy_predictor(h_s1).mean(dim=-1, keepdim=True) # (B, 1)
            pac_temp = torch.where(pred_entropy > 0.65, 0.40 + 0.20 * seeking, 0.10)
            
            sub_logits = logits[:, valid_ascii_bytes] / pac_temp
            if sub_logits.dim() == 3:
                sub_logits = sub_logits.squeeze(1)
            probs = torch.softmax(sub_logits, dim=-1)
            if probs.dim() > 2:
                probs = probs.view(batch_size, -1)
            next_sub = torch.multinomial(probs, num_samples=1)
            next_tok = valid_ascii_bytes[next_sub]
