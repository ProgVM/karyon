...
def run_survival_evaluation(agent, mode="system1", max_steps=50, device="cpu"):
    ...
    # 100% Vectorized Episodic Memory to cache hazard coordinates
    memory = BatchedEpisodicMemory(batch_size=1, capacity=100, device=device)
    ...
    for step in range(max_steps):
        ...
        if mode == "system2_episodic":
            ...
            for act_cand in range(3):
                ...
                # Key = candidate simulated position representation (first 8 elements of sensory vector)
                q_key = w_sim[:, :8]
        ...
        if info == "Food Consumed" or info == "Hazard Hit":
            # Key = current position representation, Value = homeostatic state, Surprise = absolute reward magnitude
            memory.write(w_t[:, :8], hu.state, surprise=torch.tensor([[abs(env_reward)]], device=device))
