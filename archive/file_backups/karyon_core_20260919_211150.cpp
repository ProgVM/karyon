...
public:
    ...
    torch::Tensor forward(torch::Tensor x) {
        // x: [batch, seq_len, dim]
        bool is_3d = (x.dim() == 3);
        auto x_norm = x / (torch::norm(x, -1, true) + 1e-6f);
        auto k_norm = memory_keys / (torch::norm(memory_keys, -1, true) + 1e-6f);

        torch::Tensor sim;
        if (is_3d) {
            // x_norm: [batch, seq_len, dim], k_norm: [num_basins, dim]
            // sim: [batch, seq_len, num_basins]
            sim = torch::matmul(x_norm, k_norm.t()) * 8.0f;
        } else {
            // x_norm: [batch, dim], k_norm: [num_basins, dim]
            // sim: [batch, num_basins]
            sim = torch::matmul(x_norm, k_norm.t()) * 8.0f;
        }
        ...
        if (is_3d) {
            // attn: [batch, seq_len, num_basins], memory_values: [num_basins, dim]
            // retrieved: [batch, seq_len, dim]
            retrieved = torch::matmul(attn, memory_values);
        } else {
            // attn: [batch, num_basins], memory_values: [num_basins, dim]
            // retrieved: [batch, dim]
            retrieved = torch::matmul(attn, memory_values);
        }
