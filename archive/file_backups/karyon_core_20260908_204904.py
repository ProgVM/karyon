...
if karyon_cpp is None:
    ...
    for candidate_name in ["karyon_cpp_ext_v33"]:
        try:
...
if karyon_cpp is None:
    print("[C++ JIT] Compiling and linking native Karyon C++20 architecture (v33.0 Master with Tsodyks-Markram Hopfield)...")
    try:
        karyon_cpp = load(
            name="karyon_cpp_ext_v33",
            sources=["karyon_core.cpp"],
            extra_cflags=["-O3", "-std=c++20"],
            verbose=False
        )
        sys.modules["karyon_cpp_ext_v33"] = karyon_cpp
        print("[C++ JIT] Native C++20 v33.0 Master architecture successfully compiled and initialized!")
    except Exception as e:
        if "already registered" in str(e):
            print("[C++ JIT] PyBind11 type registration conflict detected. Attempting fallback import...")
            for candidate_name in ["karyon_cpp_ext_v33", "karyon_cpp_ext_v32"]:
                try:
                    mod = importlib.import_module(candidate_name)
                    if _is_valid_karyon_cpp_module(mod):
