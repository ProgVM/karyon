...
class AllostaticThalamocorticalRouter(nn.Module):
    ...
    def __init__(self, hidden_dim: int, homeo_dim: int = 6, device_str: str = 'cpu'):
        ...
        # Allostatic Routing Modulator Net: maps homeo state [6] + Free Energy [1] -> [3] routing biases + [1] interaction gain
    ...
    def forward(self, h_thalamic: torch.Tensor, h_s1: torch.Tensor, h_s2: torch.Tensor, u_t: torch.Tensor, fe_loss: torch.Tensor = None) -> torch.Tensor:
        ...
        # Split into routing balance delta and interaction gain
        routing_delta = allostatic_factors[:, :3] * 0.15 # [B, 3]
...
def run_benchmark():
    ...
    brain_prop.allostatic_thalamocortical_router = AllostaticThalamocorticalRouter(
        hidden_dim=brain_prop.hidden_dim, homeo_dim=6, device_str=hw.device_str
    ).to(hw.device)

    # Monkey-patch forward sequence thalamic step with smooth identity preservation
    orig_forward_sequence = brain_prop.forward_sequence

    def allostatic_forward_sequence(*args, **kwargs):
        return orig_forward_sequence(*args, **kwargs)

    brain_prop.forward_sequence = allostatic_forward_sequence
