...
class Net2NetMorphogenesisEngine:
    ...
    def expand_agent_dimensions(
        ...
        sensor_inputs = {"text": torch.randn(1, 256, device=device)}
        ...
        with torch.no_grad():
            ...
            if hasattr(agent, 'gateway'):
                for channel_name, proj in list(agent.gateway.projections.items()):
                    agent.gateway.projections[channel_name] = Net2NetMorphogenesisEngine.expand_linear_layer(
                        proj, proj.out_features, None, device=device
                    )
